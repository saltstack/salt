...
def _read_conf_file(path):
    ...
    with salt.utils.fopen(path, 'r') as conf_file:
        ...
        for key, value in list(conf_opts.copy().items()):
...
def apply_cloud_providers_config(overrides, defaults=None):
    ...
    while True:
        ...
        for provider_alias, entries in list(providers.copy().items()):
            ...
            for driver, details in list(entries.items()):
    ...
    # extend from
    for provider_alias, entries in list(providers.copy().items()):
        for driver, details in list(entries.copy().items()):
...
def is_provider_configured(opts, provider, required_keys=()):
    ...
    for alias, drivers in list(opts['providers'].items()):
        for driver, provider_details in list(drivers.items()):
            if driver != provider:
                continue
