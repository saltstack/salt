...
def __virtual__():
    ...
    if __virtualname__ not in __opts__['fileserver_backend']:
        return False
    try:
        return __virtualname__ if gitfs.get_provider(__opts__, 'gitfs') else False
...
def init():
    ...
    '''
    return gitfs.init(__opts__, 'gitfs')

def purge_cache():
    '''
    Purge the fileserver cache
    '''
    return gitfs.purge_cache(__opts__, 'gitfs')
...
def update():
    ...
    return gitfs.update(__opts__, 'gitfs')
...
def envs(ignore_cache=False):
    ...
    '''
    return gitfs.envs(__opts__, 'gitfs', ignore_cache=ignore_cache)
...
def find_file(path, tgt_env='base', **kwargs):  # pylint: disable=W0613
    ...
    '''
    return gitfs.find_file(__opts__, 'gitfs', path, tgt_env=tgt_env, **kwargs)
...
def serve_file(load, fnd):
    ...
    '''
    return gitfs.serve_file(__opts__, 'gitfs', load, fnd)
...
def file_hash(load, fnd):
    ...
    '''
    return gitfs.file_hash(__opts__, 'gitfs', load, fnd)
...
def file_list(load):
    ...
    environment
    '''
    return gitfs.file_list(__opts__, 'gitfs', load)
...
def dir_list(load):
    ...
    '''
    return gitfs.dir_list(__opts__, 'gitfs', load)
