...
class CloudClient:
    ...
    def __init__(self, path=None, opts=None, config_dir=None, pillars=None):
        ...
        if pillars:
            ...
            for name, profile in pillars.pop("profiles", {}).items():
                ...
                driver = next(self.opts["providers"][provider].keys())
    ...
    def create(self, provider, names, **kwargs):
        ...
        if provider in providers:
            provider += ":{}".format(next(providers[provider].keys()))
        else:
            return False
        if isinstance(names, str):
            names = names.split(",")
...
class Map(Cloud):
    ...
    def run_map(self, dmap):
        ...
        create_list = sorted(dmap["create"], key=lambda x: x[1]["level"].items())
        output = {}
        if self.opts["parallel"]:
            parallel_data = []
        master_name = None
        ...
        if self.opts["parallel"] and parallel_data:
            ...
            if self.opts["start_action"]:
                ...
                for obj in output_multip:
                    next(obj.values())["ret"] = out[next(obj.keys())]
                    output.update(obj)
            else:
                for obj in output_multip:
                    output.update(obj)
