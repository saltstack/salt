...
class Map(Cloud):
    ...
    def run_map(self, dmap):
        ...
        possible_master_list = sorted(full_map.items(), key=lambda x: x[1]["level"])
        output = {}
        if self.opts["parallel"]:
            parallel_data = []
        master_name = None
        ...
        if master_name:
            ...
            if master_name not in dmap["create"]:
                ...
                master_host = master_host[master_name][
                    master_profile.get("ssh_interface", "public_ips")
                ]
                if not master_host:
                    raise SaltCloudSystemExit(
            ...
            # Otherwise, deploy it as a new master
                ...
                if (
                    salt.config.get_cloud_config_value(
                        "deploy", master_profile, self.opts
                    )
                    is False
                ):
                    raise SaltCloudSystemExit(
                ...
                priv, pub = salt.utils.cloud.gen_keys(
                    salt.config.get_cloud_config_value(
                        "keysize", master_profile, self.opts
                    )
                )
                ...
                if not isinstance(out, dict):
                    log.debug(
                        "Master creation details is not a dictionary: {}".format(out)
                    )
