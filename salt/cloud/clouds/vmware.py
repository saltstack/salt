    ...
    if datastore:
        ...
        if not datastore_ref:
            ...
            for ds_ref in datastore_list:
                log.trace("Found datastore (%s) with free space (%s) in datastore cluster (%s)", ds_ref.name, ds_ref.summary.freeSpace, datastore)
        ...
        log.trace("Using datastore (%s) for disk (%s), vm_name (%s)", datastore_ref.name, disk_label, vm_name)
...
def _edit_existing_network_adapter(network_adapter, new_network_name, adapter_type, switch_type, container_ref=None):
    ...
    if adapter_type in ["vmxnet", "vmxnet2", "vmxnet3", "e1000", "e1000e"]:
        ...
        if isinstance(network_adapter, type(edited_network_adapter)):
            ...
            log.debug("Changing type of '%s' from '%s' to '%s'", network_adapter.deviceInfo.label, type(network_adapter).__name__.rsplit(".", 1)[1][7:].lower(), adapter_type)
    else:
...
def _add_new_network_adapter_helper(network_adapter_label, network_name, adapter_type, switch_type, mac, container_ref=None):
    ...
    if adapter_type in ["vmxnet", "vmxnet2", "vmxnet3", "e1000", "e1000e"]:
        ...
        if not adapter_type:
            log.debug("The type of '%s' has not been specified. Creating of default type 'vmxnet3'", network_adapter_label)
        else:
            log.error("Cannot create network adapter of type '%s'. Creating '%s' of default type 'vmxnet3'", adapter_type, network_adapter_label)
...
def _add_new_cd_or_dvd_drive_helper(drive_label, controller_key, device_type, mode, iso_path):
    ...
    if device_type in ['datastore_iso_file', 'client_device']:
        ...
        if not device_type:
            log.debug("The 'device_type' of '%s' has not been specified.  Creating of default type 'client_device'", drive_label)
        else:
            log.error("Cannot create CD/DVD drive of type '%s'. Creating '%s' of default type 'client_device'", device_type, drive_label)
    ...
    if 'cd' in list(devices.keys()):
        ...
        for cd_drive_label in cd_drives_to_create:
            ...
            if not controller_key:
                log.error("No more available controllers for '%s'. All IDE controllers are currently in use", cd_drive_label)
            else:
...
def _wait_for_vmware_tools(vm_ref, max_wait):
    ...
    while time_counter < max_wait:
        if time_counter % 5 == 0:
            log.info("[ %s ] Waiting for VMware tools to be running [%s s]", vm_ref.name, time_counter)
        if six.text_type(vm_ref.summary.guest.toolsRunningStatus) == "guestToolsRunning":
            log.info("[ %s ] Successfully got VMware tools running on the guest in %s seconds", vm_ref.name, time_counter)
            return True

        time.sleep(1.0 - ((time.time() - starttime) % 1.0))
        time_counter += 1
    log.warning("[ %s ] Timeout Reached. VMware tools still not running after waiting for %s seconds", vm_ref.name, max_wait)
    return False
...
def _wait_for_ip(vm_ref, max_wait):
    ...
    while time_counter < max_wait_ip:
        if time_counter % 5 == 0:
            log.info("[ %s ] Waiting to retrieve IPv4 information [%s s]", vm_ref.name, time_counter)

        if vm_ref.summary.guest.ipAddress and _valid_ip(vm_ref.summary.guest.ipAddress):
            log.info("[ %s ] Successfully retrieved IPv4 information in %s seconds", vm_ref.name, time_counter)
        ...
        time.sleep(1.0 - ((time.time() - starttime) % 1.0))
        time_counter += 1
    log.warning("[ %s ] Timeout Reached. Unable to retrieve IPv4 information after waiting for %s seconds", vm_ref.name, max_wait_ip)
    return False
...
def create(vm_):
    ...
    if 'clonefrom' in vm_:
        ...
        if not datastore:
            ...
            if not datastore_ref:
                raise SaltCloudSystemExit("Specified datastore: '%s' does not exist", datastore)
    ...
    if hardware_version and object_ref is not None:
        hardware_version = "vmx-{0}".format(six.text_type(hardware_version).zfill(2))
        if hardware_version != object_ref.config.version:
            log.debug("Scheduling hardware version upgrade from %s to %s", object_ref.config.version, hardware_version)
...
def create_cluster(kwargs=None, call=None):
    ...
    if isinstance(folder, vim.Folder):
        ...
        log.debug("Created cluster %s under datacenter %s", cluster_name, datacenter.name)
...
def add_host(kwargs=None, call=None):
    ...
    if host_ssl_thumbprint:
        ...
        try:
            ...
            out = salt.utils.stringutils.to_unicode(p3.stdout.read())
    ...
    try:
        ...
        if isinstance(exc, vim.fault.SSLVerifyFault):
            ...
            log.info('Try again after setting the esxi_host_ssl_thumbprint to %s in provider configuration', exc.thumbprint)
        log.error(
            'Error while adding host %s: %s', host_name, exc,
            # Show the traceback if the debug logging level is enabled
            exc_info_on_loglevel=logging.DEBUG
        )
        return {host_name: 'failed to add host'}
