...
def _edit_existing_network_adapter(
    ...
    if adapter_type in ["vmxnet", "vmxnet2", "vmxnet3", "e1000", "e1000e"]:
        ...
        if isinstance(network_adapter, type(edited_network_adapter)):
            ...
            log.debug(
                "Changing type of '{0}' from '{1}' to '{2}'".format(
        ...
        if adapter_type:
            log.error(
                "Cannot change type of '{0}' to '{1}'. Not changing type".format(
                    network_adapter.deviceInfo.label,
                    adapter_type))
        edited_network_adapter = network_adapter
...
def _add_new_network_adapter_helper(
    ...
    if adapter_type in ["vmxnet", "vmxnet2", "vmxnet3", "e1000", "e1000e"]:
        network_spec.device = salt.utils.vmware.get_network_adapter_type(
            adapter_type)
    else:
        # If type not specified or does not match, create adapter of type
        # vmxnet3
        if not adapter_type:
            log.debug("The type of '{0}' has not been specified. Creating of default type 'vmxnet3'".format(
                network_adapter_label))
        else:
            log.error(
                "Cannot create network adapter of type '{0}'. Creating '{1}' of default type 'vmxnet3'".format(
...
def _add_new_scsi_controller_helper(
        scsi_controller_label,
        properties,
        bus_number):
    random_key = randint(-1050, -1000)
    adapter_type = properties['type'].strip(
    ).lower() if 'type' in properties else None
    bus_sharing = properties['bus_sharing'].strip(
    ).lower() if 'bus_sharing' in properties else None
    ...
    if bus_sharing == "virtual":
        # Virtual disks can be shared between virtual machines on the same
        # server
...
def _add_new_ide_controller_helper(
...
def _edit_existing_cd_or_dvd_drive(drive, device_type, mode, iso_path):
    ...
    drive_spec.device = _set_cd_or_dvd_backing_type(
        drive, device_type, mode, iso_path)
...
def _add_new_cd_or_dvd_drive_helper(
    ...
    if device_type in ['datastore_iso_file', 'client_device']:
        ...
        # If device_type not specified or does not match, create drive of
        # Client type with Passthough mode
        if not device_type:
            log.debug(
                "The 'device_type' of '{0}' has not been specified. Creating of default type 'client_device'".format(drive_label))
        else:
            log.error(
                "Cannot create CD/DVD drive of type '{0}'. Creating '{1}' of default type 'client_device'".format(
...
def _manage_devices(devices, vm=None, container_ref=None):
        ...
        for device in vm.config.hardware.device:
            if isinstance(device, vim.vm.device.VirtualDisk):
                ...
                if 'disk' in list(devices.keys()):
                    # there is atleast one disk specified to be
                    # created/configured
                    unit_number += 1
                ...
                if 'network' in list(devices.keys()):
                    # there is atleast one network adapter specified to be
                    # created/configured
                    existing_network_adapters_label.append(
                        device.deviceInfo.label)
                    if device.deviceInfo.label in list(
                            devices['network'].keys()):
                        network_name = devices['network'][
                            device.deviceInfo.label]['name']
                        adapter_type = devices['network'][
                            device.deviceInfo.label]['adapter_type'] if 'adapter_type' in devices['network'][
                            device.deviceInfo.label] else ''
                        switch_type = devices['network'][
                            device.deviceInfo.label]['switch_type'] if 'switch_type' in devices['network'][
                            device.deviceInfo.label] else ''
                        network_spec = _edit_existing_network_adapter(
                            device, network_name, adapter_type, switch_type)
                ...
                if 'scsi' in list(devices.keys()):
                    # there is atleast one SCSI controller specified to be
                    # created/configured
                    bus_number += 1
                    existing_scsi_controllers_label.append(
                        device.deviceInfo.label)
                    if device.deviceInfo.label in list(devices['scsi'].keys()):
                        # Modify the existing SCSI controller
                        scsi_controller_properties = devices[
                            'scsi'][device.deviceInfo.label]
                        bus_sharing = scsi_controller_properties['bus_sharing'].strip(
                        ).lower() if 'bus_sharing' in scsi_controller_properties else None
                        if bus_sharing and bus_sharing in [
                                'virtual', 'physical', 'no']:
                            bus_sharing = '{0}Sharing'.format(bus_sharing)
                            if bus_sharing != device.sharedBus:
                                # Only edit the SCSI controller if bus_sharing
                                # is different
                ...
                if 'cd' in list(devices.keys()):
                    # there is atleast one cd/dvd drive specified to be
                    # created/configured
                    existing_cd_drives_label.append(device.deviceInfo.label)
                    if device.deviceInfo.label in list(devices['cd'].keys()):
                        device_type = devices['cd'][
                            device.deviceInfo.label]['device_type'] if 'device_type' in devices['cd'][
                            device.deviceInfo.label] else ''
                        mode = devices['cd'][
                            device.deviceInfo.label]['mode'] if 'mode' in devices['cd'][
                            device.deviceInfo.label] else ''
                        iso_path = devices['cd'][
                            device.deviceInfo.label]['iso_path'] if 'iso_path' in devices['cd'][
                            device.deviceInfo.label] else ''
    ...
    if 'network' in list(devices.keys()):
        network_adapters_to_create = sorted(
            set(devices['network'].keys()) - set(existing_network_adapters_label))
        log.debug("Networks adapters to create: {0}".format(
            network_adapters_to_create)) if network_adapters_to_create else None  # pylint: disable=W0106
        for network_adapter_label in network_adapters_to_create:
            network_name = devices['network'][network_adapter_label]['name']
            adapter_type = devices['network'][network_adapter_label][
                'adapter_type'] if 'adapter_type' in devices['network'][network_adapter_label] else ''
            switch_type = devices['network'][network_adapter_label][
                'switch_type'] if 'switch_type' in devices['network'][network_adapter_label] else ''
            # create the network adapter
            network_spec = _add_new_network_adapter_helper(
                network_adapter_label, network_name, adapter_type, switch_type, container_ref)
            adapter_mapping = _set_network_adapter_mapping(
    ...
    if 'scsi' in list(devices.keys()):
        scsi_controllers_to_create = sorted(
            set(devices['scsi'].keys()) - set(existing_scsi_controllers_label))
        log.debug("SCSI controllers to create: {0}".format(
            scsi_controllers_to_create)) if scsi_controllers_to_create else None  # pylint: disable=W0106
    ...
    if 'ide' in list(devices.keys()):
        ide_controllers_to_create = sorted(
            set(devices['ide'].keys()) - set(existing_ide_controllers_label))
        log.debug('IDE controllers to create: {0}'.format(
            ide_controllers_to_create)) if ide_controllers_to_create else None  # pylint: disable=W0106
        for ide_controller_label in ide_controllers_to_create:
            # create the IDE controller
            ide_spec = _add_new_ide_controller_helper(
                ide_controller_label, None, bus_number)
            device_specs.append(ide_spec)
            bus_number += 1

    if 'disk' in list(devices.keys()):
        disks_to_create = sorted(
            set(devices['disk'].keys()) - set(existing_disks_label))
        log.debug("Hard disks to create: {0}".format(
            disks_to_create)) if disks_to_create else None  # pylint: disable=W0106
        for disk_label in disks_to_create:
            # create the disk
            size_gb = float(devices['disk'][disk_label]['size'])
            thin_provision = bool(devices['disk'][disk_label][
                                  'thin_provision']) if 'thin_provision' in devices['disk'][disk_label] else False
            disk_spec = _add_new_hard_disk_helper(
                disk_label, size_gb, unit_number, thin_provision=thin_provision)
            ...
            if 'controller' in devices['disk'][disk_label]:
                for spec in device_specs:
                    if spec.device.deviceInfo.label == devices[
                            'disk'][disk_label]['controller']:
    ...
    if 'cd' in list(devices.keys()):
        cd_drives_to_create = sorted(
            set(devices['cd'].keys()) - set(existing_cd_drives_label))
        log.debug("CD/DVD drives to create: {0}".format(
            cd_drives_to_create)) if cd_drives_to_create else None  # pylint: disable=W0106
        for cd_drive_label in cd_drives_to_create:
            # create the CD/DVD drive
            device_type = devices['cd'][cd_drive_label][
                'device_type'] if 'device_type' in devices['cd'][cd_drive_label] else ''
            mode = devices['cd'][cd_drive_label][
                'mode'] if 'mode' in devices['cd'][cd_drive_label] else ''
            iso_path = devices['cd'][cd_drive_label][
                'iso_path'] if 'iso_path' in devices['cd'][cd_drive_label] else ''
            controller_key = None
            ...
            if 'controller' in devices['cd'][cd_drive_label]:
                for spec in device_specs:
                    if spec.device.deviceInfo.label == devices[
                            'cd'][cd_drive_label]['controller']:
                        controller_key = spec.device.key
                        ide_controllers[controller_key] = 0
                        break
            else:
                for ide_controller_key, num_devices in six.iteritems(
                        ide_controllers):
                    if num_devices < 2:
            ...
            if not controller_key:
                log.error("No more available controllers for '{0}'. All IDE controllers are currently in use".format(
                    cd_drive_label))
            else:
...
def _wait_for_vmware_tools(vm_ref, max_wait):
    ...
    while time_counter < max_wait:
        if time_counter % 5 == 0:
            log.info(
                "[ {0} ] Waiting for VMware tools to be running [{1} s]".format(
                    vm_ref.name, time_counter))
        if str(vm_ref.summary.guest.toolsRunningStatus) == "guestToolsRunning":
            log.info(
                "[ {0} ] Successfully got VMware tools running on the guest in {1} seconds".format(
                    vm_ref.name, time_counter))
            return True
        ...
        time_counter += 1
    log.warning(
        "[ {0} ] Timeout Reached. VMware tools still not running after waiting for {1} seconds".format(
            vm_ref.name,
            max_wait))
    return False
...
def _wait_for_ip(vm_ref, max_wait):
    ...
    while time_counter < max_wait_ip:
        if time_counter % 5 == 0:
            log.info(
                "[ {0} ] Waiting to retrieve IPv4 information [{1} s]".format(
                    vm_ref.name, time_counter))

        if vm_ref.summary.guest.ipAddress and _valid_ip(
                vm_ref.summary.guest.ipAddress):
            log.info(
                "[ {0} ] Successfully retrieved IPv4 information in {1} seconds".format(
                    vm_ref.name, time_counter))
        ...
        time_counter += 1
    log.warning(
        "[ {0} ] Timeout Reached. Unable to retrieve IPv4 information after waiting for {1} seconds".format(
            vm_ref.name,
            max_wait_ip))
    return False
...
def _wait_for_host(host_ref, task_type, sleep_seconds=5, log_level='debug'):
    ...
    while host_ref.runtime.connectionState != 'notResponding':
        if time_counter % sleep_seconds == 0:
            message = "[ {0} ] Waiting for host {1} to finish [{2} s]".format(
    ...
    if host_ref.runtime.connectionState == 'connected':
        message = "[ {0} ] Successfully completed host {1} in {2} seconds".format(
            host_ref.name, task_type, time_counter)
        if log_level == 'info':
            log.info(message)
        else:
...
def _format_instance_info_select(vm, selection):
    ...
    if 'image' in selection:
        vm_select_info['image'] = "{0} (Detected)".format(
            vm["config.guestFullName"]) if "config.guestFullName" in vm else "N/A"

    if 'size' in selection:
        cpu = vm[
            "config.hardware.numCPU"] if "config.hardware.numCPU" in vm else "N/A"
        ram = "{0} MB".format(
            vm["config.hardware.memoryMB"]) if "config.hardware.memoryMB" in vm else "N/A"
        vm_select_info['size'] = u"cpu: {0}\nram: {1}".format(cpu, ram)

    if 'state' in selection:
        vm_select_info['state'] = str(
            vm["summary.runtime.powerState"]) if "summary.runtime.powerState" in vm else "N/A"

    if 'guest_id' in selection:
        vm_select_info['guest_id'] = vm[
            "config.guestId"] if "config.guestId" in vm else "N/A"

    if 'hostname' in selection:
        vm_select_info['hostname'] = vm["object"].guest.hostName

    if 'path' in selection:
        vm_select_info['path'] = vm[
            "config.files.vmPathName"] if "config.files.vmPathName" in vm else "N/A"

    if 'tools_status' in selection:
        vm_select_info['tools_status'] = str(
            vm["guest.toolsStatus"]) if "guest.toolsStatus" in vm else "N/A"
        ...
        if "config.hardware.device" in vm:
            for device in vm["config.hardware.device"]:
                ...
                if 'devices' in selection:
                    device_full_info[
                        device.deviceInfo.label]['key'] = device.key,
                    device_full_info[device.deviceInfo.label][
                    ...
                    if hasattr(device,
                               'controllerKey') and device.controllerKey:
                        device_full_info[device.deviceInfo.label][
                    ...
                    if hasattr(device, 'device'):
                        device_full_info[device.deviceInfo.label][
                            'deviceKeys'] = device.device

                    if hasattr(device, 'videoRamSizeInKB'):
                        device_full_info[device.deviceInfo.label][
                            'videoRamSizeInKB'] = device.videoRamSizeInKB
    ...
    if 'storage' in selection:
        storage_full_info = {
            'committed': int(
                vm["summary.storage.committed"]) if "summary.storage.committed" in vm else "N/A",
            'uncommitted': int(
                vm["summary.storage.uncommitted"]) if "summary.storage.uncommitted" in vm else "N/A",
            'unshared': int(
                vm["summary.storage.unshared"]) if "summary.storage.unshared" in vm else "N/A"}
...
def _format_instance_info(vm):
    ...
    ram = "{0} MB".format(vm["config.hardware.memoryMB"]
                          ) if "config.hardware.memoryMB" in vm else "N/A"
    vm_full_info = {
        'id': str(
            vm['name']),
        'image': "{0} (Detected)".format(
            vm["config.guestFullName"]) if "config.guestFullName" in vm else "N/A",
        'size': u"cpu: {0}\nram: {1}".format(
                cpu,
                ram),
        ...
        'guest_id': str(
            vm["config.guestId"]) if "config.guestId" in vm else "N/A",
        'hostname': str(
            vm["object"].guest.hostName),
        'mac_addresses': device_mac_addresses,
        'networks': network_full_info,
        'path': str(
            vm["config.files.vmPathName"]) if "config.files.vmPathName" in vm else "N/A",
        'tools_status': str(
            vm["guest.toolsStatus"]) if "guest.toolsStatus" in vm else "N/A"}
...
def list_datastore_clusters(kwargs=None, call=None):
    ...
    return {
        'Datastore Clusters': salt.utils.vmware.list_datastore_clusters(
            _get_si())}
    ...
    for vm in vm_list:
        ...
        vm_info = {
            ...
            'state': str(
                vm["summary.runtime.powerState"]) if "summary.runtime.powerState" in vm else "N/A",
            'private_ips': [
                vm["guest.ipAddress"]] if "guest.ipAddress" in vm else [],
            'public_ips': []}
    ...
    for vm in vm_list:
        if vm["rootSnapshot"]:
            if kwargs and kwargs.get('name') == vm["name"]:
                return {
                    vm["name"]: _get_snapshots(
                        vm["snapshot"].rootSnapshotList)}
            else:
                ret[vm["name"]] = _get_snapshots(
                    vm["snapshot"].rootSnapshotList)
...
def create(vm_):
    ...
    if 'clonefrom' in vm_:
        # If datacenter is specified, set the container reference to start
        # search from it instead
        container_ref = None
        if datacenter:
        ...
        object_ref = salt.utils.vmware.get_mor_by_property(
            _get_si(), vim.VirtualMachine, vm_['clonefrom'], container_ref=container_ref)
        if object_ref:
    ...
    # Either a cluster, or a resource pool must be specified when cloning from
    # template or creating.
    if resourcepool:
        resourcepool_ref = salt.utils.vmware.get_mor_by_property(
            _get_si(), vim.ResourcePool, resourcepool, container_ref=container_ref)
        if not resourcepool_ref:
            log.error(
                "Specified resource pool: '{0}' does not exist".format(resourcepool))
        ...
        cluster_ref = salt.utils.vmware.get_mor_by_property(
            _get_si(), vim.ClusterComputeResource, cluster, container_ref=container_ref)
        if not cluster_ref:
            log.error(
                "Specified cluster: '{0}' does not exist".format(cluster))
            if not clone_type or clone_type == "template":
                raise SaltCloudSystemExit(
                    'You must specify a cluster that exists.')
        else:
        ...
        log.debug(
            "Using resource pool used by the {0} {1}".format(
                clone_type, vm_['clonefrom']))

    # Either a datacenter or a folder can be optionally specified when cloning, required when creating.
    # If not specified when cloning, the existing VM/template\'s parent folder
    # is used.
    if folder:
        folder_ref = salt.utils.vmware.get_mor_by_property(
            _get_si(), vim.Folder, folder, container_ref=container_ref)
        if not folder_ref:
            log.error("Specified folder: '{0}' does not exist".format(folder))
            log.debug(
                "Using folder in which {0} {1} is present".format(
                    clone_type, vm_['clonefrom']))
            folder_ref = object_ref.parent
    elif datacenter:
        if not datacenter_ref:
            log.error(
                "Specified datacenter: '{0}' does not exist".format(datacenter))
            log.debug(
                "Using datacenter folder in which {0} {1} is present".format(
                    clone_type, vm_['clonefrom']))
            folder_ref = object_ref.parent
        else:
    ...
    if 'clonefrom' in vm_:
        ...
        if datastore:
            datastore_ref = salt.utils.vmware.get_mor_by_property(
                _get_si(), vim.Datastore, datastore, container_ref=container_ref)
            if datastore_ref:
                ...
                datastore_cluster_ref = salt.utils.vmware.get_mor_by_property(
                    _get_si(), vim.StoragePod, datastore, container_ref=container_ref)
                if not datastore_cluster_ref:
                    log.error(
                        "Specified datastore/datastore cluster: '{0}' does not exist".format(datastore))
                    log.debug(
                        "Using datastore used by the {0} {1}".format(
                            clone_type, vm_['clonefrom']))
        else:
    ...
    if hardware_version:
        ...
        if hardware_version != object_ref.config.version:
            log.debug(
                "Scheduling hardware version upgrade from {0} to {1}".format(
            ...
            log.debug(
                "Virtual hardware version already set to {0}".format(hardware_version))
    ...
    if memory:
        try:
            ...
            if memory_unit.lower() == "mb":
                ...
                err_msg = "Invalid memory type specified: '{0}'".format(
                    memory_unit)
                log.error(err_msg)
    ...
    if 'clonefrom' in vm_:
        ...
        if customization and (devices and 'network' in list(
                devices.keys())) and 'Windows' not in object_ref.config.guestFullName:
        ...
        if customization and (devices and 'network' in list(
                devices.keys())) and 'Windows' in object_ref.config.guestFullName:
            global_ip = vim.vm.customization.GlobalIPSettings()
    ...
    try:
        ...
        if 'clonefrom' in vm_:
            log.info(
                "Creating {0} from {1}({2})".format(
                    vm_['name'],
                    clone_type,
                    vm_['clonefrom']))

            if datastore and not datastore_ref and datastore_cluster_ref:
                # datastore cluster has been specified so apply Storage DRS
                # recomendations
    ...
    new_vm_ref = salt.utils.vmware.get_mor_by_property(
        _get_si(), vim.VirtualMachine, vm_name, container_ref=container_ref)
    ...
    # If it a template or if it does not need to be powered on then do not
    # wait for the IP
...
def create_datacenter(kwargs=None, call=None):
    ...
    datacenter_name = kwargs.get(
        'name') if kwargs and 'name' in kwargs else None
    ...
    datacenter_ref = salt.utils.vmware.get_mor_by_property(
        _get_si(), vim.Datacenter, datacenter_name)
    if datacenter_ref:
...
def create_cluster(kwargs=None, call=None):
    ...
    datacenter = kwargs.get(
        'datacenter') if kwargs and 'datacenter' in kwargs else None
    ...
    cluster_ref = salt.utils.vmware.get_mor_by_property(
        _get_si(), vim.ClusterComputeResource, cluster_name)
    if cluster_ref:
    ...
    if isinstance(folder, vim.Folder):
        ...
        log.debug(
            "Created cluster {0} under datacenter {1}".format(
...
def upgrade_tools(name, reboot=False, call=None):
    ...
    vm_ref = salt.utils.vmware.get_mor_by_property(
        _get_si(), vim.VirtualMachine, name)
...
def list_hosts_by_cluster(kwargs=None, call=None):
    ...
    cluster_name = kwargs.get(
        'cluster') if kwargs and 'cluster' in kwargs else None
        ...
        if datacenter_name and datacenter_name == datacenter['name']:
            return {
                'Clusters by Datacenter': {
                    datacenter_name: ret[datacenter_name]}}
...
def list_hbas(kwargs=None, call=None):
    ...
    ret = {}
    hba_type = kwargs.get('type').lower(
    ) if kwargs and 'type' in kwargs else None
    host_name = kwargs.get('host') if kwargs and 'host' in kwargs else None
    host_properties = [
        "name",
...
def list_dvs(kwargs=None, call=None):
    ...
    return {
        'Distributed Virtual Switches': salt.utils.vmware.list_dvs(
            _get_si())}
...
def enter_maintenance_mode(kwargs=None, call=None):
    ...
    try:
        task = host_ref.EnterMaintenanceMode(
            timeout=0, evacuatePoweredOffVms=True)
        salt.utils.vmware.wait_for_task(
            task, host_name, 'enter maintenance mode')
    except Exception as exc:
        log.error(
...
def exit_maintenance_mode(kwargs=None, call=None):
    ...
    try:
        ...
        salt.utils.vmware.wait_for_task(
            task, host_name, 'exit maintenance mode')
    except Exception as exc:
        log.error(
...
def create_folder(kwargs=None, call=None):
    ...
    for index, folder_name in enumerate(
        ...
        if isinstance(folder_ref, vim.Folder):
            ...
            log.debug(
                "Path {0}/ exists in the inventory".format(inventory_path))
            ...
            if not folder_refs:
                ...
                log.debug(
                    "Creating folder {0} under rootFolder in the inventory".format(folder_name))
                folder_refs.append(
                    si.content.rootFolder.CreateFolder(folder_name))
            else:
                # Create the folder under the parent folder
                log.debug(
                    "Creating path {0}/ in the inventory".format(inventory_path))
                folder_refs.append(
                    folder_refs[
                        index -
                        1].CreateFolder(folder_name))
...
def create_snapshot(name, kwargs=None, call=None):
    ...
    snapshot_name = kwargs.get(
        'snapshot_name') if kwargs and 'snapshot_name' in kwargs else None
    ...
    if vm_ref.summary.runtime.powerState != "poweredOn":
        log.debug(
            'VM {0} is not powered on. Setting both memdump and quiesce to False'.format(name))
        memdump = False
        quiesce = False
    ...
    try:
        ...
        salt.utils.vmware.wait_for_task(
            task, name, 'create snapshot', 5, 'info')
    except Exception as exc:
        log.error(
    ...
    return {
        'Snapshot created successfully': _get_snapshots(
            vm_ref.snapshot.rootSnapshotList,
...
def revert_to_snapshot(name, kwargs=None, call=None):
    ...
    try:
        task = vm_ref.RevertToCurrentSnapshot(
            suppressPowerOn=suppress_power_on)
        salt.utils.vmware.wait_for_task(
            task, name, 'revert to snapshot', 5, 'info')
...
def remove_all_snapshots(name, kwargs=None, call=None):
    ...
    connection = _str_to_bool(kwargs.get(
        'merge_snapshots')) if kwargs and 'merge_snapshots' in kwargs else True
    ...
    try:
        ...
        salt.utils.vmware.wait_for_task(
            task, name, 'remove snapshots', 5, 'info')
    except Exception as exc:
        log.error(
...
def add_host(kwargs=None, call=None):
    ...
    if (cluster_name and datacenter_name) or not(
            cluster_name or datacenter_name):
        raise SaltCloudSystemExit(
    ...
    if host_ssl_thumbprint:
        ...
        log.warning(
            'SSL thumbprint has not been specified in provider configuration')
        try:
            log.debug(
                'Trying to get the SSL thumbprint directly from the host system')
            p1 = subprocess.Popen(
            ...
            log.debug(
                'SSL thumbprint received from the host system: {0}'.format(ssl_thumbprint))
    ...
    try:
        ...
        salt.utils.vmware.wait_for_task(
            task, host_name, 'add host system', 5, 'info')
    except Exception as exc:
        if isinstance(exc, vim.fault.SSLVerifyFault):
            log.error(
                'Authenticity of the host\'s SSL certificate is not verified')
            log.info(
                'Try again after setting the esxi_host_ssl_thumbprint to {0} in provider configuration'.format(
                    exc.thumbprint))
        log.error(
...
def connect_host(kwargs=None, call=None):
    ...
    try:
        ...
        salt.utils.vmware.wait_for_task(
            task, host_name, 'connect host', 5, 'info')
    except Exception as exc:
        log.error(
...
def reboot_host(kwargs=None, call=None):
    ...
    host_name = kwargs.get('host') if kwargs and 'host' in kwargs else None
    force = _str_to_bool(
        kwargs.get('force')) if kwargs and 'force' in kwargs else False
...
def create_datastore_cluster(kwargs=None, call=None):
    ...
    datastore_cluster_name = kwargs.get(
        'name') if kwargs and 'name' in kwargs else None
    datacenter_name = kwargs.get(
        'datacenter') if kwargs and 'datacenter' in kwargs else None
    ...
    datastore_cluster_ref = salt.utils.vmware.get_mor_by_property(
        _get_si(), vim.StoragePod, datastore_cluster_name)
    ...
    try:
        datacenter_ref.datastoreFolder.CreateStoragePod(
