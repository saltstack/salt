...
def create_node(vm_, newid):
    ...
    log.debug(
        'Preparing to generate a node using these parameters: %s',
        newnode
    )
...
def wait_for_state(vmid, state, timeout=300):
    ...
    while True:
        ...
        if time.time() - start_time > timeout:
            log.debug(
                'Timeout reached while waiting for %s to become %s',
                node['name'], state
            )
            return False
        node = get_vm_status(vmid=vmid)
        log.debug(
            'State for %s is: "%s" instead of "%s"',
...
def set_vm_status(status, name=None, vmid=None):
    ...
    if vmid is not None:
        log.debug(
            'set_vm_status: via ID - VMID %s (%s): %s',
            vmid, name, status
        )
        vmobj = _get_vm_by_id(vmid)
    else:
        log.debug(
            'set_vm_status: via name - VMID %s (%s): %s',
            vmid, name, status
        )
        vmobj = _get_vm_by_name(name)
    ...
    if not vmobj or 'node' not in vmobj or 'type' not in vmobj or 'vmid' not in vmobj:
        log.error(
            'Unable to set status %s for %s (%s)',
            status, name, vmid
        )
...
def get_vm_status(vmid=None, name=None):
    ...
    if vmobj is not None and 'node' in vmobj:
        log.debug(
            "VM_STATUS: Has desired info. Retrieving.. (%s)",
            vmobj['name']
        )
