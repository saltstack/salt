...
def create_network_interface(call=None, kwargs=None):
    ...
    try:
        ...
        log.warn("Network interface creation couldn't be polled. It's likely that we're reusing an existing interface. ({0})".format(exc))
...
def request_instance(vm_):
    ...
    if win_installer or (vm_password is not None and not disable_password_authentication):
        ...
        if complexity < 3:
            raise SaltCloudSystemExit(
                'The admin password must contain at least 3 of the following types: upper, lower, digits, special characters'
            )
...
def _get_block_blob_service(kwargs=None):
    ...
    if not storage_key:
        ...
        storage_key = next(storage_keys.itervalues())
...
def create_or_update_vmextension(call=None, kwargs=None):  # pylint: disable=unused-argument
    ...
    try:
        ...
        log.error('Error attempting to create the VM extension: {0}'.format(str(e)))
