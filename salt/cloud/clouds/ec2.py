...
def query(params=None, setname=None, requesturl=None, location=None,
    ...
    while attempts > 0:
        ...
        try:
            ...
            log.debug('EC2 Response Status Code: %s', result.status_code)
...
def ssh_interface(vm_):
    ...
    if ret not in ('public_ips', 'private_ips'):
        log.warning(
            'Invalid ssh_interface: %s. Allowed options are '
            '("public_ips", "private_ips"). Defaulting to "public_ips".',
            ret
        )
        ret = 'public_ips'
    return ret
...
def request_instance(vm_=None, call=None):
    ...
    if set_del_root_vol_on_destroy:
        ...
        log.info(
            'Attempting to look up root device name for image id %s on '
    ...
    if spot_config:
        ...
        def __query_spot_instance_request(sir_id, location):
            ...
            if state in ['cancelled', 'failed', 'closed']:
                ...
                log.error(
                    'Spot instance request resulted in state \'%s\'. '
                    'Nothing else we can do here.', state
                )
                return False
...
def query_instance(vm_=None, call=None):
    ...
    while attempts > 0:
        ...
        if isinstance(data, list) and not data:
            log.warning(
                'Query returned an empty list. %s attempts remaining.',
                attempts
            )
            attempts -= 1
            # Just a little delay between attempts...
