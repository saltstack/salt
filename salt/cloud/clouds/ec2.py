...
def _create_eni(interface, eip=None):
    ...
    if 'SecondaryPrivateIpAddressCount' in interface:
        params['SecondaryPrivateIpAddressCount'] = interface['SecondaryPrivateIpAddressCount']
...
def _associate_eip_with_interface(eni_id, eip_id, private_ip=None):
    ...
    result = query(params, return_root=True)
    if not result[1].get('return') or not result[2].get('associationId'):
        raise SaltCloudException('Could not associate elastic ip address <%s> '+
                                 'with network interface <%s>' %
                                 (eip_id, eni_id))
