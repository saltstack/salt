...
def avail_locations(conn=None, call=None):
    ...
    for img in locations:
        if isinstance(img.name, six.string_types) and not six.PY3:
            ...
            img_name = six.text_type(img.name)
...
def avail_sizes(conn=None, call=None):
    ...
    for size in sizes:
        if isinstance(size.name, six.string_types) and not six.PY3:
            ...
            size_name = six.text_type(size.name)
...
def get_size(conn, vm_):
    ...
    for size in sizes:
        if vm_size and six.text_type(vm_size) in (six.text_type(size.id), six.text_type(size.name)):
            return size
    raise SaltCloudNotFound(
