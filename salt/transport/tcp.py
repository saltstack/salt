...
class AsyncTCPReqChannel(salt.transport.client.ReqChannel):
    ...
    @tornado.gen.coroutine
    def send(self, load, tries=3, timeout=60, raw=False, StreamClosedError=iostream.StreamClosedError):
        '''
...
class SaltMessageServer(tornado.tcpserver.TCPServer, object):
    ...
    @tornado.gen.coroutine
    def handle_stream(self, stream, address, StreamClosedError=iostream.StreamClosedError):
        '''
...
class TCPClientKeepAlive(tornado.tcpclient.TCPClient):
    ...
    def _create_stream(self, max_buffer_size, af, addr, **kwargs):  # pylint: disable=unused-argument
        ...
        stream = iostream.IOStream(sock, max_buffer_size=max_buffer_size)
        ...
        if stream is not None:
            if not stream.closed():
                ...
                try:
                    ...
                    instance_io_loop.add_callback(stream.close)
