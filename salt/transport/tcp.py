    ...
    async def handle_stream(self, stream):
        ...
        while not stream.closed():
            try:
                ...
                for framed_msg in unpacker:
                    body = framed_msg["body"]
                    # Await the payload handler inline instead of firing
                    # it as a background task.  ``create_task`` here made
                    # the reader loop return immediately, so under
                    # sustained publish load (~5000 events/sec on the
                    # stress rig) tasks accumulated in the io_loop
                    # faster than they could complete: 909,120 pending
                    # tasks on the EventPublisher after ~5 min drove RSS
                    # to 10 GB.  Awaiting inline gives natural TCP
                    # backpressure -- if publishing is slow the reader
                    # stops, the kernel's pull-socket buffer absorbs a
                    # bounded burst, the peer eventually blocks on
                    # write.
                    try:
                        await self.payload_handler(body)
                    except Exception as exc:  # pylint: disable=broad-except
                        # A misbehaving handler must not break the whole
                        # reader loop; log and continue.  Matches the
                        # pre-await behavior where ``create_task``
                        # swallowed the failure into a fire-and-forget
                        # task.
                        log.error(
