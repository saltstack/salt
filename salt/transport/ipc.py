    ...
    def handle_stream(self, stream):
        ...
        while not stream.closed():
            try:
                ...
                if exc.errno == 0:
                    log.trace(
                        'Exception occured with error number 0, spurious '
                        'exception: %s', exc
                    )
                else:
...
class IPCClient(object):
    ...
    def _connect(self, timeout=None):
        ...
        while True:
            ...
            try:
                log.trace(
                    'IPCClient: Connecting to socket: %s',
                    self.socket_path
                )
...
class IPCMessagePublisher(object):
    ...
    def start(self):
        ...
        # Start up the ioloop
        log.trace(
            'IPCMessagePublisher: binding to socket: %s',
            self.socket_path
        )
        if isinstance(self.socket_path, int):
            self.sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
...
class IPCMessageSubscriber(IPCClient):
    ...
    def _read_sync(self, timeout):
        ...
        try:
            ...
            log.trace(
                'Subscriber disconnected from IPC %s',
                self.socket_path
            )
            self._read_stream_future = None
            exc_to_raise = exc
    ...
    def read_async(self, callback):
        ...
        while not self.connected():
            try:
                ...
                log.error(
                    'Exception occurred while Subscriber connecting: %s',
                    exc
                )
                yield tornado.gen.sleep(1)
