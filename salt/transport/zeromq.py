...
class AsyncZeroMQReqChannel(salt.transport.client.ReqChannel):
    ...
    def __new__(cls, opts, **kwargs):
        ...
        if obj is None:
            ...
            log.trace(
                'Inserted key into loop_instance_map id %s for key %s and '
                'process %s', id(loop_instance_map), key, os.getpid()
            )
        else:
...
class ZeroMQReqServerChannel(salt.transport.mixins.auth.AESReqServerMixin, salt.transport.server.ReqServerChannel):
    ...
    def handle_message(self, stream, payload):
        ...
        if not isinstance(payload, dict) or not isinstance(payload.get('load'), dict):
            log.error(
                'payload and load must be a dict. Payload was: %s and '
                'load was %s', payload, payload.get('load')
            )
