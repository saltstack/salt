...
class AESPubClientMixin(object):
    def _verify_master_signature(self, payload):
        if payload.get(b'sig') and self.opts.get('sign_pub_messages'):
            # Verify that the signature is valid
            master_pubkey_path = os.path.join(self.opts['pki_dir'], 'minion_master.pub')
            if not salt.crypt.verify_signature(master_pubkey_path, payload[b'load'], payload.get(b'sig')):
    ...
    def _decode_payload(self, payload):
        ...
        if payload[b'enc'] == b'aes':
            self._verify_master_signature(payload)
            try:
                payload[b'load'] = self.auth.crypticle.loads(payload[b'load'])
...
class AESReqServerMixin(object):
    ...
    def _decode_payload(self, payload):
        ...
        if payload[b'enc'] == b'aes':
            try:
                payload[b'load'] = self.crypticle.loads(payload[b'load'])
            except salt.crypt.AuthenticationError:
    ...
    def _auth(self, load):
        ...
        if not salt.utils.verify.valid_id(self.opts, load[b'id']):
            log.info(
                'Authentication request from invalid id {0}'.format(load[b'id'])
                )
            return {'enc': 'clear',
                    'load': {'ret': False}}
        log.info('Authentication request from {0}'.format(load[b'id']))
        ...
        auto_reject = self.auto_key.check_autoreject(load[b'id'])
        auto_sign = self.auto_key.check_autosign(load[b'id'])
        pki_dir = salt.utils.to_bytes(self.opts['pki_dir'])

        pubfn = os.path.join(pki_dir,
                             b'minions',
                             load[b'id'])
        pubfn_pend = os.path.join(pki_dir,
                                  b'minions_pre',
                                  load[b'id'])
        pubfn_rejected = os.path.join(pki_dir,
                                      b'minions_rejected',
                                      load[b'id'])
        pubfn_denied = os.path.join(pki_dir,
                                    b'minions_denied',
                                    load[b'id'])
        if self.opts['open_mode']:
            ...
            log.info('Public key rejected for {0}. Key is present in '
                     'rejection key dir.'.format(load[b'id']))
            eload = {'result': False,
                     'id': load[b'id'],
                     'pub': load[b'pub']}
            self.event.fire_event(eload, salt.utils.event.tagify(prefix='auth'))
            ...
            if salt.utils.fopen(pubfn, 'rb').read().strip() != load[b'pub'].strip():
                log.error(
                    'Authentication attempt from {0} failed, the public '
                    'keys did not match. This may be an attempt to compromise '
                    'the Salt cluster.'.format(load[b'id'])
                )
                # put denied minion key into minions_denied
                with salt.utils.fopen(pubfn_denied, 'wb+') as fp_:
                    fp_.write(load[b'pub'])
                eload = {'result': False,
                         'id': load[b'id'],
                         'pub': load[b'pub']}
                self.event.fire_event(eload, salt.utils.event.tagify(prefix='auth'))
            ...
            if os.path.isdir(pubfn_pend):
                ...
                log.info(
                    'New public key {0} is a directory'.format(load[b'id'])
                )
                eload = {'result': False,
                         'id': load[b'id'],
                         'pub': load[b'pub']}
            ...
            if key_path is not None:
                ...
                with salt.utils.fopen(key_path, 'wb+') as fp_:
                    fp_.write(load[b'pub'])
                ret = {'enc': 'clear',
                       'load': {'ret': key_result}}
            ...
            if auto_reject:
                ...
                log.info('Pending public key for {0} rejected via '
                         'autoreject_file'.format(load[b'id']))
                ret = {'enc': 'clear',
                       'load': {'ret': False}}
                eload = {'result': False,
                         'act': 'reject',
                ...
                # pending.
                if salt.utils.fopen(pubfn_pend, 'rb').read() != load[b'pub']:
                    log.error(
                        'Authentication attempt from {0} failed, the public '
        ...
        log.info('Authentication accepted from {0}'.format(load[b'id']))
        # only write to disk if you are adding the file, and in open mode,
        # which implies we accept any key from a minion.
        if not os.path.isfile(pubfn) and not self.opts['open_mode']:
            with salt.utils.fopen(pubfn, 'wb+') as fp_:
                fp_.write(load[b'pub'])
        elif self.opts['open_mode']:
            disk_key = ''
            if os.path.isfile(pubfn):
                with salt.utils.fopen(pubfn, 'rb') as fp_:
                    disk_key = fp_.read()
            if load[b'pub'] and load[b'pub'] != disk_key:
        ...
        if self.cache_cli:
            self.cache_cli.put_cache([load[b'id']])
