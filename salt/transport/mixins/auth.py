...
class AESReqServerMixin(object):
    ...
    def _auth(self, load):
        ...
        if self.opts['open_mode']:
            ...
            log.info(
                'Public key rejected for %s. Key is present in rejection '
                'key dir.', load['id']
            )
            eload = {'result': False,
                     'id': load['id'],
            ...
            with salt.utils.files.fopen(pubfn, 'r') as pubfn_handle:
                pub = salt.utils.stringutils.to_unicode(pubfn_handle.read()).strip()
                if pub != salt.utils.stringutils.to_unicode(load['pub']).strip():
                    log.error(
                        'Authentication attempt from %s failed, the public '
                        'keys did not match. This may be an attempt to compromise '
                        'the Salt cluster.',
                        salt.utils.stringutils.to_unicode(load['id'])
                    )
            ...
            if auto_reject:
                ...
                log.info(
                    'New public key for %s rejected via autoreject_file',
                    load['id']
                )
                key_act = 'reject'
                key_result = False
                ...
                key_path = pubfn_pend
                log.info(
                    'New public key for %s placed in pending',
                    load['id']
                )
                key_act = 'pend'
                key_result = True
            ...
            if auto_reject:
                ...
                with salt.utils.files.fopen(pubfn_pend, 'r') as pubfn_handle:
                    pub = salt.utils.stringutils.to_unicode(pubfn_handle.read())
                    if pub != salt.utils.stringutils.to_unicode(load['pub']):
                        log.error(
                            'Authentication attempt from %s failed, the public '
        ...
        if not os.path.isfile(pubfn) and not self.opts['open_mode']:
            ...
            if os.path.isfile(pubfn):
                with salt.utils.files.fopen(pubfn, 'r') as fp_:
                    disk_key = salt.utils.stringutils.to_unicode(fp_.read())
            if load['pub'] and salt.utils.stringutils.to_unicode(load['pub']) != disk_key:
