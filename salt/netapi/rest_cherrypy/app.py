...
class Events(object):
    ...
    def GET(self, token=None, salt_token=None):
        ...
        **Example response:**
        ...
        A full example shows establishing a long held connection,
        running a job by submitting a request to salt api
        and receiving events generated by a job run.
        Parts of the example below are attributed to
        Mozilla's `Using XMLHttpRequest
        <https://developer.mozilla.org/en-US/docs/Web/API/XMLHttpRequest/Using_XMLHttpRequest>`_.

        .. code-block:: javascript
            // Subscribe to the event stream and stay subscribed
            var source = new EventSource('/events');
            source.onopen = function() { console.info('Listening ...') };
            source.onerror = function(err) { console.error(err) };
            // Receive job results
            source.onmessage = function(message) {
              var saltEvent = JSON.parse(message.data);
              console.log('Got return data from salt!')
            };
            function reqListener () {
                console.log('Request sent to server!');
            }
            // Submit a post request to run a salt job
        ...
        .. code-block:: javascript
            ...
            Got return data from salt!

        Due to the asynchronous nature of the above example
        we can also expect the output below
