...
class CleanExpiredTokens:
    ...
    def __call__(self):
        for tok in self.loadauth.list_tokens():
            token_data = self.loadauth.get_tok(tok)
            if "expire" not in token_data or token_data.get("expire", 0) < time.time():
                self.loadauth.rm_token(tok)
...
class CleanOldJobs:
    ...
    def __init__(self, opts):
        self.mminion = salt.minion.MasterMinion(opts, states=False, rend=False,)
        self.fstr = "{}.clean_old_jobs".format(opts["master_job_cache"])

    def __call__(self):
        if self.fstr in self.mminion.returners:
            self.mminion.returners[self.fstr]()
