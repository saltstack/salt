...
class MasterKeys(dict):
    ...
    def _setup_keys(self):
        ...
        if self.cache.contains("master_keys", f"{self.master_id}.pem"):
            self.key = self.find_or_create_keys(
                name=self.master_id, passphrase=key_pass
            )
        else:
            self.key = self.find_or_create_keys(name="master", passphrase=key_pass)
            ...
            if not self.cache.contains("master_keys", f"{self.master_id}.pem"):
                ...
                # symlinks don't seem to work as expected on windows unfortunately
