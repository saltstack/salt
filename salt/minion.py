...
class Minion(MinionBase):
    ...
    def _post_master_init(self, master):
        ...
        if self.connected:
            ...
            event.fire_event({'master': master}, master_event(type='connected'))
    ...
    def pillar_refresh(self, force_refresh=False, notify=False):
        ...
        if self.connected:
            ...
            try:
                ...
                evt = salt.utils.event.get_event('minion', opts=self.opts, listen=False, sync=False)
                if notify:
                    evt.fire_event({'complete': True}, tag=salt.defaults.events.MINION_PILLAR_COMPLETE)
                evt.fire_event(self.opts['pillar'], 'new_pillar')
            except SaltClientError:
    ...
    def _handle_tag_minion_return(self, tag, data):
        if self.connected:
        ...
        if self.connected:
            ...
            if self.opts['master_type'] != 'failover':
                ...
                if self.connected:
                    ...
                    self.schedule.functions = self.functions
                    self.pub_channel.on_recv(self.handle_payload)
...
class WorkerMinion(Minion):
    ...
    def handle_event(self, package):
        ...
        '''
        tag, data = salt.utils.event.SaltEvent.unpack(package)
        log.debug('WORKER is handling event tag %s: %s', tag, data)
