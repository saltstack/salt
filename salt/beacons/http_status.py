...
attr_func_map = {"status": lambda x: x.status_code, "content": lambda x: x.json()}
...
def validate(config):
    ...
    for site, settings in sites.items():
        if required_site_attributes.isdisjoint(set(settings.keys())):
            valid = False
            messages.append(
                "[-] Sites for {} beacon requires {}".format(
                    __virtualname__, required_site_attributes
                )
            )
        log.debug("[+] site: %s", site)
        log.debug("[+] settings: %s", settings)

        for optional_attrs in itertools.chain(
            settings.get(attr, []) for attr in optional_site_attributes
        ):
    ...
    for site, site_config in _config.get("sites", {}).items():
        ...
        try:
            ...
            if r.raise_for_status:
                log.info(
                    "[-] Response from status endpoint was invalid: " "%s",
                    r.status_code,
                )
        ...
        for attr, checks in site_config.items():
            for check in checks:
                ...
                if attr_path:
                    received_value = salt.utils.data.traverse_dict_and_list(
                        attr_func_map[attr](r), attr_path
                    )
                else:
                    received_value = attr_func_map[attr](r)
                if received_value is None:
                    log.info(
                        "[-] No data found at location {} for url {}".format(
                            attr_path, url
                        )
                    )
                    continue
