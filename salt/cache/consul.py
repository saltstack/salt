...
def __virtual__():
    ...
    '''
    if not HAS_CONSUL:
        return (False, u"Please install python-consul package to use consul data cache driver")

    consul_kwargs = {
            u'host': __opts__.get(u'consul.host', u'127.0.0.1'),
            u'port': __opts__.get(u'consul.port', 8500),
            u'token': __opts__.get(u'consul.token', None),
            u'scheme': __opts__.get(u'consul.scheme', u'http'),
            u'consistency': __opts__.get(u'consul.consistency', u'default'),
            u'dc': __opts__.get(u'consul.dc', None),
            u'verify': __opts__.get(u'consul.verify', True),
            }
    ...
    try:
        ...
        return (False, u"Failed to invoke consul.Consul, please make sure you have python-consul >= 0.2.0 installed")
...
def store(bank, key, data):
    ...
    Store a key value.
    '''
    c_key = u'{0}/{1}'.format(bank, key)
    try:
        c_data = __context__[u'serial'].dumps(data)
        api.kv.put(c_key, c_data)
    except Exception as exc:
        raise SaltCacheError(
            u'There was an error writing the key, {0}: {1}'.format(
                c_key, exc
            )
        )
...
def fetch(bank, key):
    ...
    try:
        ...
        if value is None:
            return {}
        return __context__[u'serial'].loads(value[u'Value'])
    except Exception as exc:
        raise SaltCacheError(
...
def list_(bank):
    ...
    '''
    try:
        _, keys = api.kv.get(bank + u'/', keys=True, separator=u'/')
    except Exception as exc:
        raise SaltCacheError(
    ...
    if keys is None:
        ...
        out = set()
        for key in keys:
            out.add(key[len(bank) + 1:].rstrip(u'/'))
        keys = list(out)
    return keys
