...
def _conn(commit=False):
    ...
    for key, value in defaults.items():
        conn_kwargs[key] = __opts__.get(
            "queue.{0}.{1}".format(__virtualname__, key), value
        )
    try:
...
def _create_table(cur, queue):
    cmd = "CREATE TABLE {0}(id SERIAL PRIMARY KEY, " "data jsonb NOT NULL)".format(
        queue
    )
    log.debug("SQL Query: %s", cmd)
    cur.execute(cmd)
...
def _list_items(queue, mode=None):
    ...
    with _conn() as cur:
        cmd = "SELECT data FROM {0}{1}".format(queue, order_by)
        log.debug("SQL Query: %s", cmd)
        cur.execute(cmd)
...
def insert(queue, items):
    ...
    with _conn(commit=True) as cur:
        if isinstance(items, dict):
            ...
            cmd = str("""INSERT INTO {0}(data) VALUES('{1}')""").format(
                queue, items
        ...
        if isinstance(items, list):
            ...
            cmd = str("INSERT INTO {0}(data) VALUES (%s)").format(
                queue
            )  # future lint: disable=blacklisted-function
            log.debug("SQL Query: %s", cmd)
            try:
...
def pop(queue, quantity=1, is_runner=False, mode=None):
    ...
    with _conn(commit=True) as cur:
        ...
        if len(result) > 0:
            ...
            idlist = "','".join(ids)
            del_cmd = """DELETE FROM {0} WHERE id IN ('{1}');""".format(queue, idlist)
