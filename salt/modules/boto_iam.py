...
def __init__(opts):
    if HAS_BOTO:
        salt.utils.botomod.assign_funcs(__name__, "iam", pack=__salt__)
...
def delete_policy(policy_name, region=None, key=None, keyid=None, profile=None):
    ...
    try:
        ...
        aws = salt.utils.botomod.get_error(e)
        log.debug(aws)
...
def list_policies(region=None, key=None, keyid=None, profile=None):
    ...
    try:
        policies = []
        for ret in salt.utils.botomod.paged_call(conn.list_policies):
            policies.append(
...
def create_policy_version(
    ...
    try:
        ...
        return {"created": False, "error": salt.utils.botomod.get_error(e)}
...
def list_entities_for_policy(
    ...
    while retries:
        try:
            ...
            for ret in salt.utils.botomod.paged_call(
...
def list_saml_providers(region=None, key=None, keyid=None, profile=None):
    ...
    try:
        ...
        log.debug(salt.utils.botomod.get_error(e))
