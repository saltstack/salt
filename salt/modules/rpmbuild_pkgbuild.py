...
def _sign_file(runas, define_gpg_name, phrase, abs_file):
    '''
...
def _sign_files_with_gpg_agent(runas, local_keyid, abs_file, repodir, env):
    '''
...
##                                retrc)
##                        )
                    _sign_files_with_gpg_agent(runas, local_keyid, abs_file, repodir, env):
                else:
##                     cmd = 'rpm {0} --addsign {1}'.format(define_gpg_name, abs_file)
##                     preexec_fn = functools.partial(salt.utils.user.chugid_and_umask, runas, None)
##                     try:
...
##                     finally:
##                         proc.close(terminate=True, kill=True)
                    _sign_file(runas, define_gpg_name, phrase, abs_file)
