...
def verify_password(name, password):
    ...
    Returns:
        bool: ``True`` if valid password, otherwise ``False``
    ...
    .. code-block:: bash
        ...
        salt * user.verify_password spongebob P@ssW0rd
    '''
    # Get current status
    pre_info = info(name=name)
    try:
        ...
        if not pre_info['account_locked']:
            if info(name=name)['account_locked']:
                update(name, unlock_account=True)
        # This is the error code you get when the logon attempt fails
        if exc.winerror == 1326:
