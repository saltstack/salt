...
def _run_psql(cmd, runas=None, password=None, host=None, port=None, user=None):
    ...
    kwargs = {
        ...
        "timeout": __salt__["config.get"](
            "postgres.timeout", default=_DEFAULT_COMMAND_TIMEOUT_SECS, delimeter="."
        ),
    }
    if runas is None:
        if not host:
            host = __salt__["config.get"]("postgres.host", delimeter=".")
    ...
    if password is None:
        password = __salt__["config.get"]("postgres.pass", delimeter=".")
...
def _connection_defaults(user=None, host=None, port=None, maintenance_db=None):
    ...
    """
    if not user:
        user = __salt__["config.get"]("postgres.user", delimeter=".")
    if not host:
        host = __salt__["config.get"]("postgres.host", delimeter=".")
    if not port:
        port = __salt__["config.get"]("postgres.port", delimeter=".")
    if not maintenance_db:
        maintenance_db = __salt__["config.get"](
            "postgres.maintenance_db", delimeter="."
        )
