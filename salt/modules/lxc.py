...
def version():
    ...
    if not __context__.get(k, None):
        ...
        if cversion['retcode'] == exitcodes.EX_OK:
...
def _after_ignition_network_profile(cmd,
                                    ...
                                    nic_opts):
    _clear_context()
    if ret['retcode'] == exitcodes.EX_OK and exists(name, path=path):
        if network_profile:
...
def info(name, path=None):
    ...
    try:
        ...
        if ret['state'] == 'running':
            ...
            if ip_cmd['retcode'] == exitcodes.EX_OK:
...
def running_systemd(name, cache=True, path=None):
    ...
    if ret is None or not cache:
        ...
        if result['retcode'] in (exitcodes.EX_OK, 2):
            __context__[k] = ret = result['retcode'] == exitcodes.EX_OK
    return ret
        ...
        if needs_install or force_install or unconditional_install:
            if install:
                ...
                ret = retcode(name, cmd, output_loglevel='info',
                              path=path, use_vt=True) == exitcodes.EX_OK
