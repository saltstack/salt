...
def __virtual__():
    ...
    # Return False if Windows versions is < Windows 8 or Windows 2012
    osversion = float('.'.join(__grains__['osversion'].split('.')[:2]))
    if osversion < 6.2:
        return (False, 'Can only win_servermanager_ng modules on Windows 8 or Windows 2012 and later')
...
def install(feature, recurse=False, management=False, restart=False):
    '''
    Install a feature
    ...
    :param bool management: Install applicable management tools for the role
    ...
    .. code-block:: bash
        ...
        salt '*' win_servermanager.install DNS management=True restart=True
    '''
    sub = ''
    if recurse:
    ...
    mgt = ''
    if management:
        mgt = '-IncludeManagementTools'

    rst = ''
    if restart:
        restart = '-Restart'

    out = _pshell('Install-WindowsFeature -Name {0} {1} {2} '
                  '-erroraction silentlycontinue '
                  '-warningaction silentlycontinue {3}'
                  '| format-list'.format(_cmd_quote(feature), sub, mgt, rst))
