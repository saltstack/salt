...
def sync_liquid(saltenv=None, refresh=False, extmod_whitelist=None, extmod_blacklist=None):
    '''
    .. versionadded:: Oxygen

    Sync liquid modules from ``salt://_liquid`` to the minion
    ...
    refresh: ``True``
        If ``True``, refresh the available execution modules on the minion.
        This refresh will be performed even if no new liquid modules are synced.
        Set to ``False`` to prevent this refresh.
    ...
    .. code-block:: bash
        ...
        salt '*' saltutil.sync_liquid
        salt '*' saltutil.sync_liquid saltenv=base,dev
    '''
    ret = _sync('liquid', saltenv, extmod_whitelist, extmod_blacklist)
    if refresh:
        refresh_modules()
    return ret
...
def sync_all(saltenv=None, refresh=True, extmod_whitelist=None, extmod_blacklist=None):
    ...
    ret['liquid'] = sync_liquid(saltenv, False, extmod_whitelist, extmod_blacklist)
    if __opts__['file_client'] == 'local':
