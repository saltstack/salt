...
:configuration: This module accepts explicit rds credentials but can also utilize
...
def exists(name, region=None, key=None, keyid=None, profile=None):
    ...
    if not conn:
        return False
    try:
        rds = conn.get_all_dbinstances(instance_id=name)
        if rds:
            return True
        else:
            msg = 'The rds instance does not exist in region {0}'.format(region)
            log.debug(msg)
            return False
...
def get_rds_config(name, region=None, key=None, keyid=None, profile=None):
    '''
    Check to see if an RDS exists.
...
def create(id, allocated_storage, instance_class, master_username, master_password,
           port=3306, engine='MySQL5.6', db_name=None, param_group=None, 
           security_groups=None, availability_zone=None, preferred_maintenance_window=None,
           backup_retention_period=None, preferred_backup_window=None, multi_az=False,
           engine_version=None, auto_minor_version_upgrade=True, character_set_name=None,
           db_subnet_group_name=None, license_model=None, option_group_name=None, iops=None,
    ...
    CLI example to create an RDS::
        ...
        salt myminion boto_rds.create myrds id='db-master1' allocated_storage=10 \
            instance_class='db.m1.small' master_username='root' master_password='pass'
    '''
    conn = _get_conn(region, key, keyid, profile)
    if not conn:
        return False
