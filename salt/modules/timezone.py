...
def _get_windows_timezone():
    '''
    Get windows timezone but in POSIX-like format
    '''
...
def _set_windows_zone(zone):
    '''
    Configure the time zone
    '''
    
    return __salt__['cmd.retcode']( 'tzutil /s "{0}"'.format(LINTOWIN[zone]) ) == 0
...
def set_zone(timezone):
    ...
    if salt.utils.is_windows():
        return _set_windows_zone(timezone)
