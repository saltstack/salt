...
def _complete_transaction(
    cleanup_only, recursive, max_attempts, run_count, cmd_ret_list
):
    """
    .. versionadded:: Fluorine
    ...
    if (
        cmd_ret_list[-1]["retcode"] == salt.defaults.exitcodes.EX_OK
        and recursive
        and "No unfinished transactions left." not in cmd_ret_list[-1]["stdout"]
    ):
        if run_count >= max_attempts:
            ...
            return _complete_transaction(
                cleanup_only, recursive, max_attempts, run_count + 1, cmd_ret_list
            )
