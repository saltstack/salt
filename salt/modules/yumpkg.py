...
def update(name=None,
            ...
            **kwargs):
    '''
    Run a full system update (a ``yum update`` or ``dnf upgrade --obsoletes=False``), or
    ...
    name
        The name of the package to be updated. Note that this parameter is
        ignored if "pkgs" is passed.

        32-bit packages can be updated on 64-bit systems by appending the
        architecture designation (``.i686``, ``.i586``, etc.) to the end of the
        package name.

        Warning: if you forget 'name=' and run pkg.update openssl, ALL packages
        are updated. This will be addressed in next releases.
    ...
    update_minimal : False
        Use update-minimal instead of update (e.g., ``yum update-minimal``)
...
.. note::
    ...
    if result['retcode'] != 0:
        raise CommandExecutionError(
            'Problem encountered updating packages',
            info={'changes': ret, 'result': result}
        )
...
def update_minimal(name=None,
            ...
            **kwargs):
    '''
    Run a full system update-minimal (a ``yum update-minimal`` or ``dnf upgrade-minimal --obsoletes=False``), or
