    ...
    for brick in bricks:
        try:
            ...
            if not path.startswith('/'):
                raise SaltInvocationError('Brick paths must start with / in {0}'.format(brick))
        except ValueError:
            raise SaltInvocationError('Brick syntax is <peer>:<path> got {0}'.format(brick))
...
def status(name):
    ...
    for node in root.iter('node'):
        ...
        if not hostname in ('NFS Server','Self-heal Daemon'):
...
def delete(target, stop=True):
    ...
    if not stop and running:
        ...
        raise SaltInvocationError('Volume {0} must be stopped before deletion'.format(target))
