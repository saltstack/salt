...
def check_dns_name_availability(name, region, **kwargs):
    ...
    '''
    netconn = _get_client('network', **kwargs)
    try:
        ...
        _log_cloud_error('network', str(exc), **kwargs)
...
def security_rules_list(security_group, resource_group, **kwargs):
    ...
    try:
        ...
        result = _paged_object_to_list(secrules)
    except CloudError as exc:
        _log_cloud_error('network', str(exc), **kwargs)
        result = {'error': str(exc)}
...
def security_rule_create_or_update(name, access, direction, priority, protocol, security_group, resource_group,
    ...
    try:
        rulemodel = _create_object_model(
            'network',
            'SecurityRule',
            name=name,
            access=access,
...
def network_security_group_create_or_update(name, resource_group, **kwargs):  # pylint: disable=invalid-name
    ...
    try:
        secgroupmodel = _create_object_model('network', 'NetworkSecurityGroup', **kwargs)
    except TypeError as exc:
        result = {'error': 'The object model could not be built. ({0})'.format(str(exc))}
        return result
...
def network_security_groups_list(resource_group, **kwargs):
    ...
    netconn = _get_client('network', **kwargs)
    try:
        secgroups = _paged_object_to_list(
...
def subnet_create_or_update(name, address_prefix, virtual_network, resource_group, **kwargs):
    ...
    try:
        snetmodel = _create_object_model(
            'network',
            'Subnet',
...
def virtual_networks_list_all(**kwargs):
    ...
    netconn = _get_client('network', **kwargs)
    try:
        vnets = _paged_object_to_list(netconn.virtual_networks.list_all())
...
def virtual_network_create_or_update(name,
    ...
    try:
        vnetmodel = _create_object_model(
            'network',
            'VirtualNetwork',
...
def load_balancers_list_all(**kwargs):
    ...
    netconn = _get_client('network', **kwargs)
    try:
        load_balancers = _paged_object_to_list(netconn.load_balancers.list_all())
...
def load_balancers_list(resource_group, **kwargs):
    ...
    netconn = _get_client('network', **kwargs)
    try:
        load_balancers = _paged_object_to_list(
...
def load_balancer_create_or_update(name, resource_group, **kwargs):
    ...
    try:
        lbmodel = _create_object_model('network', 'LoadBalancer', **kwargs)
    except TypeError as exc:
        result = {'error': 'The object model could not be built. ({0})'.format(str(exc))}
        return result
...
def usages_list(location, **kwargs):
    ...
    netconn = _get_client('network', **kwargs)
    try:
        result = _paged_object_to_list(netconn.usages.list(location))
    except CloudError as exc:
        _log_cloud_error('network', str(exc), **kwargs)
        result = {'error': str(exc)}
...
def network_interfaces_list_all(**kwargs):
    ...
    netconn = _get_client('network', **kwargs)
    try:
        nics = _paged_object_to_list(netconn.network_interfaces.list_all())
...
def public_ip_address_create_or_update(name, resource_group, **kwargs):
    ...
    try:
        pub_ip_model = _create_object_model('network', 'PublicIPAddress', **kwargs)
    except TypeError as exc:
        result = {'error': 'The object model could not be built. ({0})'.format(str(exc))}
        return result
...
def public_ip_addresses_list_all(**kwargs):
    ...
    netconn = _get_client('network', **kwargs)
    try:
        pub_ips = _paged_object_to_list(netconn.public_ip_addresses.list_all())
...
def route_filter_rule_create_or_update(name, access, communities, route_filter, resource_group, **kwargs):
    ...
    try:
        rule_model = _create_object_model(
            'network',
            'RouteFilterRule',
            access=access,
    ...
    try:
        ...
        _log_cloud_error('network', message, **kwargs)
...
def route_filter_create_or_update(name, resource_group, **kwargs):
    ...
    try:
        rt_filter_model = _create_object_model('network', 'RouteFilter', **kwargs)
    except TypeError as exc:
        result = {'error': 'The object model could not be built. ({0})'.format(str(exc))}
        return result
...
def route_filters_list_all(**kwargs):
    ...
    netconn = _get_client('network', **kwargs)
    try:
        filters = _paged_object_to_list(netconn.route_filters.list())
...
def route_table_create_or_update(name, resource_group, **kwargs):
    ...
    try:
        rt_tbl_model = _create_object_model('network', 'RouteTable', **kwargs)
    except TypeError as exc:
        result = {'error': 'The object model could not be built. ({0})'.format(str(exc))}
        return result
...
def route_tables_list_all(**kwargs):
    ...
    netconn = _get_client('network', **kwargs)
    try:
        tables = _paged_object_to_list(netconn.route_tables.list_all())
