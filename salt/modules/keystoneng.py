...
def __virtual__():
    ...
    return (
        False,
        "The keystoneng execution module failed to load: openstacksdk or shade "
...
def _clean_kwargs(keep_name=False, **kwargs):
    """
    Sanatize the arguments for use with openstack
    """
...
def get_openstack_connection(auth=None):
    ...
    if "openstack_connection" in __context__:
        if __context__["openstack_connection"].auth == auth:
            return __context__["openstack_connection"]
    __context__["openstack_connection"] = openstack.connect(**auth)
    return __context__["openstack_connection"]
...
def get_operator_cloud(auth=None):
    ...
    __context__["openstack_opcloud"] = openstack.operator_cloud(**auth)
    return __context__["openstack_opcloud"]
...
def get_openstack_cloud(auth=None):
    ...
    __context__["openstack_oscloud"] = openstack.openstack_cloud(**auth)
    return __context__["openstack_oscloud"]
...
def group_create(auth=None, **kwargs):
    ...
    """
    if HAS_OPENSTACK:
        cloud = get_openstack_connection(auth)
    else:
