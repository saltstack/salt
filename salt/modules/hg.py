...
def revision(cwd, rev='tip', short=False, user=None):
    ...
    result = __salt__['cmd.run_all'](cmd, python_shell=False, cwd=cwd, runas=user)
...
def describe(cwd, rev='tip', user=None):
    ...
    desc = __salt__['cmd.run_stdout'](cmd, python_shell=False, cwd=cwd, runas=user)
...
def archive(cwd, output, rev='tip', fmt=None, prefix=None, user=None):
    ...
    return __salt__['cmd.run'](cmd, python_shell=False, cwd=cwd, runas=user)
...
def pull(cwd, opts=None, user=None):
    ...
    if not opts:
        opts = ''
    return __salt__['cmd.run']('hg pull {0}'.format(opts), python_shell=False, cwd=cwd, runas=user)
...
def clone(cwd, repository, opts=None, user=None):
    ...
    return __salt__['cmd.run'](cmd, python_shell=False, runas=user)
