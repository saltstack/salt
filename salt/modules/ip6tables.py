...
def __virtual__():
    '''
    Only load the module if ip6tables is installed
    '''
    if salt.utils.which('ip6tables'):
        return 'ip6tables'
    return False
...
def version():
    '''
    Return version from ip6tables --version

    CLI Example:

    .. code-block:: bash

        salt '*' ip6tables.version
    '''
...
def build_rule(table=None, chain=None, command=None, position='', full=None,
               **kwargs):
    '''
    Build a well-formatted ip6tables rule based on kwargs. Long options must be
    ...
    (`--insert`). This will return the ip6tables command, exactly as it would
    ...
    .. code-block:: bash
        ...
        salt '*' ip6tables.build_rule match=state \\
            connstate=RELATED,ESTABLISHED jump=ACCEPT
        salt '*' ip6tables.build_rule filter INPUT command=I position=3 \\
    ...
    if full is True:
        ...
        return 'ip6tables -t {0} {1}{2} {3} {4} {5}'.format(table,
...
def get_saved_rules(conf_file=None):
    ...
    .. code-block:: bash
        ...
        salt '*' ip6tables.get_saved_rules
    '''
...
def get_saved_policy(table='filter', chain=None, conf_file=None):
    ...
    .. code-block:: bash
        ...
        salt '*' ip6tables.get_saved_policy filter INPUT
        salt '*' ip6tables.get_saved_policy filter INPUT \\
            conf_file=/etc/ip6tables.saved
    '''
    if not chain:
        return 'Error: Chain needs to be specified'
...
def get_policy(table='filter', chain=None):
    ...
    .. code-block:: bash
        ...
        salt '*' ip6tables.get_policy filter INPUT
    '''
    if not chain:
        return 'Error: Chain needs to be specified'
...
def set_policy(table='filter', chain=None, policy=None):
    ...
    .. code-block:: bash
        ...
        salt '*' ip6tables.set_policy filter INPUT ACCEPT
    '''
    if not chain:
        return 'Error: Chain needs to be specified'
    if not policy:
    ...
    cmd = 'ip6tables -t {0} -P {1} {2}'.format(table, chain, policy)
...
def save(filename=None):
    ...
    .. code-block:: bash
        ...
        salt '*' ip6tables.save /etc/sysconfig/ip6tables
    '''
    ...
    cmd = 'ip6tables-save > {0}'.format(filename)
    out = __salt__['cmd.run'](cmd)
    return out
...
def check(table='filter', chain=None, rule=None):
    ...
    This function accepts a rule in a standard ip6tables command format,
    ...
    .. code-block:: bash
        ...
        salt '*' ip6tables.check filter INPUT \\
    ...
    if __grains__['os_family'] == 'RedHat':
        ...
        cmd = 'ip6tables -t {0} -C {1} {2}'.format(table, chain, rule)
        out = __salt__['cmd.run'](cmd)
...
def check_chain(table='filter', chain=None):
    ...
    .. code-block:: bash
        ...
        salt '*' ip6tables.check_chain filter INPUT
    '''

    if not chain:
        return 'Error: Chain needs to be specified'

    cmd = 'ip6tables-save -t {0}'.format(table)
...
def new_chain(table='filter', chain=None):
    ...
    .. code-block:: bash
        ...
        salt '*' ip6tables.new_chain filter CUSTOM_CHAIN
    '''

    if not chain:
        return 'Error: Chain needs to be specified'

    cmd = 'ip6tables -t {0} -N {1}'.format(table, chain)
    out = __salt__['cmd.run'](cmd)
...
def delete_chain(table='filter', chain=None):
    ...
    cmd = 'ip6tables -t {0} -X {1}'.format(table, chain)
    out = __salt__['cmd.run'](cmd)
...
def append(table='filter', chain=None, rule=None):
    ...
    .. code-block:: bash
        ...
        salt '*' ip6tables.append filter INPUT \\
            rule='-m state --state RELATED,ESTABLISHED -j ACCEPT'
    '''
    if not chain:
        return 'Error: Chain needs to be specified'
    ...
    cmd = 'ip6tables -t {0} -A {1} {2}'.format(table, chain, rule)
    out = __salt__['cmd.run'](cmd)
    if len(out) == 0:
        return True
    else:
...
def insert(table='filter', chain=None, position=None, rule=None):
    ...
    .. code-block:: bash
        ...
        salt '*' ip6tables.insert filter INPUT position=3 \\
            rule='-m state --state RELATED,ESTABLISHED -j ACCEPT'
    '''
    if not chain:
        return 'Error: Chain needs to be specified'
    ...
    cmd = 'ip6tables -t {0} -I {1} {2} {3}'.format(table, chain, position, rule)
    out = __salt__['cmd.run'](cmd)
    return out
...
def delete(table, chain=None, position=None, rule=None):
    ...
    .. code-block:: bash
        ...
        salt '*' ip6tables.delete filter INPUT position=3
        salt '*' ip6tables.delete filter INPUT \\
            rule='-m state --state RELATED,ESTABLISHED -j ACCEPT'
    '''
    ...
    cmd = 'ip6tables -t {0} -D {1} {2}'.format(table, chain, rule)
    out = __salt__['cmd.run'](cmd)
    return out
...
def flush(table='filter', chain=''):
    ...
    .. code-block:: bash
        ...
        salt '*' ip6tables.flush filter INPUT
    '''

    if chain:
        cmd = 'ip6tables -t {0} -F {1}'.format(table, chain)
    else:
        cmd = 'ip6tables -t {0} -F'.format(table)
    out = __salt__['cmd.run'](cmd)
    return out
...
def _parser():
    '''
    This function contains _all_ the options I could find in man 8 ip6tables,
