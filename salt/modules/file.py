...
def check_file_meta(
    ...
    if contents is not None:
        ...
        text = not isinstance(contents, six.binary_type)
        tmp = salt.utils.files.mkstemp(
            prefix=salt.utils.files.TEMPFILE_PREFIX, text=text
        )
        with salt.utils.files.fopen(tmp, "wb") as tmp_:
            ...
            if text:
                if salt.utils.platform.is_windows():
                    ...
                    contents = salt.utils.stringutils.to_str(contents)
            with salt.utils.files.fopen(tmp, "w" + ("" if text else "b")) as tmp_:
                tmp_.write(contents)
