...
## would be nice to get support for this in the loader.
import os.path
import sys
org_path = sys.path
...
def __virtual__():
    return (REQUIREMENTS_ERROR == None, REQUIREMENTS_ERROR)
...
class IpmiSession:
    ...
    def __init__(self, **kwargs):
        #cache_key = api_host + api_user + str(api_port)
        config = _get_config(**kwargs)
        self.o = session.Session(bmc=config['api_host'], userid=config['api_user'],
                                 password=config['api_pass'], port=config['api_port'],
                                 kg=config['api_key'], onlogon=self._onlogon)
...
def raw_command(netfn, command, bridge_request=None, data=(), retry=True, delay_xmit=None, **kwargs_conn):
    '''
    ...
    Example: ipmi.raw_command netfn=0x06 command=0x46 data=[0x02]
    this will return the name bytes of the user with id 2
    ...
    '''
    with IpmiSession(**kwargs_conn) as s:
...
def fast_connect_test(**kwargs_conn):
    '''
    returns True if connection success
    '''
    try:
        if 'api_login_timeout' not in kwargs_conn:
            kwargs_conn['api_login_timeout'] = 0
        with IpmiSession(**kwargs_conn) as s:
...
def set_channel_access(channel=1, access_update_mode='non_volatile', 
                       ...
                       privilege_level='administrator', **kwargs_conn):
    '''
...
'''
    with IpmiCommand(**kwargs_conn) as s:
...
def get_channel_access(channel=14, read_mode='non_volatile',  **kwargs_conn):
    '''
    :param kwargs_conn: api_host='127.0.0.1' api_user='admin' api_pass='example' api_port=623
...
def get_channel_info(channel=14):
    '''
    Get channel info
...
def set_user_access(uid, channel=14, callback=True, link_auth=True, ipmi_msg=True, privilege_level='administrator', **kwargs_conn):
    '''
    Set user access
...
def get_user_access(uid, channel=14, **kwargs_conn):
    '''
    Get user access
...
def set_user_name(uid, name, **kwargs_conn):
    '''
    Set user name
...
def get_user_name(uid, return_none_on_error=True, **kwargs_conn):
    '''
    Get user name
...
def set_user_password(uid, mode='set_password', password=None, **kwargs_conn):
    '''
...
def get_sensor_data(**kwargs_conn):
    '''
    Get sensor readings
...
def set_power(name='power_on', wait=True, **kwargs_conn):
    '''
    ...
    '''
    if name == True or name == 'power_on':
        name = 'on'
    if name == False or name == 'power_off':
        name = 'off'
    with IpmiCommand(**kwargs_conn) as s:
        return s.set_power(name, wait=wait)


def set_bootdev(bootdev='default', persist=False, uefiboot=False, **kwargs_conn):
    '''
...
def set_identify(on=True, duration=None, **kwargs_conn):
    '''
...
def get_channel_max_user_count(channel=14, **kwargs_conn):
    '''
    ...
    :return: int -- often 16
    '''
    access = get_user_access(channel=channel, uid=1, **kwargs_conn)
    return access['channel_info']['max_user_count']


def get_user(uid, channel=14, **kwargs_conn):
    '''
    ...
    '''
    name = get_user_name(uid, **kwargs_conn)
    access = get_user_access(uid, channel, **kwargs_conn)
    data = { 'name': name, 'uid': uid, 'channel': channel, 'access': access['access']  }
    return data


def get_name_uids(name, channel=1, **kwargs_conn):
    '''
    ...
    '''
    with IpmiCommand(**kwargs_conn) as c:
        return c.get_name_uids(name, channel)


def get_users(channel=14, **kwargs_conn):
    '''
...
def user_delete(uid, channel=14, **kwargs_conn):
    '''
    Delete user (helper)
