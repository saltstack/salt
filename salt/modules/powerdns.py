...
def __virtual__():
    ...
    if HAS_REQUESTS is False:
        return (
            False,
            "The requests python package is required for the powerdns execution module",
        )
...
def _get_zone(name, headers, server):
    ...
    url = requests.compat.urljoin(
        requests.compat.urljoin(server, POWERDNS_URL_ZONES_TEMPLATE + "/"), name
    )
...
def _check_records_equality(old_record, new_record):
    if old_record["name"] == new_record["name"] and (
        ...
        or (
            (old_record["type"] == DNS_RECORD_A or new_record["type"] == DNS_RECORD_A)
            and (
                old_record["type"] == DNS_RECORD_CNAME
                or new_record["type"] == DNS_RECORD_CNAME
            )
        )
    ):
        return True
...
def _check_are_rrsets_changed(old_rrsets, new_rrsets):
    ...
    for new_rrset in new_rrsets:
        ...
        for old_rrset in old_rrsets:
            if (
                new_rrset["name"] == old_rrset["name"]
                and new_rrset["type"] == old_rrset["type"]
            ):
                is_new = False
                if _check_are_records_changed(
                    old_rrset["records"], new_rrset["records"]
                ):
                    changes.append(
                        {
                            new_rrset["name"]: {
                                ...
                                "old": {
                                    "ttl": old_rrset["ttl"],
                                    "content": _extract_content_from_records(
                                        old_rrset["records"]
                                    ),
                                },
                                "new": {
        ...
        for new_rrset in new_rrsets:
            ...
            for idx, current in enumerate(result):
                ...
                if _check_records_equality(current, new_rrset):
                    ...
                    if (
                        current["type"] == DNS_RECORD_CNAME
                        or new_rrset["type"] == DNS_RECORD_CNAME
                    ) and (
                        current["type"] == DNS_RECORD_A
                        or new_rrset["type"] == DNS_RECORD_A
                    ):
                        is_conflicting = True
    ...
    if rrsets:
        for rrset in rrsets:
            ...
            if not rrset.get("type", None):
                raise CommandExecutionError(
                    "Resource Record Set {} doesn't have a type".format(rrset["name"])
                )
            if (
                not rrset.get("content", None)
                and not rrset.get("records", None)
                and rrset.get("changetype", "") != DELETE_CHANGETYPE
            ):
                raise CommandExecutionError(
                    "Resource Record Set {} doesn't have a records list".format(
                        rrset["name"]
                    )
                )
...
def manage_zone(
    name, key, server, rrsets=None, nameservers=None, drop_existing=False, dnssec=False
):
    """
    ...
    headers = {POWERDNS_AUTH_HEADER: key}
    ...
    if zone is None:
        ...
        log.info("Create new zone {}".format(name))
        log.info("Starting request to {} for zone {}".format(url, name))
        log.info(request)
    ...
    # Update zone
        ...
        if zone_changes:
            ...
            log.info("Update zone {}".format(name))
            log.info("Starting request to {} for zone {}".format(update_url, name))
            log.info(request)
        ...
        prepared_rrsets = _prepare_rrsets(
            name, drop_existing, nameservers, rrsets, zone["rrsets"]
        )
        ...
        if rrsets_changes or rrsets_deletions:
            ...
            log.info("Update rrsets {}".format(name))
            log.info("Starting request to {} for zone {}".format(update_url, name))
            log.info(request)
