...
def set_reboot_required_witnessed():
    ...
    the event gets cleared upon reboot. Note that the function returns False if
    the condition has been set already.
    ...
    if __grains__.get("os_family") == "NILinuxRT":
        reboot_flag_file = "/var/volatile/tmp/salt/reboot_required"

    errcode = -1
    dir_path = os.path.dirname(reboot_flag_file)
