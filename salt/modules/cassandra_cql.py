...
def create_role(
    ...
    query = """create role if not exists {0} with password '{1}' and {2} and {3} ;""".format(
        username, password, superuser_cql, login_cql
    )
    log.debug(
        "Attempting to create a new role with username=%s superuser=%s login=%s",
        username,
        superuser,
        login,
    )
