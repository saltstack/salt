...
def ping(host, timeout=False, return_boolean=False):
    ...
    if timeout:
        ...
        cmd = f"ping -c 4 {salt.utils.network.sanitize_host(host)}"
    if return_boolean:
...
def traceroute(host):
    ...
    """
    ret = []
    cmd = f"traceroute {salt.utils.network.sanitize_host(host)}"
    out = __salt__["cmd.run"](cmd)
...
def dig(host):
    ...
    """
    cmd = f"dig {salt.utils.network.sanitize_host(host)}"
    return __salt__["cmd.run"](cmd)
