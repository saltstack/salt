...
def describe_load_balancers(
    names=None, load_balancer_arns=None, region=None, key=None, keyid=None, profile=None
):
    """
    ...
    if names and load_balancer_arns:
        raise SaltInvocationError(
            "At most one of names or load_balancer_arns may " "be provided"
        )
    if names:
        albs = names
    ...
    try:
        if names:
            ...
            ret = conn.describe_load_balancers(LoadBalancerArns=albs_list)[
                "LoadBalancers"
            ]
        else:
            ret = []
...
def describe_target_groups(
    ...
    if names and target_group_arns:
        raise SaltInvocationError(
            "At most one of names or target_group_arns may " "be provided"
        )
    if names:
        target_groups = names
    ...
    try:
        if names:
            ...
            ret = conn.describe_target_groups(LoadBalancerArn=load_balancer_arn)[
                "TargetGroups"
            ]
        else:
            ret = []
