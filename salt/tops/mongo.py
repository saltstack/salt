...
def top(**kwargs):
    ...
    '''
    host = __opts__[u'mongo.host']
    port = __opts__[u'mongo.port']
    collection = __opts__[u'master_tops'][u'mongo'].get(u'collection', u'tops')
    id_field = __opts__[u'master_tops'][u'mongo'].get(u'id_field', u'_id')
    re_pattern = __opts__[u'master_tops'][u'mongo'].get(u're_pattern', u'')
    re_replace = __opts__[u'master_tops'][u'mongo'].get(u're_replace', u'')
    states_field = __opts__[u'master_tops'][u'mongo'].get(u'states_field', u'states')
    environment_field = __opts__[u'master_tops'][u'mongo'].get(u'environment_field', u'environment')

    log.info(u'connecting to %s:%s for mongo ext_tops', host, port)
    conn = pymongo.MongoClient(host, port)

    log.debug(u'using database \'%s\'', __opts__[u'mongo.db'])
    mdb = conn[__opts__[u'mongo.db']]

    user = __opts__.get(u'mongo.user')
    password = __opts__.get(u'mongo.password')

    if user and password:
        log.debug(u'authenticating as \'%s\'', user)
    ...
    log.info(
        u'ext_tops.mongo: looking up tops def for {\'%s\': \'%s\'} in mongo',
        id_field, minion_id
    )
    ...
    if result and states_field in result:
        if environment_field in result:
            ...
            environment = u'base'
        log.debug(u'ext_tops.mongo: found document, returning states')
        return {environment: result[states_field]}
    else:
        # If we can't find the minion the database it's not necessarily an
        # error.
        log.debug(
            u'ext_tops.mongo: no document found in collection %s', collection)
        return {}
