...
def ext_pillar(minion_id,
    ...
    if re.search(r'^i-([0-9a-z]{17}|[0-9a-z]{8})$', minion_id) is None:
        if 'instance-id' not in __grains__:
            log.debug(
                'Minion-id is not in AWS instance-id formation, and there is '
                'no instance-id grain for minion %s', minion_id
            )
            return {}
        if not use_grain:
            log.debug(
                'Minion-id is not in AWS instance-id formation, and option '
        ...
        if minion_ids is not None and minion_id not in minion_ids:
            log.debug(
                ...
                minion_id, minion_ids
            )
            return {}
        if re.search(r'^i-([0-9a-z]{17}|[0-9a-z]{8})$', __grains__['instance-id']) is not None:
            minion_id = __grains__['instance-id']
            log.debug(
                'Minion-id is not in AWS instance ID format, but a grain '
                'is. Using %s as the minion ID', minion_id
            )
        else:
            log.debug(
                'Nether minion id nor a grain named instance-id is in '
                'AWS format, can\'t query EC2 tags for minion %s',
                minion_id
            )
            return {}
