                ...
                for key in results:
                    ...
                    if key in immutable:
                        ...
                        if rkey not in output:
                            ...
                            log.error(
                                'Unsupported type (must be list or dict) for key %s',
                                rkey
                            )
...
if __name__ == '__main__':
    ...
    with salt.utils.files.fopen(args.config) as fh_:
        __opts__.update(
            salt.utils.yaml.safe_load(
                salt.utils.stringutils.to_unicode(fh_.read())
            )
        )
    ...
    if args.grains:
        __grains__.update(
            salt.utils.yaml.safe_load(args.grains)
        )
    ...
    if args.pillar:
        __pillar__.update(
            salt.utils.yaml.safe_load(args.pillar)
        )
