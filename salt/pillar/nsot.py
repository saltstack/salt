...
def _get_token(url, email, secret_key):
    ...
    query = salt.utils.http.query(url, data=data_dict, method="POST", decode=True)
    error = query.get("error")
    if error:
        log.error("Cannot obtain NSoT authentication token due to: %s.", error)
        log.debug(
            "Please verify NSoT URL %s is reachable and email %s is valid", url, email
        )
        return False
    else:
...
def ext_pillar(
    ...
    if minion_regex:
        ...
        if get_ext_pillar:
            ret["nsot"] = _proxy_info(
                minion_id, api_url, email, secret_key, fqdn_separator
            )
    ...
    if all_devices_regex:
        ...
        if get_ext_pillar:
            if not ret.get("nsot"):
                ret["nsot"] = {}
            ret["nsot"]["devices"] = _all_nsot_devices(api_url, email, secret_key)
