...
def set_config_modify(dn=None, inconfig=None):
    '''
    The configConfMo method configures the specified managed object in a single subtree (for example, DN).
    '''
    ret = {}
    cookie = logon()
    payload = '<configConfMo cookie="{0}" inHierarchical="false" dn="{1}">' \
              '<inConfig>{2}</inConfig></configConfMo>'.format(cookie, dn, inconfig)
...
def get_config_resolver_class(cid=None):
    '''
    The configResolveClass method returns requested managed object in a given class.
    '''
    ret = {}
    cookie = logon()
    payload = '<configResolveClass cookie="{0}" inHierarchical="false" classId="{1}"/>'.format(cookie, cid)
    r = __utils__['http.query'](DETAILS['url'],
                                data=payload,
                                method='POST',
                                decode_type='plain',
...
def grains():
    ...
    if not DETAILS.get('grains_cache', {}):
        ...
        try:
            compute_rack = get_config_resolver_class('computeRackUnit')
            DETAILS['grains_cache']['vendor'] = compute_rack['outConfigs']['computeRackUnit']['vendor']
