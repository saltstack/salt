...
def logon():
    ...
    logon_response = session.post(
        DETAILS["url"], data=json.dumps(payload), verify=False
    )

    if logon_response.status_code != 200:
        log.error(
            "Error logging into proxy. HTTP Error code: {0}".format(
    ...
    try:
        cookies = {
            "sslng_csrf_token": logon_response.cookies["sslng_csrf_token"],
...
def logout(session, cookies, csrf_token):
    ...
    """
    payload = {"jsonrpc": "2.0", "id": "ID0", "method": "logout", "params": []}
    session.post(
        DETAILS["url"],
        data=json.dumps(payload),
        cookies=cookies,
