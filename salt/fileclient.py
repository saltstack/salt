...
class Client(object):
    ...
    def get_url(self, url, dest, makedirs=False, saltenv='base',
        ...
        if dest:
            ...
            if not os.path.isdir(destdir):
                if makedirs:
                    ...
                    return ''
        elif not no_cache and getattr(url, '{0}.needs_cache'.format(url_scheme), lambda: False)():
        ...
        if '{0}.get'.format(url_scheme) in fileprotos:
            try:
                return fileprotos['{0}.get'.format(url_scheme)](
                    url=url,
                    dest=dest,
                    saltenv=saltenv,
                    makedirs=makedirs,
