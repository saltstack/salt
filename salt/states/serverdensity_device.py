...
def monitored(
    ...
    if __opts__["test"]:
        if not device_in_sd or not sd_agent_installed:
            ...
            ret[
                "comment"
            ] = "Server Density agent is set to be installed and/or device created in the Server Density DB"
            return ret
        else:
            ret["result"] = None
            ret[
                "comment"
            ] = "Server Density agent is already installed, or device already exists"
            return ret
