...
def property(
    properties, admin_username="root", admin_password="calvin", host=None, **kwargs
):
    """
    properties = {}
    """
    ...
    for key, value in properties.items():
        response = __salt__["dracr.get_property"](
            host, admin_username, admin_password, key
        )
    ...
    if __opts__["test"]:
        for key, value in properties.items():
            if properties_get[key] == value:
                ...
                ret["changes"][key] = "Will be changed to {0}".format(
                    properties_get[key]
                )
        return ret
    ...
    for key, value in properties.items():
        if properties_get[key] != value:
            response = __salt__["dracr.set_property"](
                host, admin_username, admin_password, key, value
            )
            if response is False or response["retcode"] != 0:
                ret["result"] = False
            ...
            ret["changes"][
                key
            ] = "will be changed - old value {0} , new value {1}".format(
                properties_get[key], value
            )
