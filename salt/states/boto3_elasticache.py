    ...
    **args
    ...
    if check_update:
        ...
        if need_update:
            ...
            ret["comment"] = "Replication group {0} is in the desired state.".format(
                name
            )
    return ret
...
def cache_subnet_group_present(
    ...
    if check_update:
        ...
        if need_update:
            if __opts__["test"]:
                ret["comment"] = "Cache subnet group {0} would be modified.".format(
                    name
                )
                ret["result"] = None
                return ret
            ...
            ret["comment"] = "Cache subnet group {0} is in the desired state.".format(
                name
            )
    return ret
...
def cache_parameter_group_present(
    name, region=None, key=None, keyid=None, profile=None, **args
):
    """
    ...
    current = __salt__["boto3_elasticache.describe_cache_parameter_groups"](
        name, region=region, key=key, keyid=keyid, profile=profile
    )
    ...
    if not current:
        ...
        if not created:
            ...
            return ret
        new = __salt__["boto3_elasticache.describe_cache_parameter_groups"](
            name, region=region, key=key, keyid=keyid, profile=profile
        )
        new = new[0]
        ...
        if tunables:
            ...
            updated = __salt__["boto3_elasticache.modify_cache_parameter_groups"](
            ...
            if not updated:
                ...
                ret[
                    "comment"
                ] = "Failed to update new cache parameter group `{}`.".format(name)
                return ret
            kwargs = {"Source": "user"}
            new["ParameterNameValues"] = __salt__[
                "boto3_elasticache.describe_cache_parameters"
            ](name, region=region, key=key, keyid=keyid, profile=profile, **kwargs)
        ...
        if not tunables:
            ...
            oldvals = []
            newvals = []
            curr_params = __salt__["boto3_elasticache.describe_cache_parameters"](
                name, region=region, key=key, keyid=keyid, profile=profile
            )
            ...
            curr_kvs = {
                ...
                for p in curr_params["Parameters"]
            }
            ...
            for pname, pval in req_kvs.items():
                if pname in curr_kvs and pval != curr_kvs[pname]:
                    oldvals += [
                        {"ParameterName": pname, "ParameterValue": curr_kvs[pname]}
                    ]
            ...
            if newvals:
                if __opts__["test"]:
                    ret[
                        "comment"
                    ] = "Cache parameter group {} would be updated.".format(name)
                    ret["result"] = None
                ...
                if not __salt__["boto3_elasticache.modify_cache_parameter_groups"](
                    name, region=region, key=key, keyid=keyid, profile=profile, **kwargs
                ):
                    ret["result"] = False
                    ret[
                        "comment"
                    ] = "Failed to update cache parameter group `{}`.".format(name)
                    return ret
                ret["changes"]["old"] = current[0]
                ret["changes"]["old"]["ParameterNameValues"] = oldvals
                ret["changes"]["new"] = deepcopy(current[0])
                ...
                ret[
                    "comment"
                ] = "Cache parameter group `{}` is in the desired state.".format(name)
    return ret


def cache_parameter_group_absent(
    name, region=None, key=None, keyid=None, profile=None, **args
):
    """
    ...
    ret = {"name": name, "result": True, "comment": "", "changes": {}}
    args = dict([(k, v) for k, v in args.items() if not k.startswith("_")])
    exists = __salt__["boto3_elasticache.describe_cache_parameter_groups"](
        name, region=region, key=key, keyid=keyid, profile=profile
    )
    if exists:
        if __opts__["test"]:
            ...
            ret["result"] = None
            return ret
        deleted = __salt__["boto3_elasticache.delete_cache_parameter_group"](
            name, region=region, key=key, keyid=keyid, profile=profile, **args
        )
        if deleted:
            ret["changes"]["old"] = name
