...
def mounted(name,
    ...
    # and a hash, eg. sshfs#.  In the mount list the
    # only the hostname is included.  So if we detect
    # that the mounted device is a FUSE device then we
    # remove the prefaced string so that the devices match
    ...
    if real_name in active:
        if mount:
            ...
            if opts:
                mount_invisible_options = ['defaults', 'comment', 'nobootwait', 'reconnect']
                for opt in opts:
