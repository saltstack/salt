...
def present(
    ...
    if default_organization:
        try:
            ...
            ret[
                "comment"
            ] = "Error while looking up user {}'s default grafana org {}: {}".format(
                name, default_organization, e
            )
            ret["result"] = False
            return ret
    new_data = _get_json_data(
        login=name,
        email=email,
        name=fullname,
        theme=theme,
    ...
    old_data = _get_json_data(
        login=None, email=None, name=None, theme=None, orgId=None, defaults=user_data
    )
    if organizations:
    ...
    if new_data != old_data:
        if __opts__["test"]:
            ...
            return ret
        __salt__["grafana4.update_user"](
            user["id"], profile=profile, orgid=org_id, **new_data
        )
        dictupdate.update(
            ret["changes"],
...
def _update_user_organizations(user_name, user_id, organizations, ret, profile):
    for org in organizations.items():
        org_name, org_role = (
            org if isinstance(org, tuple) and len(org) == 2 else (org, "Viewer")
        )
        try:
            ...
            ret[
                "comment"
            ] = "Error while looking up user {}'s grafana org {}: {}".format(
                ...
                if org_user["role"] != org_role:
                    try:
                        __salt__["grafana4.update_org_user"](
                            user_id, orgname=org_name, profile=profile, role=org_role
                        )
                    except HTTPError as e:
                        ret[
                            "comment"
                        ] = "Error while setting role {} for user {} in grafana org {}: {}".format(
        ...
        if not user_found:
            ...
            __salt__["grafana4.create_org_user"](
                orgname=org_name, profile=profile, role=org_role, loginOrEmail=user_name
            )
    return ret
