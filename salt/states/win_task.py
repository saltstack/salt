...
def __virtual__():
    ...
    if (
        ...
        or "task.list_tasks" not in __salt__
        or "pywintypes" not in globals()
    ):
        return (
            False,
            "State win_task: state only works on Window systems with task loaded",
        )
...
def _get_arguments(
    ...
    for key in key_arguments:
        ...
        if block[key] not in arguments_need_it:
            return "{0} item {1} is not in key item list {2}".format(
                repr(key), repr(block[key]), list(arguments_need_it)
            )
...
def present(
    ...
    # make task
    ret["result"] = __salt__["task.create_task"](
        name=name,
        location=location,
        user_name=user_name,
        password=password,
