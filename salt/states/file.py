...
def keyvalue(
    ...
    for line in file_contents:
        ...
        if prune:
            if not test_line.startswith(tuple(key.lower() if key_ignore_case else key for key in key_values.keys())):
    ...
    if changes > 0 or deletions > 0:
        ...
        if __opts__["test"]:
            ret["comment"] = "File {n} is set to be changed ({c} lines modified, {d} deleted)".format(
