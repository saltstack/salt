...
def deprecation_warning_raised(name):
    """
    Raise deprecation warnings during a state run.
    """
    ret = {"name": name, "changes": {}, "result": True, "comment": ""}
    ret["result"] = __salt__["test.deprecation_warning"]()
    if ret["result"]:
        ret["comment"] = "Deprecation warnings were raised"
    return ret
