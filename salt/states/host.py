...
def present(name, ip):  # pylint: disable=C0103
    ...
    for _ip in ip:
        ...
        if salt.utils.validate.net.ipv4_addr(_ip) or salt.utils.validate.net.ipv6_addr(_ip):
            ...
            ret['result'] = False
            comments.append('Failed to set host {0} ({1})'.format(name, _ip))
            continue
...
def absent(name, ip):  # pylint: disable=C0103
    ...
    for _ip in ip:
        ...
        ret['result'] = False
        comments.append('Failed to remove host  {0} ({1})'.format(name, _ip))
...
def only(name, hostnames):
    ...
    aliases = (x.strip() for x in hostnames)
    ...
    ret['result'] = __salt__['hosts.set_host'](name, aliases)
    if not ret['result']:
