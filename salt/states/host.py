...
def present(name, ip, comment='', clean=False):  # pylint: disable=C0103
    ...
    for addr, host_info in six.iteritems(all_hosts):
        if addr not in ip:
            ...
            if 'aliases' in host_info and name in host_info['aliases']:
                if comment and 'comment' in host_info and host_info['comment'] != comment:
    ...
    for addr, comment in update_comment:
        if __opts__['test']:
            ...
            if __salt__['hosts.set_comment'](addr, comment):
                ...
                ret['result'] = False
                comments.append('Failed to add comment for host {0} ({1})'.format(addr, comment))
                continue
        ret['changes'].setdefault('comment_added', {}).setdefault(addr, []).append(comment)
