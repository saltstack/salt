# -*- coding: utf-8 -*-
'''
Management of Microsoft SQLServer Users (logins)
================================================

The mssql_user module is used to create
and manage SQL Server Users and their
...
def present(name, password=None, domain=None, database=None, server_roles=None, roles=None, login_options=None, user_options=None, **kwargs):
    ...
    if __salt__['mssql.user_exists'](name, domain=domain, database=database, **kwargs):
        ret['comment'] = 'User {0} is already present (Not going to try to set its password)'.format(name)
        return ret
    if __opts__['test']:
        ret['result'] = None
