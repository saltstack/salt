...
def present(name: str, kms_host="", kms_port=0, activate=False):
    ...
    if (info[0] == None) or (name[-5:] != info[1][0]["PartialProductKey"]):
        ...
        if test_install[0] != True:
            ...
            ret["comment"] = "{0} was present".format(name[-5:])
            ret["changes"]["product key"] = {
                "old":  infoIsNone(info, "PartialProductKey"),
                "new": "XXXXX-XXXXX-XXXXX-XXXXX-{0}".format(name[-5:]),
            }
    ...
    if kms_host != "" and kms_host != info[1][0]["KeyManagementServiceMachine"]:
        ...
        install = __salt__["mslicense.set_kms_host"](kms_host, name)
        if install[0] != True:
    ...
    if kms_port != 0 and kms_port != info[1][0]["KeyManagementServicePort"]:
        ...
        install = __salt__["mslicense.set_kms_port"](kms_port, name)
        if install[0] != True:
...
def absent(name: str, all=False):
    ...
    info = __salt__["mslicense.info"](name[-5:])
    if info[0] == False:
        log.error("error occurred while collecting data", info[1])
    ...
    if __opts__["test"]:
        ...
        if info[0] == None:
            ret["changes"]["not uninstall product key"] = {
                "old": "",
                "new": ""
            }
        else:
    ...
    if info[0] == True:
        remove = __salt__["mslicense.uninstall"](name[-5:])
        if remove[0] == False:
            log.error("error when removing product key", remove[1])
        ...
        test_remove = __salt__["mslicense.info"](name[-5:])
        ...
        if test_remove[0] == True:
            log.error("product keys was not removed", test_remove[1])

            ret["result"] = False
            ret["comment"] = "product kes(-s) not removed"
            ret["changes"]["absent key"] = {
                "old": info[1],
                "new": "check failed, keys found"
            }
            return ret
        else:
...
def present_kms(name: str, kms_port=0, key=""):
    ...
    if kms_port != 0:
        ...
        if info_port[0] == False:
            log.error(
                "error occurred while collecting kms-port info", info_port[1])
    ...
    if __opts__["test"]:
        ...
        if (info[0] == None) or (info[1] != name):
            ret["changes"]["kms host"] = {
                "old": info[1],
                "new": name,
            }

        if (kms_port != 0) and ((info_port[0] == None) or kms_port != info_port[1]):
            ret["changes"]["kms port"] = {
                "old": info_port[1],
                "new": kms_port,
            }
    ...
    # If the kms server is not installed, install it
    if name != info[1]:
        ...
        if test_install[0] != True:
            ...
            ret["comment"] = "kms-host was installed"
            ret["changes"]["kms host"] = {
                "old": info[1],
                "new": test_install[1]
            }
    ...
    if kms_port != 0 and kms_port != info_port[1]:
        ...
        if install[0] == False:
            ...
            ret["changes"]["kms port"] = {
                "old": info_port[1],
                "new": "error when installing the kms-port: {0}".format(install[1])
            }
            return ret

        test_install = __salt__["mslicense.get_kms_port"](key)
        if test_install[0] != True:
            log.error("error check the kms-port", install[1])

            ret["result"] = False
            ret["comment"] = "error when check the kms-port"
            ret["changes"]["kms port"] = {
                "old": info_port[1],
                "new": "error kms port checking: {0}".format(test_install[0])
            }
            return ret
        else:
            ret["changes"]["kms port"] = {
