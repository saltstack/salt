...
def run_check(cmd_kwargs, onlyif, unless, group, creates):
    '''
...
def run(name,
    ...
    try:
        cret = run_check(cmd_kwargs, onlyif, unless, group, creates)
...
def call(name,
    ...
    if HAS_GRP:
        pgid = os.getegid()
    try:
        cret = run_check(cmd_kwargs, onlyif, unless, None, creates)
        if isinstance(cret, dict):
            ret.update(cret)
            return ret
    finally:
