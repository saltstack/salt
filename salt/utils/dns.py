...
def parse_systemd_resolve():
    """
    Parse the information from systemd-resolve
    """
    ...
    cmd = __salt__["cmd.run_all"](
        "{}".format(cmd), python_shell=False, output_loglevel="quiet"
    )

    if cmd["retcode"] == 0:
        prev_section = None
        lines = [re.sub("[\t ]", "", line) for line in cmd["stdout"].splitlines()]
