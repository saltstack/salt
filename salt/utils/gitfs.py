...
def _verify_gitpython(opts, folder, quiet=False):
    '''
    ...
    opts['verified_gitfs_provider'] = 'gitpython'
    return True


def _verify_pygit2(opts, folder, quiet=False):
    '''
    ...
    opts['verified_gitfs_provider'] = 'pygit2'
    return True


def _verify_dulwich(opts, folder, quiet=False):
    '''
    ...
    opts['verified_gitfs_provider'] = 'dulwich'
    return True


def get_provider(opts, folder):
    '''
    Determine which gitfs_provider to use
    '''
    # Don't re-perform all the verification if we already have a verified
    # provider
    if 'verified_gitfs_provider' in opts:
        return opts['verified_gitfs_provider']
    provider = opts.get('gitfs_provider', '').lower()
    if not provider:
        if _verify_pygit2(opts, folder, quiet=True):
            return 'pygit2'
        elif _verify_gitpython(opts, folder, quiet=True):
            return 'gitpython'
        elif _verify_dulwich(opts, folder, quiet=True):
            return 'dulwich'
        else:
            log.error(
        ...
        elif provider == 'pygit2' and _verify_pygit2(opts, folder):
            return 'pygit2'
        elif provider == 'gitpython' and _verify_gitpython(opts, folder):
            return 'gitpython'
        elif provider == 'dulwich' and _verify_dulwich(opts, folder):
            return 'dulwich'
    return ''


def _dulwich_conf(opts, folder, repo):
    '''
...
def _dulwich_remote(opts, folder, repo):
    '''
    Returns the remote url for the specified repo
    '''
    return _dulwich_conf(opts, folder, repo).get(('remote', 'origin'), 'url')


def _dulwich_walk_tree(opts, folder, repo, tree, path):
    '''
...
def _get_tree_gitpython(opts, folder, repo, tgt_env):
    '''
    Return a git.Tree object if the branch/tag/SHA is found, otherwise None
    '''
    if tgt_env == 'base':
        tgt_env = repo['base']
    if tgt_env == repo['base'] or tgt_env in envs(opts, folder):
    ...
    if not _env_is_exposed(opts, folder, tgt_env):
        return None
    try:
...
def _get_tree_pygit2(opts, folder, repo, tgt_env):
    '''
    ...
    if tgt_env == repo['base'] or tgt_env in envs(opts, folder):
        for ref in repo['repo'].listall_references():
            ...
            if rtype in ('remotes', 'tags'):
                ...
                rspec = parted[2] if parted[2] else parted[0]
                if rspec == tgt_env and _env_is_exposed(opts, folder, rspec):
...
def _get_tree_dulwich(opts, folder, repo, tgt_env):
    '''
...
def _stale_refs_pygit2(opts, folder, repo):
    '''
...
def _verify_auth(opts, folder, repo):
    '''
    ...
    '''
    if os.path.isabs(repo['url']) or get_provider(opts, folder) not in AUTH_PROVIDERS:
...
def init(opts, folder, remotes=None):
    ...
    '''
    bp_ = os.path.join(opts['cachedir'], folder)
    ...
    ssl_verify = 'true' if opts.get('gitfs_ssl_verify', True) else 'false'
    new_remote = False
    repos = []
    ...
    for remote in remotes:
        ...
        try:
            if provider == 'gitpython':
                repo, new = _init_gitpython(opts, folder, rp_, repo_url, ssl_verify)
                if new:
                    new_remote = True
            elif provider == 'pygit2':
                repo, new = _init_pygit2(opts, folder, rp_, repo_url, ssl_verify)
                if new:
                    new_remote = True
            elif provider == 'dulwich':
                repo, new = _init_dulwich(opts, folder, rp_, repo_url, ssl_verify)
                if new:
                    new_remote = True
            else:
            ...
            if repo is not None:
                ...
                if not _verify_auth(opts, folder, repo_conf):
                    continue
    ...
    if new_remote:
        remote_map = os.path.join(opts['cachedir'], folder, 'remote_map.txt')
        try:
...
def _init_gitpython(opts, folder, rp_, repo_url, ssl_verify):
    '''
...
def _init_pygit2(opts, folder, rp_, repo_url, ssl_verify):
    '''
...
def _init_dulwich(opts, folder, rp_, repo_url, ssl_verify):
    '''
    ...
    if not os.listdir(rp_):
        ...
        try:
            ...
            new = True
            conf = _dulwich_conf(opts, folder, repo)
...
def purge_cache(opts, folder):
    ...
    for repo in init(opts, folder):
        try:
            with _aquire_update_lock_for_repo(opts, folder, repo):
...
def _aquire_update_lock_for_repo(opts, folder, repo):
    provider = get_provider(opts, folder)
    ...
    with wait_for_write_lock(opts, folder, os.path.join(working_dir, 'update.lk')):
        yield


@contextlib.contextmanager
def wait_for_write_lock(opts, folder, filename):
...
def update(opts, folder, fire_events=True):
    '''
    ...
    env_cache = os.path.join(opts['cachedir'], folder, 'envs.p')
    if data.get('changed', False) is True or not os.path.isfile(env_cache):
        env_cachedir = os.path.dirname(env_cache)
        if not os.path.exists(env_cachedir):
            os.makedirs(env_cachedir)
        new_envs = envs(opts, folder, ignore_cache=True)
    ...
    if opts.get('fileserver_events', False) and fire_events:
    ...
    try:
        def _find_file(_path, _tgt_env='base', **_kwargs):
            return find_file(opts, folder, _path, tgt_env=_tgt_env, **_kwargs)

        salt.fileserver.reap_fileserver_cache_dir(
            os.path.join(opts['cachedir'], folder, 'hash'),
            _find_file
        )
...
def _env_is_exposed(opts, folder, env):
    '''
    ...
    return salt.utils.check_whitelist_blacklist(
        env,
        whitelist=opts['gitfs_env_whitelist'],
        blacklist=opts['gitfs_env_blacklist']
    )


def envs(opts, folder, ignore_cache=False):
    '''
    Return a list of refs that can be used as environments
    '''
    if not ignore_cache:
        env_cache = os.path.join(opts['cachedir'], folder, 'envs.p')
        cache_match = salt.fileserver.check_env_cache(opts, env_cache)
    ...
    for repo in init(opts, folder):
        if provider == 'gitpython':
            ret.update(_envs_gitpython(opts, folder, repo))
        elif provider == 'pygit2':
            ret.update(_envs_pygit2(opts, folder, repo))
        elif provider == 'dulwich':
            ret.update(_envs_dulwich(opts, folder, repo))
        else:
            # Should never get here because the provider has been verified
            # in __virtual__(). Log an error and return an empty list.
            log.error(
...
def _envs_gitpython(opts, folder, repo):
    '''
    ...
    for ref in repo['repo'].refs:
        ...
        if isinstance(ref, git.Head):
            if rspec == repo['base']:
                rspec = 'base'
            if ref not in remote.stale_refs and _env_is_exposed(opts, folder, rspec):
                ret.add(rspec)
        elif isinstance(ref, git.Tag) and _env_is_exposed(opts, folder, rspec):
            ret.add(rspec)
    return ret


def _envs_pygit2(opts, folder, repo):
    '''
    Check the refs and return a list of the ones which can be used as salt
    environments.
    '''
    ret = set()
    stale_refs = _stale_refs_pygit2(opts, folder, repo['repo'])
    for ref in repo['repo'].listall_references():
        ref = re.sub('^refs/', '', ref)
        ...
        if rtype == 'remotes':
            if rspec not in stale_refs:
                ...
                if rspec == repo['base']:
                    rspec = 'base'
                if _env_is_exposed(opts, folder, rspec):
                    ret.add(rspec)
        elif rtype == 'tags' and _env_is_exposed(opts, folder, rspec):
            ret.add(rspec)
    return ret


def _envs_dulwich(opts, folder, repo):
    '''
    Check the refs and return a list of the ones which can be used as salt
    environments.
    '''
...
def find_file(opts, folder, path, tgt_env='base', **kwargs):  # pylint: disable=W0613
    '''
    ...
    fnd = {'path': '',
           'rel': ''}
    if os.path.isabs(path) or tgt_env not in envs(opts, folder):
        return fnd

    provider = get_provider(opts, folder)
    dest = os.path.join(opts['cachedir'], folder, 'refs', tgt_env, path)
    hashes_glob = os.path.join(opts['cachedir'],
                               folder,
                               'hash',
                               tgt_env,
                               '{0}.hash.*'.format(path))
    blobshadest = os.path.join(opts['cachedir'],
                               folder,
                               'hash',
                               tgt_env,
                               '{0}.hash.blob_sha1'.format(path))
    lk_fn = os.path.join(opts['cachedir'],
                         folder,
                         'hash',
                         tgt_env,
                         '{0}.lk'.format(path))
    ...
    for repo in init(opts, folder):
        with _aquire_update_lock_for_repo(opts, folder, repo):
            ...
            depth = 0
            if provider == 'gitpython':
                tree = _get_tree_gitpython(opts, folder, repo, tgt_env)
                if not tree:
                ...
                tree = _get_tree_pygit2(opts, folder, repo, tgt_env)
                if not tree:
                    # Branch/tag/SHA not found in repo, try the next
                    continue
                while True:
                ...
                while True:
                    ...
                    tree = _get_tree_dulwich(opts, folder, repo, tgt_env)
                    tree = _dulwich_walk_tree(opts, folder, repo['repo'], tree, prefix_dirs)
...
def serve_file(opts, folder, load, fnd):
    '''
    ...
    with salt.utils.fopen(fnd['path'], 'rb') as fp_:
        fp_.seek(load['loc'])
        data = fp_.read(opts['file_buffer_size'])
        if gzip and data:
...
def file_hash(opts, folder, load, fnd):
    '''
    ...
    if not all(x in load for x in ('path', 'saltenv')):
        return ''
    ret = {'hash_type': opts['hash_type']}
    relpath = fnd['rel']
    path = fnd['path']
    hashdest = os.path.join(opts['cachedir'],
                            folder,
                            'hash',
                            load['saltenv'],
    ...
    if not os.path.isfile(hashdest):
        ...
        ret['hsum'] = salt.utils.get_hash(path, opts['hash_type'])
...
def _file_lists(opts, folder, load, form):
    '''
    ...
    list_cachedir = os.path.join(opts['cachedir'], 'file_lists', folder)
    ...
    cache_match, refresh_cache, save_cache = \
        salt.fileserver.check_file_list_cache(
            opts, form, list_cache, w_lock
        )
    if cache_match is not None:
        return cache_match
    if refresh_cache:
        ret = {}
        ret['files'], ret['symlinks'] = _get_file_list(opts, folder, load)
        ret['dirs'] = _get_dir_list(opts, folder, load)
        if save_cache:
            salt.fileserver.write_file_list_cache(
                opts, ret, list_cache, w_lock
            )
...
def file_list(opts, folder, load):
    '''
    Return a list of all files on the file server in a specified
    environment
    '''
    return _file_lists(opts, folder, load, 'files')


def _get_file_list(opts, folder, load):
    '''
    Return a list of all files on the file server in a specified
    environment
    '''
    ...
    provider = get_provider(opts, folder)
    if 'saltenv' not in load or load['saltenv'] not in envs(opts, folder):
        return [], {}
    files = set()
    symlinks = {}
    for repo in init(opts, folder):
...
def _file_list_gitpython(opts, folder, repo, tgt_env):
    '''
...
def _file_list_pygit2(opts, folder, repo, tgt_env):
    '''
...
def _file_list_dulwich(opts, folder, repo, tgt_env):
    '''
    ...
    if tgt_env == 'base':
        tgt_env = repo['base']
    tree = _get_tree_dulwich(opts, folder, repo, tgt_env)
    tree = _dulwich_walk_tree(opts, folder, repo['repo'], tree, repo['root'])
    if not isinstance(tree, dulwich.objects.Tree):
        return files, symlinks
    blobs = {}
    if len(tree):
...
def file_list_emptydirs(opts, folder, load):  # pylint: disable=W0613
    '''
...
def dir_list(opts, folder, load):
    '''
    Return a list of all directories on the master
    '''
    return _file_lists(opts, folder, load, 'dirs')


def _get_dir_list(opts, folder, load):
    '''
    ...
    for repo in init(opts, folder):
        if provider == 'gitpython':
            ret.update(
                _dir_list_gitpython(opts, folder, repo, load['saltenv'])
            )
        elif provider == 'pygit2':
            ret.update(
                _dir_list_pygit2(opts, folder, repo, load['saltenv'])
            )
        elif provider == 'dulwich':
            ret.update(
                _dir_list_dulwich(opts, folder, repo, load['saltenv'])
            )
    return sorted(ret)


def _dir_list_gitpython(opts, folder, repo, tgt_env):
    '''
...
def _dir_list_pygit2(opts, folder, repo, tgt_env):
    '''
...
def _dir_list_dulwich(opts, folder, repo, tgt_env):
    '''
    Get a list of directories using pygit2
    '''
    def _traverse(tree, repo_obj, blobs, prefix):
...
def symlink_list(opts, folder, load):
    '''
    ...
    if load['saltenv'] not in envs(opts, folder):
        return {}
    try:
        prefix = load['prefix'].strip('/')
    except KeyError:
        prefix = ''
    symlinks = _file_lists(opts, folder, load, 'symlinks')
    return dict([(key, val)
