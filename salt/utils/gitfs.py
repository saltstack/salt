...
def update(opts, folder, ignore_event=False):
    '''
    ...
    if opts.get('fileserver_events', False) and ignore_event == False:
    ...
    try:
        def _find_file(_path, _tgt_env='base', **_kwargs):
            return find_file(opts, folder, _path, _tgt_env, **_kwargs)
...
def _file_lists(opts, folder, load, form):
    ...
    list_cachedir = os.path.join(opts['cachedir'], 'file_lists/gitfs')
    ...
    if refresh_cache:
        ret = {}
        ret['files'] = _get_file_list(opts, folder, load)
...
def _get_file_list(opts, folder, load):
    ...
    for repo in init(opts, folder):
        if provider == 'gitpython':
            ret.update(
                _file_list_gitpython(opts, folder, repo, load['saltenv'])
            )
        elif provider == 'pygit2':
            ret.update(
                _file_list_pygit2(opts, folder, repo, load['saltenv'])
            )
        elif provider == 'dulwich':
            ret.update(
                _file_list_dulwich(opts, folder, repo, load['saltenv'])
            )
    return sorted(ret)
