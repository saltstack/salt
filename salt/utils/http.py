...
def query(
    ...
    **kwargs,
    ...
    if backend == "requests":
        ...
        try:
            # Tornado's IOStream defaults max_buffer_size to 100MiB
            # independently of max_body_size; with a streaming_callback and
            # no Content-Length response header, Tornado silently truncates
            # the download at that limit instead of raising an error. Pass
            # max_buffer_size alongside max_body_size so both track the
            # http_max_body opt (see #69916).
            download_client_kwargs = (
                {"max_body_size": max_body, "max_buffer_size": max_body}
