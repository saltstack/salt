...
def get_tops(extra_mods="", so_mods=""):
    ...
    for mod in [m for m in extra_mods.split(",") if m]:
        if mod not in locals() and mod not in globals():
            try:
                # __import__ would return the top-level package, which for a saltext
                # is a namespace package, whose __file__ is None.
                ...
                continue
            # The directory to pack is the first parent that is not a namespace package.
            root_mod, namespace = _get_package_root_mod(imported_mod)
            moddir, modname = os.path.split(root_mod.__file__)
            base, _ = os.path.splitext(modname)
            if base == "__init__":
                tops.append((moddir, namespace))
            else:
...
def _saltext_dists(allowlist=None, blocklist=None):
    """Return the Salt extensions that are packed into the thin archive, without importing them."""
    ...
    for entry_point in salt.utils.entrypoints.iter_entry_points("salt.loader"):
        ...
        if dist is None:
            continue
        if allowlist is not None and dist.name not in allowlist:
            continue
        if dist.name in blocklist:
            continue
        dists.add(f"{dist.name}=={dist.version}")
    return sorted(dists)


def _thin_contents_id(
    extra_mods="",
    so_mods="",
    ...
    """Return an identity of the configured archive contents, to detect an outdated cached archive."""
    saltexts = (
        []
        if exclude_saltexts
        else _saltext_dists(allowlist=saltext_allowlist, blocklist=saltext_blocklist)
    )
    return "\n".join(
        [
            f"extra_mods={extra_mods}",
            f"so_mods={so_mods}",
            "saltexts=" + ",".join(saltexts),
        ]
    )
...
def gen_thin(
    ...
    thincfg = os.path.join(thindir, ".thin-gen-config")
    contents_id = _thin_contents_id(
        extra_mods=extra_mods,
        so_mods=so_mods,
        exclude_saltexts=exclude_saltexts,
        saltext_allowlist=saltext_allowlist,
        saltext_blocklist=saltext_blocklist,
    )
        ...
        if not overwrite:
            if os.path.isfile(thinver):
                ...
                if overwrite is False:
                    if os.path.isfile(thincfg):
                        with salt.utils.files.fopen(thincfg) as fh_:
                            overwrite = fh_.read() != contents_id
                    else:
                        # An archive without this file was generated by an older Salt.
                        overwrite = True
            else:
                overwrite = True
    ...
    with salt.utils.files.fopen(thincfg, "w+") as fp_:
        fp_.write(contents_id)
...
def thin_sum(cachedir, form="sha1", **kwargs):
    """
    Return the checksum of the current thin tarball

    Keyword arguments are passed to :func:`gen_thin`, which must receive the same
    arguments as when the archive was generated, otherwise it is regenerated.
    """
    thintar = gen_thin(cachedir, **kwargs)
