...
class Single:
    ...
    def _compile_pillar_for_relenv(self):
        ...
        try:
            ...
            log.info(
                "RELENV: Successfully compiled pillar data with %d top-level keys",
                len(pillar_data),
            )
        except Exception as e:  # pylint: disable=broad-exception-caught
            # We catch all exceptions here to ensure that a failure in pillar
            # compilation doesn't crash the entire salt-ssh run.
            log.warning(
                "RELENV: Failed to compile pillar, continuing with empty pillar: %s", e
            )
            pillar_data = {}
...
ARGS = {arguments}\n'''.format(
    ...
    def _cmd_block_relenv(self):
        ...
        """
        log.info(
            "RELENV: _cmd_block_relenv() starting - thin_dir=%s, fun=%s",
            self.thin_dir,
            self.fun,
        )
        ...
        stdout, stderr, retcode = self.shell.exec_cmd(cmd)
        log.info(
            "RELENV CMD_BLOCK: exec_cmd() returned - retcode=%s, stdout_len=%d, stderr_len=%d",
            retcode,
            len(stdout) if stdout else 0,
            len(stderr) if stderr else 0,
        )
