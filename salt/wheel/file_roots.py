...
def find(path, saltenv=u'base'):
    ...
    ret = []
    if saltenv not in __opts__[u'file_roots']:
        return ret
    for root in __opts__[u'file_roots'][saltenv]:
...
def list_env(saltenv=u'base'):
    ...
    if saltenv not in __opts__[u'file_roots']:
        return ret
    for f_root in __opts__[u'file_roots'][saltenv]:
        ret[f_root] = {}
...
def list_roots():
    ...
    '''
    ret = {}
    for saltenv in __opts__[u'file_roots']:
        ret[saltenv] = []
...
def write(data, path, saltenv=u'base', index=0):
    ...
    if len(__opts__[u'file_roots'][saltenv]) <= index:
    ...
    dest = os.path.join(__opts__[u'file_roots'][saltenv][index], path)
