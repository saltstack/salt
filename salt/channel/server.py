...
class ReqServerChannel:
    ...
    def _encrypt_private(
        ...
        try:
            ...
            pub = salt.crypt.PublicKey(key=pub["pub"])
        except Exception as exc:  # pylint: disable=broad-except
            log.error(
    ...
    def _auth(self, load, sign_messages=False):
        ...
        if (not key or key["state"] != "accepted") and not self.opts["open_mode"]:
            ...
            if not key:
                key = {"pub": load["pub"], "state": "accepted"}
            if load["pub"] and load["pub"] != key["pub"]:
                log.debug("Host key change detected in open mode.")
                self.cache.store(
                    "keys", load["id"], {"pub": load["pub"], "state": "accepted"}
                )
            elif not load["pub"]:
        ...
        try:
            pub = salt.crypt.PublicKey(key=key["pub"])
...
class MasterPubServerChannel:
    ...
    def send_aes_key_event(self):
        ...
        for peer in self.opts.get("cluster_peers", []):
            ...
            if peer_pub.exists():
                pub = salt.crypt.PublicKey(path=peer_pub)
    ...
    async def handle_pool_publish(self, payload, _):
        ...
        try:
            ...
            if tag.startswith("cluster/peer"):
                ...
                key = salt.crypt.PublicKey(path=pub_path)
