...
def _generate_html_table(data, f, level=0, extra_style=''):
  '''
  Generate a single table of data
  '''
  print>>f, '<table style="{0}">'.format(
  ...
  for subdata in data:
    ...
    if isinstance(subdata, dict):
      ...
      if len(subdata) == 1:
        ...
        print>>f, '<tr style="{0}">'.format(_lookup_style('tr', [row_style]))
        print>>f, '<td style="{0}">{1}</td>'.format(_lookup_style(
          'td', [cell_style, first_style, 'name', new_extra_style]), name)
        if isinstance(value, list):
          print>>f, '<td style="{0}">'.format(_lookup_style('td',
            [cell_style, second_style, 'container', new_extra_style]))
          _generate_html_table(value, f, level + 1, new_extra_style)
          print>>f, '</td>'
        else:
          print>>f, '<td style="{0}">{1}</td>'.format(_lookup_style('td',
      ...
      _generate_html_table(subdata, f, level + 1, extra_style)
      print>>f, '</td>'
      print>>f, '</tr>'
    else:
      print>>f, '<tr style="{0}">'.format(_lookup_style('tr', [row_style]))
...
def _generate_html(data, f):
  '''
  Generate report data as HTML
  '''
  print>>f, '<html>'
  print>>f, '<body>'
  _generate_html_table(data, f, 0)
  print>>f, '</body>'
  print>>f, '</html>'
...
def _generate_report(ret, setup):
  ...
  sorted_data = sorted(retdata.items(),
  ...
  if setup.get('report_everything', False) or \
    ...
    stats = [
      ...
      {'unchanged': total - failed - changed, '__style__': 'unchanged'},
    ...
    for state, data in sorted_data:
      ...
      states.append({id: single, '__style__': style})
    ...
    if function:
      ...
      report.insert(0, {'extra':
        [{function: _dict_to_name_value(func_result)}]})
...
def _sprinkle(s):
  ...
  '''
  parts = [x for sub in s.split('{') for x in sub.split('}')]
...
if __name__ == '__main__':
  ...
  '''
  with open('test.rpt', 'r') as input:
    data_text = input.read()
  data = yaml.safe_load(data_text)
  
  stringFile = StringIO.StringIO()
  _generate_html(data, stringFile)
  stringFile.seek(0)
  result = stringFile.read()
