# -*- coding: utf-8 -*-
'''
Provide authentication to Active Directory using LDAP, with extensions
to support restricting access to certain salt-minions based on the
minion's membership in an AD OU.
...
__defopts__ = {'auth.adext.uri': '',
               'auth.adext.server': 'localhost',
               'auth.adext.port': '389',
               'auth.adext.tls': False,
               'auth.adext.no_verify': False,
               'auth.adext.anonymous': False,
               'auth.adext.scope': 2,
               'auth.adext.groupou': 'Groups',
               'auth.adext.accountattributename': 'memberUid',
               'auth.adext.groupattribute': 'memberOf',
               'auth.adext.persontype': 'person',
               'auth.adext.groupclass': 'posixGroup',
               }
...
def _config(key, mandatory=True):
    ...
    '''
    try:
        value = __opts__['auth.adext.{0}'.format(key)]
    except KeyError:
        try:
            value = __defopts__['auth.adext.{0}'.format(key)]
        except KeyError:
            if mandatory:
                msg = 'missing auth.adext.{0} in master config'.format(key)
...
class _LDAPConnection(object):
    ...
    def __init__(self, uri, server, port, tls, no_verify, binddn, bindpw,
                 anonymous, accountattributename):
        '''
