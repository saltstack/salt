...
def _virtual(osdata):
    ...
    if osdata['kernel'] == 'Linux' and os.path.isfile('/usr/sbin/dmidecode'):
        vbios = subprocess.Popen(
                '/usr/sbin/dmidecode --type 1|grep -i "Product Name" ',
                shell=True,
                stdout=subprocess.PIPE
                ).communicate()[0].split(':')[1].split(' ')[1]
	grains['virtual']=vbios
