...
def windows(
    ...
    if sign:
        ...
        if ret.returncode:
            ctx.error(f"Failed to run '{' '.join(command)}'")
        ctx.info(f"{pathlib.Path('~/.signingmanager/logs/').glob('*')}")
        ctx.run("powershell.exe", "-C", "Get-WinEvent -LogName \"*Microsoft-Windows-AppxPackaging*\" -MaxEvents 150", check=False)
