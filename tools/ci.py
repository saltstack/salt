...
def pkg_download_matrix(ctx: Context):
    """
    Generate the test matrix.
    """
    _platforms = {}
    ...
    windows_entries = (("windows-2022", "Windows 2022", "amd64"),)
    _matrix = []
    for distro, display_name, arch, pkg_type in linux_entries:
        _matrix.append({"distro-slug": distro, "arch": arch, "pkg_type": pkg_type})
    _platforms["linux"] = _matrix
    _matrix = []
    for distro, display_name, arch, pkg_type in macos_entries:
        _matrix.append({"distro-slug": distro, "arch": arch, "pkg_type": pkg_type})
    _platforms["macos"] = _matrix
    _matrix = []
    for distro, display_name, arch in windows_entries:
        for pkg_type in ("nsis", "msi", "onedir"):
            _matrix.append({"distro-slug": distro, "arch": arch, "pkg_type": pkg_type})
    _platforms["windows"] = _matrix

    ctx.info("Generated matrix:")
    ctx.print(_platforms, soft_wrap=True)

    github_output = os.environ.get("GITHUB_OUTPUT")
    if github_output is not None:
        with open(github_output, "a", encoding="utf-8") as wfh:
            wfh.write(f"matrix={json.dumps(_platforms)}\n")
    ctx.exit(0)
