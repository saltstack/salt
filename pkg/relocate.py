    ...
    if LC_LOAD_DYLIB in obj:
        for x in obj[LC_LOAD_DYLIB]:
            ...
            if os.path.exists(x):
                ...
                z = pathlib.Path('@loader_path') / os.path.relpath(y, pathlib.Path(path).resolve().parent)
...
def main():
    ...
    logging.basicConfig(level=logging.getLevelName(args.log_level.upper()), format="%(asctime)s %(message)s")
