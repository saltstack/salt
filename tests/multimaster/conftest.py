...
from pytestsalt.fixtures.daemons import SaltMaster, start_daemon
...
@pytest.fixture(scope='session')
def session_master_default_options(session_master_default_options):
    with salt.utils.files.fopen(os.path.join(RUNTIME_VARS.CONF_DIR, 'mm_master')) as rfh:
        ...
        opts['root_dir'] = os.path.join(RUNTIME_VARS.TMP_ROOT_DIR, 'mm')
        return opts


@pytest.fixture(scope='session')
def session_master_config_overrides(session_master_config_overrides, session_root_dir):
    overrides = session_master_config_overrides.copy()
    pytest_stop_sending_events_file = session_root_dir.join('pytest_mm_stop_sending_events_file').strpath
...
@pytest.fixture(scope='session')
def session_minion_default_options(session_minion_default_options):
...
@pytest.fixture(scope='session')
def session_minion_config_overrides(session_master_return_port, session_secondary_master_return_port):
    return {
        'master': [
            'localhost:{}'.format(session_master_return_port),
            'localhost:{}'.format(session_secondary_master_return_port)
        ],
        'master_port': None,
        'file_roots': {
            'base': [
...
@pytest.fixture(scope='session')
def session_secondary_minion_default_options(session_secondary_minion_default_options):
...
@pytest.fixture(scope='session')
def session_secondary_minion_config_overrides(session_secondary_minion_config_overrides,
                                              session_master_return_port,
                                              session_secondary_master_return_port):
...
# ----- Sub Master Fixtures ----------------------------------------------------------------------------------------->
@pytest.fixture(scope='session')
def session_secondary_master_id():
    '''
    Returns the session scoped master id
    '''
...
@pytest.fixture(scope='session')
def session_secondary_master_publish_port():
    '''
...
@pytest.fixture(scope='session')
def session_secondary_master_return_port():
    '''
...
@pytest.fixture(scope='session')
def session_secondary_master_engine_port():
    '''
...
@pytest.fixture(scope='session')
def session_secondary_master_tcp_master_pub_port():
    '''
...
@pytest.fixture(scope='session')
def session_secondary_master_tcp_master_pull_port():
    '''
    Returns an unused localhost port
    '''
    return get_unused_localhost_port()
...
@pytest.fixture(scope='session')
def session_secondary_master_tcp_master_publish_pull():
    '''
    Returns an unused localhost port
    '''
    return get_unused_localhost_port()
...
@pytest.fixture(scope='session')
def session_secondary_master_tcp_master_workers():
    '''
    Returns an unused localhost port
    '''
    return get_unused_localhost_port()
...
@pytest.fixture(scope='session')
def session_secondary_master_log_prefix(session_secondary_master_id):
    return 'salt-master/{}'.format(session_secondary_master_id)


@pytest.fixture(scope='session')
def session_secondary_master_config_file(session_secondary_conf_dir):
    '''
    Returns the path to the salt master configuration file
    '''
    return session_secondary_conf_dir.join('master').realpath().strpath


@pytest.fixture(scope='session')
def session_secondary_master_default_options(session_master_default_options):
    opts = session_master_default_options.copy()
...
@pytest.fixture(scope='session')
def session_secondary_master_config_overrides(session_master_config_overrides,
                                              session_secondary_root_dir,
                                              session_secondary_master_id):
    overrides = session_master_config_overrides.copy()
    pytest_stop_sending_events_file = session_secondary_root_dir.join('pytest_mm_stop_sending_events_file').strpath
    with salt.utils.files.fopen(pytest_stop_sending_events_file, 'w') as wfh:
        wfh.write('')
    overrides['pytest_stop_sending_events_file'] = pytest_stop_sending_events_file
    return overrides
...
@pytest.fixture(scope='session')
def session_secondary_master_config(session_secondary_root_dir,
                                    session_secondary_master_default_options,
                                    session_secondary_master_config_file,
                                    session_secondary_master_publish_port,
                                    session_secondary_master_return_port,
                                    session_secondary_master_engine_port,
                                    session_secondary_master_config_overrides,
                                    session_secondary_master_id,
                                    ...
                                    engines_dir,
                                    log_handlers_dir,
                                    session_secondary_master_log_prefix,
                                    session_secondary_master_tcp_master_pub_port,
                                    session_secondary_master_tcp_master_pull_port,
                                    session_secondary_master_tcp_master_publish_pull,
                                    session_secondary_master_tcp_master_workers):
    '''
    ...
    '''
    return apply_master_config(session_secondary_master_default_options,
                               session_secondary_root_dir,
                               session_secondary_master_config_file,
                               session_secondary_master_publish_port,
                               session_secondary_master_return_port,
...
@pytest.fixture(scope='session')
def session_secondary_salt_master(request,
                                  session_secondary_conf_dir,
                                  session_secondary_master_id,
                                  session_secondary_master_config,
    ...
    for keyfile in ('master.pem', 'master.pub'):
        shutil.copyfile(
            os.path.join(session_master_config['pki_dir'], keyfile),
            os.path.join(session_secondary_master_config['pki_dir'], keyfile)
        )
    return start_daemon(request,
                        daemon_name='salt-master',
                        daemon_id=session_secondary_master_id,
                        daemon_log_prefix=session_secondary_master_log_prefix,
                        daemon_cli_script_name=cli_master_script_name,
                        daemon_config=session_secondary_master_config,
                        daemon_config_dir=session_secondary_conf_dir,
                        daemon_class=SaltMaster,
                        bin_dir_path=_cli_bin_dir,
                        fail_hard=_salt_fail_hard,
                        event_listener_config_dir=session_secondary_conf_dir,
                        start_timeout=60)
# <---- Sub Master Fixtures ------------------------------------------------------------------------------------------
...
def default_session_daemons(request,
    ...
    request.session.stats_processes.update(OrderedDict((
        ...
        ('Salt Sub Master', psutil.Process(session_secondary_salt_master.pid)),
    ...
    for daemon in (session_secondary_salt_minion,
                   session_secondary_salt_master,
                   session_salt_minion,
                   session_salt_master
                   ):
...
def bridge_pytest_and_runtests(reap_stray_processes,
    ...
    RUNTIME_VARS.TMP_MM_CONF_DIR = session_conf_dir.realpath().strpath
    RUNTIME_VARS.TMP_MM_SUB_CONF_DIR = session_secondary_conf_dir.realpath().strpath
    ...
    RUNTIME_VARS.RUNTIME_CONFIGS['mm_master'] = freeze(session_master_config)
    RUNTIME_VARS.RUNTIME_CONFIGS['mm_minion'] = freeze(session_minion_config)
    RUNTIME_VARS.RUNTIME_CONFIGS['mm_sub_master'] = freeze(session_secondary_master_config)
    RUNTIME_VARS.RUNTIME_CONFIGS['mm_sub_minion'] = freeze(session_secondary_minion_config)
