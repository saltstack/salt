...
def test_add(pathsep):
    ...
    def _run(name, index=None, retval=True, path=None):
        ...
        win_path_sep = patch.object(win_path, "PATHSEP", pathsep)
        win_path_path = patch.object(win_path, "get_path", mock_get)
        win_path_env = patch.object(os, "environ", env)
        win_path_dict = patch.dict(win_path.__utils__, {"reg.set_value": mock_set})
        win_path_rehash = patch.object(win_path, "rehash", MagicMock(return_value=True))

        with win_path_sep, win_path_path, win_path_env, win_path_dict, win_path_rehash:
    ...
    assert ret is True
    # assert mock_set.assert_called_once_with(
    #     win_path.HIVE,
    #     win_path.KEY,
    #     win_path.VNAME,
    #     pathsep.join(new_path),
    #     win_path.VTYPE,
    # )
...
#         mock_get = MagicMock(return_value=list(path))
#         mock_set = MagicMock(return_value=retval)
#         with patch.object(win_path, "PATHSEP", PATHSEP), patch.object(
