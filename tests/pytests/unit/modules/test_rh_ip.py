            ...
            with patch.object(rh_ip, "_raise_error_iface", return_value=None):
                with patch.object(rh_ip, "_parse_settings_bond", MagicMock()):
                    ...
                    with patch.object(
                        ...
                        assert rh_ip.build_interface("iface", "vlan", True) == ""
                    ...
                    with patch.object(rh_ip, "_read_temp", return_value="A"):
                        with patch.object(
                            ...
                            assert (
                                rh_ip.build_interface("iface", "vlan", True, test="A")
                                == "A"
                            )
...
def test_apply_network_settings():
    ...
    for majorrelease, expected_service_name in (
        ...
        with patch.dict(rh_ip.__salt__, {"service.restart": mock_service}), patch.dict(
            rh_ip.__grains__,
...
def test_build_interface_teamport():
    """
...
def test_build_interface_bond_mode_4_xmit():
    """
...
def test_build_interface_bond_mode_4_lacp():
    """
    Test that mode 4 bond interfaces are properly built
    """
    kwargs = {
