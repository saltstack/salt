import tempfile

import pytest
import salt.utils.files
from salt.modules import x509 as x509_mod
from salt.states import x509
from tests.support.helpers import dedent
from tests.support.mock import MagicMock, patch

try:
    import M2Crypto  # pylint: disable=unused-import

    HAS_M2CRYPTO = True
except ImportError:
    HAS_M2CRYPTO = False


@pytest.fixture
def configure_loader_modules():
    file_managed_mock = MagicMock()
    file_managed_mock.return_value = {"changes": True}

    return {
        x509: {
            "__opts__": {"fips_mode": True},
            "__salt__": {
                "x509.get_pem_entry": x509_mod.get_pem_entry,
                "x509.get_private_key_size": x509_mod.get_private_key_size,
            },
            "__states__": {"file.managed": file_managed_mock},
        }
    }


def test_certificate_info_matches_fips_off():
    cert_info = {"MD5 Finger Print": ""}
    required_info = {"MD5 Finger Print": ""}
    with patch.dict(x509.__opts__, {"fips_mode": False}):
        ret = x509._certificate_info_matches(cert_info, required_info)
        assert ret == (True, [])


@pytest.mark.skipif(not HAS_M2CRYPTO, reason="Skipping, M2Crypto is unavailable")
def test_private_key_fips_mode():
    test_key = dedent(
        """
    -----BEGIN PRIVATE KEY-----
    
    lVz1j/xGOseQk4cVos681Wpw
    -----END PRIVATE KEY-----"""
    )
    test_cert = dedent(
        """
    -----BEGIN CERTIFICATE-----
    
    Y3BMA5m+J+PZrNy7EQSa
    -----END CERTIFICATE-----
    """
    )
    fp, name = tempfile.mkstemp()
    with salt.utils.files.fopen(name, "w") as fd:
        fd.write(test_key)
        fd.write(test_cert)
    ret = x509.private_key_managed(name)
    x509.__states__.file_managed_mock.assert_called_once()
    assert (
        x509.__states__.file_managed_mock.call_args.kwargs["contents"].strip()
        == test_key.strip()
    )


def test_certificate_info_matches():
    cert_info = {"MD5 Finger Print": ""}
    required_info = {"MD5 Finger Print": ""}
    ret = x509._certificate_info_matches(cert_info, required_info)
    assert ret == (False, ["MD5 Finger Print"])
