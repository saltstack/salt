...
@pytest.fixture
def pki_dir():
    pki_dir_path = tempfile.mkdtemp(dir=RUNTIME_VARS.TMP)
    return pki_dir_path


def test_keys(pki_dir):
    """
    ...
    with patch("os.path.isfile", MagicMock(return_value=False)):
        ...
        with patch.dict(virt.__salt__, {"pillar.ext": mock}):
            ...
            assert virt.keys(name, basepath=pki_dir) == ret
...
def test_keys_with_expiration_days(pki_dir):
    """
    Test to manage libvirt keys.
    """
    with patch("os.path.isfile", MagicMock(return_value=False)):
        ...
        with patch.dict(virt.__salt__, {"pillar.ext": mock}):
            comt = "All keys are correct"
            ret.update({"comment": comt})
            assert virt.keys(name, basepath=pki_dir, expiration_days=700) == ret
...
def test_keys_with_state(pki_dir):
    """
    Test to manage libvirt keys.
    """
    with patch("os.path.isfile", MagicMock(return_value=False)):
        ...
        with patch.dict(virt.__salt__, {"pillar.ext": mock}):
            comt = "All keys are correct"
            ret.update({"comment": comt})
            assert virt.keys(name, basepath=pki_dir, st="California") == ret
...
def test_keys_with_all_options(pki_dir):
    """
    Test to manage libvirt keys.
    """
    with patch("os.path.isfile", MagicMock(return_value=False)):
...
def test_pool_defined(pki_dir):
    """
    pool_defined state test cases.
    """
    ret = {"name": "mypool", "changes": {}, "result": True, "comment": ""}
    mocks = {
        mock: MagicMock(return_value=True) for mock in ["define", "autostart", "build"]
    }
    with patch.dict(virt.__opts__, {"test": False}):
        with patch.dict(
            virt.__salt__,
            ...
            mocks["build"].assert_called_with(
                "mypool", connection="myconnection", username="user", password="secret",
            )
        ...
        mocks["build"] = MagicMock(side_effect=virt.libvirt.libvirtError("Existing VG"))
        with patch.dict(
            virt.__salt__,
            {  # pylint: disable=no-member
                "virt.pool_info": MagicMock(
...
def test_volume_defined(pki_dir):
    """
