...
def configure_loader_modules():
    return {
        ...
        file_: {
            ...
            "__salt__": {
                "config.option": MagicMock(return_value={"obfuscate_templates": False}),
                "config.backup_mode": MagicMock(return_value=False),
            },
        },
        win_file: {"__utils__": {"dacl.check_perms": salt.utils.win_dacl.check_perms}},
        dacl: {"__opts__": {"test": False}},
    }


def test_heat_deployed():
    """
    ...
    patch_file = patch.dict(
        ...
        {"file.get_managed": file_.get_managed, "file.manage_file": file_.manage_file},
    )

    patch_create = patch(
        "salt.modules.heat.create_stack",
        MagicMock(return_value={"result": True, "comment": "Created stack 'mystack'."}),
    )

    patch_check = patch("salt.modules.file.check_perms", file_.check_perms)
    if salt.utils.platform.is_windows():
        patch_check = patch("salt.modules.file.check_perms", win_file.check_perms)

    with patch_heat, patch_file, patch_create, patch_check:
        ret = heat.deployed(
            name="mystack",
            profile="openstack1",
...
def test_heat_deployed_environment():
    """
    Test salt.states.heat.deployed method
...
def test_heat_deployed_environment_error():
    """
    Test salt.states.heat.deployed method
