...
def test_managed():
    ...
    with patch.dict(
        ...
        with patch.object(
            ...
            "exists",
            autospec=True,
            side_effect=[True, True, True, False, True, True],
        ):
            ...
            with patch.dict(virtualenv_mod.__opts__, {"test": True}):
                ...
                ret.update(
                    {"comment": "Virtualenv salt is set to be created", "result": None}
                )
                assert virtualenv_mod.managed("salt") == ret
            ...
            with patch.dict(virtualenv_mod.__opts__, {"test": False}):
                ret.update(
                    {
                        "comment": "The 'use_wheel' option is only supported in pip "
                        "between 1.4 and 9.0.3."
