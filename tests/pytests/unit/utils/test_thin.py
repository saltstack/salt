...
try:
    ...
    fake_backports_dir = pathlib.Path(tempfile.mkdtemp(prefix="fake-backports"))
    fake_backports_dir.mkdir(parents=True, exist_ok=True)
    init_file = fake_backports_dir / "__init__.py"
    init_file.write_text("")
    backports = types.SimpleNamespace(__file__=str(init_file), __name__="backports")
...
def test_get_ext_tops_cfg_missing_dependencies(thin_ctx):
    """
...
@patch("salt.utils.thin.log", MagicMock())
@patch("salt.utils.thin.os.path.isfile", MagicMock(return_value=False))
def test_get_ext_tops_cfg_missing_interpreter(thin_ctx):
    """
...
@patch("salt.utils.thin.log", MagicMock())
@patch("salt.utils.thin.os.path.isfile", MagicMock(return_value=False))
def test_get_ext_tops_cfg_wrong_interpreter(thin_ctx):
    """
...
@patch("salt.utils.thin.log", MagicMock())
@patch("salt.utils.thin.os.path.isfile", MagicMock(return_value=False))
def test_get_ext_tops_cfg_interpreter(thin_ctx):
    """
...
@patch("salt.utils.thin.log", MagicMock())
@patch("salt.utils.thin.os.path.isfile", MagicMock(return_value=False))
def test_get_ext_tops_dependency_config_check(thin_ctx):
    """
...
def test_add_dep_path(thin_ctx):
    """
...
def test_get_salt_call_script(thin_ctx):
    """
...
def test_get_ext_namespaces_empty(thin_ctx):
    """
...
def test_get_ext_namespaces(thin_ctx):
    """
...
def test_get_ext_namespaces_failure(thin_ctx):
    """
...
@patch("salt.exceptions.SaltSystemExit", Exception)
def test_gen_thin_fails_ancient_python_version(thin_ctx):
    """
    ...
    with pytest.raises(salt.exceptions.SaltSystemExit) as err:
        ...
        thin.gen_thin("")
    assert 'The minimum required python version to run salt-ssh is "3"' in str(
        err.value
    )
...
def test_gen_thin_compression_fallback_py3(thin_ctx):
    """
...
@patch("salt.utils.thin.sys.version_info", _version_info(3, 6))
@patch("salt.utils.path.which", MagicMock(return_value="/usr/bin/python"))
def test_gen_thin_control_files_written_py3(thin_ctx):
    """
...
@patch("salt.utils.path.which", MagicMock(return_value="/usr/bin/python"))
def test_gen_thin_main_content_files_written_py3(thin_ctx):
    """
...
@patch("salt.utils.hashutils.DigestCollector", MagicMock())
@patch("salt.utils.path.which", MagicMock(return_value="/usr/bin/python"))
def test_gen_thin_ext_alternative_content_files_written_py3(thin_ctx):
    """
...
def test_get_supported_py_config_typecheck(thin_ctx):
    """
...
def test_get_supported_py_config_base_tops(thin_ctx):
    """
...
def test_get_supported_py_config_ext_tops(thin_ctx):
    """
...
@patch("salt.utils.thin.shutil", MagicMock())
@patch("salt.utils.thin.sys.version_info", _version_info(3, 6))
def test_gen_thin_control_files_written_access_denied_cwd(thin_ctx):
    """
    Test thin.gen_thin function if control files are written (version, salt-call etc)
...
def test_get_tops_python(thin_ctx):
    """
...
def test_pack_alternatives_exclude(thin_ctx):
    """
...
def test_pack_alternatives(thin_ctx):
    """
    test thin._pack_alternatives
    """
    with patch("salt.utils.thin.get_ext_tops", MagicMock(return_value=thin_ctx.tops)):
        thin._pack_alternative(thin_ctx.ext_conf, thin_ctx.digest, thin_ctx.tar)
        calls = thin_ctx.tar.mock_calls
        for _file in thin_ctx.exp_files:
            assert [x for x in calls if f"{_file}" in x[-2]]
            assert [
                x
                for x in calls
...
def test_pack_alternatives_not_normalized(thin_ctx):
    """
    test thin._pack_alternatives when the path
    is not normalized
    """
    tops = copy.deepcopy(thin_ctx.tops)
    tops["test"]["dependencies"] = [thin_ctx.jinja_fp + "/"]
...
def test_pack_alternatives_path_doesnot_exist(thin_ctx):
    """
    test thin._pack_alternatives when the path
    ...
    for _file in thin_ctx.exp_files:
        ...
        kwargs = [
            x for x in calls if os.path.join("test", "pyall", _file) in x[-1]["arcname"]
        ]
        if "jinja2" in _file:
            assert not arg
            assert not kwargs
...
def test_pack_alternatives_auto_detect(thin_ctx):
    """
    ...
    patch_tops_py = patch(
        "salt.utils.thin.get_tops_python", return_value=thin_ctx.fake_libs
    )
...
def test_pack_alternatives_empty_dependencies(thin_ctx):
    """
