...
def test_ordereddict_dict_items():
    """
    Test case for SaltOrderedDicts items.
    """
    dict_one = {"foo": 1, "bar": 2, "baz": 3}
    expected_items = 'odict_items([("foo", 1), ("bar", 2), ("baz", 3)])'

    odict_one = SaltOrderedDict(dict_one)
    myodict = odict_one.items()
    log.warning(f"DGM test_ordered_dict_items items '{str(myodict)}'")
    ## assert odict_one.items() == expected_items

    myodict = odict_one.keys()
    log.warning(f"DGM test_ordered_dict_items keys '{myodict}'")
    ## assert odict_one.items() == expected

    myodict = odict_one.values()
    log.warning(f"DGM test_ordered_dict_items values '{myodict}'")
    ## assert odict_one.items() == expected

    myodict = odict_one.popitem()
    log.warning(f"DGM test_ordered_dict_items popitem '{myodict}'")
    ## assert odict_one.items() == expected

    odict_one.clear()
    myodict = odict_one.popitem()
    log.warning(f"DGM test_ordered_dict_items clear '{myodict}'")
    ## assert odict_one.items() == expected

    dgm_dir = dir(odict_one)
    log.warning(f"DGM test_ordered_dict_items dgm_dir iterkeys '{dgm_dir}'")

    for myodict in odict_one.iterkeys():
        log.warning(f"DGM test_ordered_dict_items iterkeys '{myodict}'")
    ## assert odict_one.items() == expected

    for myodict in odict_one.itervalues():
        log.warning(f"DGM test_ordered_dict_items itervalues '{myodict}'")
    ## assert odict_one.items() == expected
