...
# ---------------------------------------------------------------------------
# enabled= parameter (VCOPS-98852: pillar_mask_output) — each call site reads
# its own opts.get("pillar_mask_output", True) and passes it in explicitly,
# matching the existing pillar_merge_lists/pillar_safe_render_error pattern
# (no cached/global state in this module).
# ---------------------------------------------------------------------------


def test_enabled_defaults_to_true():
    # Callers that don't pass enabled= (or pass True) keep masking on.
    assert secret.serial("hunter2") == secret.REDACT_PLACEHOLDER
    assert isinstance(secret.hide({"k": "v"}), secret.MaskedDict)


def test_enabled_false_disables_serial_redaction():
    assert secret.serial("hunter2", enabled=False) == "hunter2"
    assert secret.serial(42, enabled=False) == 42
    d = secret.MaskedDict({"password": "hunter2"})
    assert secret.serial(d, enabled=False) == {"password": "hunter2"}


def test_enabled_false_disables_hide_wrapping():
    assert secret.hide({"k": "v"}, enabled=False) == {"k": "v"}
    assert not isinstance(secret.hide({"k": "v"}, enabled=False), secret.MaskedDict)
    assert secret.hide(["a"], enabled=False) == ["a"]
    assert not isinstance(secret.hide(["a"], enabled=False), secret.MaskedList)


def test_enabled_false_disables_mask_output():
    d = {"pillar_data": secret.MaskedDict({"password": "secret"})}
    result = secret.mask_output(d, enabled=False)
    assert result["pillar_data"]["password"] == "secret"


def test_enabled_false_disables_no_log_mask():
    ret = {"comment": "plaintext_password", "changes": {}, "result": True}
    secret.no_log_mask(ret, enabled=False)
    assert ret["comment"] == "plaintext_password"
