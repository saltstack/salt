...
# ---------------------------------------------------------------------------
# configure() / global masking killswitch (VCOPS-98852: pillar_mask_output)
# ---------------------------------------------------------------------------


def test_configure_defaults_to_enabled_when_opt_absent():
    secret.configure({})
    try:
        assert secret.serial("hunter2") == secret.REDACT_PLACEHOLDER
    finally:
        secret.configure({"pillar_mask_output": True})


def test_configure_true_enables_masking():
    secret.configure({"pillar_mask_output": True})
    try:
        assert secret.serial("hunter2") == secret.REDACT_PLACEHOLDER
...
def test_configure_false_disables_serial_redaction():
    secret.configure({"pillar_mask_output": False})
    try:
        assert secret.serial("hunter2") == "hunter2"
        assert secret.serial(42) == 42
        d = secret.MaskedDict({"password": "hunter2"})
        assert secret.serial(d) == {"password": "hunter2"}
    finally:
        secret.configure({"pillar_mask_output": True})


def test_configure_false_disables_hide_wrapping():
    secret.configure({"pillar_mask_output": False})
    try:
        assert secret.hide({"k": "v"}) == {"k": "v"}
        assert not isinstance(secret.hide({"k": "v"}), secret.MaskedDict)
        assert secret.hide(["a"]) == ["a"]
        assert not isinstance(secret.hide(["a"]), secret.MaskedList)
    finally:
        secret.configure({"pillar_mask_output": True})


def test_configure_false_disables_mask_output():
    secret.configure({"pillar_mask_output": False})
    try:
        d = {"pillar_data": secret.MaskedDict({"password": "secret"})}
        result = secret.mask_output(d)
