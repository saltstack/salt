...
def pkg_paths_salt_user_exclusions():
    ...
    paths = [
        ...
        # Runtime files generated by minion scheduler in upgrade/no-install flows.
        "/etc/salt/minion.d",
        # Runtime cache files created by minion roots loader.
        "/var/cache/salt/minion/roots",
        # Runtime minion keys generated when minion (re)starts.
        "/etc/salt/pki/minion",
...
def test_pkg_paths(
    ...
    def _is_excluded(path):
        path_str = str(path)
        for excluded in pkg_paths_salt_user_exclusions:
            if path_str == excluded or path_str.startswith(f"{excluded}/"):
                return True
        return False

    def _is_minion_runtime_path(path):
        path_str = str(path)
        return any(
            ...
            # Directories owned by root:root
                ...
                for file in files:
                    ...
                    if str(file_path) in pkg_paths_salt_user:
                        ...
                        _is_minion_runtime_path(file_path)
                        and file_path.owner() == "salt"
                    ):
                        # Runtime minion paths can be salt-owned when minion runs as non-root.
                        continue
                    else:
