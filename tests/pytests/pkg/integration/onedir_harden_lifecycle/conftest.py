"""
Destructive-cycle fixtures for the SALT_ONEDIR_HARDEN packaging tests
(issue #70198).

These tests need a fresh install/uninstall cycle per case so they can
exercise:

- SALT_ONEDIR_HARDEN=0 opt-out (legacy chown layout + deprecation
  warning surfaces)
...
- upgrade idempotency (re-running postinst is a no-op)
...
# overrides win over the SALT_ONEDIR_HARDEN=1 default's per-daemon
...
@pytest.fixture
def install_harden_off(cleanup_harden_state, request, salt_factories_root_dir):
    """
    Fresh install with SALT_ONEDIR_HARDEN=0 in the DEB override file
    ...
    the RPM %pre / %posttrans). Verifies the legacy chown path still
    works and surfaces the deprecation warning.
    """
    DEB_OVERRIDE_FILE.parent.mkdir(parents=True, exist_ok=True)
    DEB_OVERRIDE_FILE.write_text("SALT_ONEDIR_HARDEN=0\n", encoding="utf-8")
    RPM_OVERRIDE_FILE.parent.mkdir(parents=True, exist_ok=True)
    RPM_OVERRIDE_FILE.write_text("SALT_ONEDIR_HARDEN=0\n", encoding="utf-8")
    yield from _install_with(
        {"SALT_ONEDIR_HARDEN": "0"}, request, salt_factories_root_dir
    )
...
def install_harden_default(cleanup_harden_state, request, salt_factories_root_dir):
    ...
    the built-in default (=1 on 3009.0+) is what actually applies. No
...
def install_harden_custom_home(cleanup_harden_state, request, salt_factories_root_dir):
    """
    Fresh install with SALT_ONEDIR_HARDEN=1 (default) AND an explicit
    SALT_HOME override. Proves the explicit override wins over the
    hardening default's per-daemon /var/lib/salt/<daemon>/home path.
    """
    DEB_OVERRIDE_FILE.parent.mkdir(parents=True, exist_ok=True)
    DEB_OVERRIDE_FILE.write_text(f"SALT_HOME={CUSTOM_HOME}\n", encoding="utf-8")
    RPM_OVERRIDE_FILE.parent.mkdir(parents=True, exist_ok=True)
    RPM_OVERRIDE_FILE.write_text(f"SALT_HOME={CUSTOM_HOME}\n", encoding="utf-8")
    yield from _install_with(
        {"SALT_HOME": CUSTOM_HOME}, request, salt_factories_root_dir
    )
...
def install_harden_custom_extras(
    ...
    the hardening default's per-daemon
    /var/lib/salt/<daemon>/extras-<py> path.
    """
    DEB_OVERRIDE_FILE.parent.mkdir(parents=True, exist_ok=True)
    DEB_OVERRIDE_FILE.write_text(f"SALT_EXTRAS_DIR={CUSTOM_EXTRAS}\n", encoding="utf-8")
    RPM_OVERRIDE_FILE.parent.mkdir(parents=True, exist_ok=True)
    RPM_OVERRIDE_FILE.write_text(f"SALT_EXTRAS_DIR={CUSTOM_EXTRAS}\n", encoding="utf-8")
    yield from _install_with(
        {"SALT_EXTRAS_DIR": CUSTOM_EXTRAS}, request, salt_factories_root_dir
    )
...
def install_harden_upgrade_migration(
    ...
    """
    Simulate the legacy-to-hardened upgrade path:

    1. Install with SALT_ONEDIR_HARDEN=0 (legacy layout).
    2. Drop a marker file into /opt/saltstack/salt/extras-<py>/.
    3. Uninstall + reinstall with SALT_ONEDIR_HARDEN=1.
    ...
    legacy_installer = SaltPkgInstall(
        ...
        prev_version=request.config.getoption("prev_version"),
        use_prev_version=request.config.getoption("use_prev_version"),
        install_env={"SALT_ONEDIR_HARDEN": "0"},
    )
    try:
    ...
    # Step 3: uninstall, then reinstall with hardening on. Legacy
    ...
    hardened_installer = SaltPkgInstall(
        ...
        prev_version=request.config.getoption("prev_version"),
        use_prev_version=request.config.getoption("use_prev_version"),
        install_env={},  # HARDEN unset -> default is 1
    )
    try:
        with contextlib.ExitStack() as stack:
            try:
