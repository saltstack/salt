...
def test_managed_file_with_pillar_sls(
    ...
    {%- set filedir = '"""
        + str(tmp_path)
        + """' %}
    {%- set filename = salt["pillar.get"]("monty", "") %}
    create-file:
      file.managed:
        - name: {{ filedir }}/file-pillar-{{ filename }}-target.txt
    """
    )
    sls_tempfile = pytest.helpers.temp_file(
        "{}.sls".format(sls_name), sls_contents, base_env_state_tree_root_dir
...
def test_issue_50221(
    ...
    expected_content = "abc{0}{0}{0}".format(os.linesep)
    ...
    with sls_tempfile, issue_50221_ext_pillar_tempfile:
        ...
        # Check to make sure the file was created
        assert target_path.is_file()
        assert target_path.read_text() == expected_content
