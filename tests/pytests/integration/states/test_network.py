...
def context(salt_call_cli):
    ...
    salt_call_cli.run('cmd.run', cmd='ip link add {} type dummy'.format(dummy_interface['iface_name']))
    # and backup config file in debian
    if 'debian' in distro.id() + distro.like():
        salt_call_cli.run('cmd.run', cmd='cp -p /etc/network/interfaces /etc/network/interfaces.bkp')
    ...
    if 'debian' in distro.id() + distro.like():
        salt_call_cli.run('cmd.run', cmd='mv /etc/network/interfaces.bkp /etc/network/interfaces')
    else:
        salt_call_cli.run('cmd.run', cmd='rm /etc/sysconfig/network-scripts/ifcfg-{}'.format(dummy_interface['iface_name']))


@pytest.mark.skip_if_not_root
@pytest.mark.skipif('debian' not in distro.id() + distro.like()
...
class TestNetwork():
    def addInterface(self, context, salt_call_cli, salt_master, iname=None, ienabled=None, iproto=None, itype=None, testFlag=False):
        """
        ...
        state_contents = """
            {0}_interface:
              network.managed:
                ...
                - proto: {2}
                - type: {3}
            """.format(iname, ienabled, iproto, itype)
    ...
    def test_managed_addInterface0(self, context, salt_call_cli, salt_master):
        ...
        """
        # Execute test
        ret = self.addInterface(context=context, salt_call_cli=salt_call_cli, salt_master=salt_master, testFlag=True)
        ...
        assert state_run['comment'] == 'Interface {} is set to be added.'.format(context['iface_name'])
    ...
    def test_managed_addInterface1(self, context, salt_call_cli, salt_master):
        ...
        """
        # Execute test
        ret = self.addInterface(context=context, salt_call_cli=salt_call_cli, salt_master=salt_master)
        ...
        assert state_run['comment'] == 'Interface {} added.'.format(context['iface_name'])
        assert state_run['changes']['interface'] == 'Added network interface.'

    def test_managed_existingInterfaceNoChanges0(self, context, salt_call_cli, salt_master):
        """
        network.managed with existing interfaces without changing configs with test flag
        """
        # Add test interface
        self.addInterface(context=context, salt_call_cli=salt_call_cli, salt_master=salt_master)
        ...
        state_run = next(iter(ret.data.values()))
        assert state_run['result'] is True
        assert state_run['comment'] == 'Interface {} is up to date.'.format(context['iface_name'])
        assert state_run['changes'] == {}

    def test_managed_existingInterfaceNoChanges1(self, context, salt_call_cli, salt_master):
        """
    ...
    def test_managed_existingInterfaceWithChanges0(self, context, salt_call_cli, salt_master):
        """
        ...
        state_run = next(iter(ret.data.values()))
        assert state_run['result'] is None
        assert 'Interface {} is set to be updated:'.format(context['iface_name']) in state_run['comment']

    def test_managed_existingInterfaceWithChanges1(self, context, salt_call_cli, salt_master):
        """
        ...
        state_run = next(iter(ret.data.values()))
        assert state_run["result"] is True
        assert 'Interface {} updated.'.format(context['iface_name']) in state_run['comment']
        assert 'Interface {} is up'.format(context['iface_name']) in state_run['changes']['status']

    def test_managed_existingInterfaceWithChanges2(self, context, salt_call_cli, salt_master):
        """
        ...
        # Execute test
        ret = self.addInterface(context=context, salt_call_cli=salt_call_cli, salt_master=salt_master,
                                ienabled=False, iproto=temp_iproto)
        ...
        assert state_run["result"] is True
        assert 'Interface {} updated.'.format(context['iface_name']) in state_run['comment']
        assert 'Interface {} down'.format(context['iface_name']) in state_run['changes']['status']

    def test_managed_existingInterfaceWithChanges3(self, context, salt_call_cli, salt_master):
        """
        ...
        assert state_run["result"] is True
        assert 'Interface {} updated.'.format(context['iface_name']) in state_run['comment']
        assert 'Interface {} restart to validate'.format(context['iface_name']) in state_run['changes']['status']
    ...
    def test_routes(self, salt_call_cli, salt_master):
        ...
        if 'debian' in distro.id() + distro.like():
            assert state_run["result"] is None
            assert state_run['comment'] == "Interface {} routes are set to be added.".format(test_route)
        else:
            assert state_run["result"] is True
            assert state_run['comment'] == "Interface {} routes are up to date.".format(test_route)
    ...
    def test_system(self, salt_call_cli, salt_master):
        ...
        state_run = next(iter(ret.data.values()))
        assert state_run["result"] is None
        assert 'Global network settings are set to be {}'.format(
                'added' if not global_settings else 'updated') in state_run['comment']
