...
def test_list_pkgs(salt_cli, salt_delta_proxy):
    ...
    is working OK.
    """
    ret = salt_cli.run("pkg.list_pkgs", minion_tgt="dummy_proxy_one")
    assert "coreutils" in ret.json
    assert "apache" in ret.json
    assert "redbull" in ret.json

    ret = salt_cli.run("pkg.list_pkgs", minion_tgt="dummy_proxy_two")
    assert "coreutils" in ret.json
    assert "apache" in ret.json
    assert "redbull" in ret.json
...
def test_install_pkgs(salt_cli, salt_delta_proxy):
    ...
    is working OK.
    """
    ret = salt_cli.run("pkg.install", "thispkg", minion_tgt="dummy_proxy_one")
...
def test_remove_pkgs(salt_cli, salt_delta_proxy):
    ret = salt_cli.run("pkg.remove", "apache", minion_tgt="dummy_proxy_one")
    assert "apache" not in ret.json


def test_upgrade(salt_cli, salt_delta_proxy):
    ret = salt_cli.run("pkg.upgrade", minion_tgt="dummy_proxy_one")
    assert ret.json["coreutils"]["new"] == "2.0"
    assert ret.json["redbull"]["new"] == "1000.99"


def test_service_list(salt_cli, salt_delta_proxy):
    ret = salt_cli.run("service.list", minion_tgt="dummy_proxy_one")
    assert "ntp" in ret.json


def test_service_stop(salt_cli, salt_delta_proxy):
    ret = salt_cli.run("service.stop", "ntp", minion_tgt="dummy_proxy_one")
    ret = salt_cli.run("service.status", "ntp", minion_tgt="dummy_proxy_one")
    assert ret.json is False


def test_service_start(salt_cli, salt_delta_proxy):
    ret = salt_cli.run("service.start", "samba", minion_tgt="dummy_proxy_one")
    ret = salt_cli.run("service.status", "samba", minion_tgt="dummy_proxy_one")
    assert ret.json is True


def test_service_get_all(salt_cli, salt_delta_proxy):
    ret = salt_cli.run("service.get_all", minion_tgt="dummy_proxy_one")
    assert ret.json
    assert "samba" in ret.json


def test_grains_items(salt_cli, salt_delta_proxy):
    ret = salt_cli.run("grains.items", minion_tgt="dummy_proxy_one")
...
def test_state_apply(
    ...
    with pytest.helpers.temp_file("core.sls", core_state, base_env_state_tree_root_dir):
        ret = salt_cli.run("state.apply", "core", minion_tgt="dummy_proxy_one")
...
@pytest.mark.skipif(
    True, reason="Not passing when all tests are run, skipping for now."
)
def test_state_highstate(
    ...
    with pytest.helpers.temp_file(
        ...
        ret = salt_cli.run("state.highstate", minion_tgt="dummy_proxy_one")
        for value in ret.json.values():
            assert value["result"] is True
