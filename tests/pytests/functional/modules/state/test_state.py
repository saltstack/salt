...
@pytest.mark.timeout(60)
def test_parallel_cmd_script_from_master(state, state_tree, tmp_path):
    """
    Regression test for https://github.com/saltstack/salt/issues/65709
    (``cmd.script`` with ``parallel: True`` hangs at ~100% CPU).

    This is the same fork-inherited ZeroMQ socket race as the
    ``file.managed`` regression above, reached through a different
    master file-server call: ``cmd.script`` first downloads the script
    via ``cp.cache_file`` before executing it.  When two ``cmd.script``
    states run with ``parallel: True``, each forked child inherits the
    parent's ZeroMQ REQ socket and the two children race it on
    ``cp.cache_file``; the asyncio loop in each child then spins
    forever waiting for a reply that was consumed by its sibling.

    With the fork-safety fix every forked child drops the inherited
    channel via ``os.register_at_fork()`` and lazily opens a fresh one,
    so the concurrent ``salt://`` script fetches complete normally.

    The timeout marker is the key assertion -- without the fix the run
    never returns.
    """
    out1 = tmp_path / "ran_one.txt"
    out2 = tmp_path / "ran_two.txt"
    ...
    sls_contents = textwrap.dedent(
        ...
        script_one:
          cmd.script:
            - source: salt://parallel_fork_script/one.sh
            - name: one.sh
            - shell: /bin/sh
            - parallel: True
        ...
        script_two:
          cmd.script:
            - source: salt://parallel_fork_script/two.sh
            - name: two.sh
            - shell: /bin/sh
            - parallel: True
        """
    ...
    with pytest.helpers.temp_file(
        "parallel_fork_script/one.sh", script_one, state_tree
    ), pytest.helpers.temp_file(
        "parallel_fork_script/two.sh", script_two, state_tree
    ), pytest.helpers.temp_file(
        "parallel_fork_script.sls", sls_contents, state_tree
    ):
        ret = state.sls("parallel_fork_script", __pub_jid="1")

    for state_return in ret:
        assert state_return.result is True, state_return.comment
    assert out1.read_text().strip() == "one"
    assert out2.read_text().strip() == "two"
