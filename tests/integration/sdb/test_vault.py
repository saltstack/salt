...
class VaultTestCase(ModuleCase, ShellCase):
    ...
    @flaky
    def test_sdb(self):
        set_output = self.run_function('sdb.set', uri='sdb://sdbvault/secret/test/test_sdb/foo', value='bar')
        self.assertEqual(set_output, True)
        get_output = self.run_function('sdb.get', arg=['sdb://sdbvault/secret/test/test_sdb/foo'])
        self.assertEqual(get_output, 'bar')

    @flaky
    def test_sdb_runner(self):
        set_output = self.run_run('sdb.set sdb://sdbvault/secret/test/test_sdb_runner/foo bar')
        self.assertEqual(set_output, ['True'])
        get_output = self.run_run('sdb.get sdb://sdbvault/secret/test/test_sdb_runner/foo')
        self.assertEqual(get_output, ['bar'])

    @flaky
    def test_config(self):
        set_output = self.run_function('sdb.set', uri='sdb://sdbvault/secret/test/test_pillar_sdb/foo', value='bar')
        self.assertEqual(set_output, True)
