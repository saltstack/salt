...
class YumTestCase(TestCase, LoaderModuleMockMixin):
    ...
    def test_pkg_hold_yum(self):
        ...
        cmd = MagicMock(return_value={'retcode': 0})
        with patch.object(yumpkg, 'list_pkgs', MagicMock(return_value=list_pkgs_mock)), \
                patch.object(yumpkg, 'list_holds', MagicMock(return_value=[])), \
                patch.dict(yumpkg.__salt__, {'cmd.run_all': cmd}), \
                patch('salt.utils.systemd.has_scope', MagicMock(return_value=False)):
            yumpkg.hold('foo')
            cmd.assert_called_once_with(
                ['yum', 'versionlock', 'foo'], env={},
                output_loglevel='trace',
                python_shell=False)

        # Test Fedora 20
        cmd = MagicMock(return_value={'retcode': 0})
        with patch.dict(yumpkg.__grains__, {'os': 'Fedora', 'osrelease': 20}), \
                patch.object(yumpkg, 'list_pkgs', MagicMock(return_value=list_pkgs_mock)), \
                patch.object(yumpkg, 'list_holds', MagicMock(return_value=[])), \
                patch.dict(yumpkg.__salt__, {'cmd.run_all': cmd}), \
                patch('salt.utils.systemd.has_scope', MagicMock(return_value=False)):
            yumpkg.hold('foo')
    ...
    def test_pkg_hold_dnf(self):
        ...
        yumpkg.__context__.pop('yum_bin')
        cmd = MagicMock(return_value={'retcode': 0})
        with patch.dict(yumpkg.__grains__, {'osmajorrelease': 8}), \
                patch.object(yumpkg, 'list_pkgs', MagicMock(return_value=list_pkgs_mock)), \
                patch.object(yumpkg, 'list_holds', MagicMock(return_value=[])), \
                patch.dict(yumpkg.__salt__, {'cmd.run_all': cmd}), \
                patch('salt.utils.systemd.has_scope', MagicMock(return_value=False)):
        ...
        yumpkg.__context__.pop('yum_bin')
        cmd = MagicMock(return_value={'retcode': 0})
        with patch.dict(yumpkg.__grains__, {'os': 'Fedora', 'osrelease': 26}), \
                patch.object(yumpkg, 'list_pkgs', MagicMock(return_value=list_pkgs_mock)), \
                patch.object(yumpkg, 'list_holds', MagicMock(return_value=[])), \
                patch.dict(yumpkg.__salt__, {'cmd.run_all': cmd}), \
                patch('salt.utils.systemd.has_scope', MagicMock(return_value=False)):
        ...
        yumpkg.__context__.pop('yum_bin')
        cmd = MagicMock(return_value={'retcode': 0})
        with patch.dict(yumpkg.__grains__, {'os': 'Fedora', 'osrelease': 25}), \
                patch.object(yumpkg, 'list_pkgs', MagicMock(return_value=list_pkgs_mock)), \
                patch.object(yumpkg, 'list_holds', MagicMock(return_value=[])), \
                patch.dict(yumpkg.__salt__, {'cmd.run_all': cmd}), \
                patch('salt.utils.systemd.has_scope', MagicMock(return_value=False)):
