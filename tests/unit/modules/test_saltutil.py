...
class ScheduleTestCase(TestCase, LoaderModuleMockMixin):
    ...
    def test_exec_kwargs(self):
        _cmd_expected_kwargs = {'tgt': s.tgt, 'fun': s.fun, 'arg': s.arg, 'timeout': s.timeout,
                                'tgt_type': s.tgt_type, 'ret': s.ret, 'kwarg': s.kwarg, 'passthrough': s.passthrough}
        ...
        saltutil._exec(client, s.tgt, s.fun, s.arg, s.timeout, s.tgt_type, s.ret, s.kwarg, passthrough=s.passthrough,
                       **{'batch': s.batch})
