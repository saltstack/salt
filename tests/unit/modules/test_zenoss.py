...
class ZenossTestCase(TestCase, LoaderModuleMockMixin):
    ...
    def setup_loader_modules(self):
        return {
            zenoss: {
                "__salt__": {"config.option": config.option}},
            config: {"__opts__": {}},
        }
    ...
    def test_zenoss_session(self):
        ...
        """
        zenoss_conf = {"zenoss": {"hostname": "https://test.zenoss.com",
                                  "username": "admin",
                                  "password": "test123"}}
        ...
        for verify in [True, False, None]:
            ...
            with patch_http, patch_opts:
                ...
                self.assertEqual(mock_http.call_args_list,
                                 [call(ca_bundle=None, headers={'Content-type': 'application/json; charset=utf-8'},
                                       password='test123', user='admin',
                                       verify_ssl=verify)])
