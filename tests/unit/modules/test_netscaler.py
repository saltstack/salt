...
class NetscalerTestCase(TestCase, LoaderModuleMockMixin):
    ...
    def test_servicegroup_exists(self):
        ...
        with patch.dict(netscaler.__salt__, {"config.option": mock}):
            ...
            self.assertFalse(
                netscaler.servicegroup_exists("serviceGrpName", sg_type="HTTP")
            )
    ...
    def test_servicegroup_server_add(self):
        ...
        with patch.dict(netscaler.__salt__, {"config.option": mock}):
            ...
            with patch.object(netscaler, "_servicegroup_get_server", mock):
                ...
                self.assertFalse(
                    netscaler.servicegroup_server_add(
                        "serviceGroupName", "serverName", "serPort"
                    )
                )
    ...
    def test_servicegroup_server_delete(self):
        ...
        with patch.dict(netscaler.__salt__, {"config.option": mock}):
            ...
            self.assertFalse(
                netscaler.servicegroup_server_delete(
                    "serviceGroupName", "serverName", "serverPort"
                )
            )
    ...
    def test_server_exists(self):
        ...
        with patch.dict(netscaler.__salt__, {"config.option": mock}):
            ...
            self.assertFalse(
                netscaler.server_exists("serviceGrpName", s_state="serverName")
            )
    ...
    def test_server_update(self):
        ...
        """
        mock = MagicMock(
            side_effect=[None, MockNSServer(), MockNSServer(), MockNSServer()]
        )
        with patch.object(netscaler, "_server_get", mock):
    ...
    def test_vserver_exists(self):
        ...
        with patch.dict(netscaler.__salt__, {"config.option": mock}):
            ...
            self.assertFalse(
                netscaler.vserver_exists("vserrName", v_ip="", v_port="vserverPort")
            )

            self.assertFalse(
                netscaler.vserver_exists(
                    "vserrName", v_ip="", v_port="", v_type="vserverType"
                )
            )
    ...
    def test_vserver_add(self):
        ...
        with patch.dict(netscaler.__salt__, {"config.option": mock}):
            self.assertFalse(
                netscaler.vserver_add("alex.patate.chaude.443", "1.2.3.4", "443", "SSL")
            )
            ...
            with patch.object(netscaler, "vserver_exists", mock):
                self.assertTrue(
                    netscaler.vserver_add(
                        "alex.pae.chaude.443", "1.2.3.4", "443", "SSL"
                    )
                )

                with patch.object(netscaler, "_connect", MagicMock(return_value=None)):
                    self.assertFalse(
                        netscaler.vserver_add("alex.chde.443", "1.2.3.4", "443", "SSL")
                    )
    ...
    def test_vser_sergrp_exists(self):
        ...
        with patch.dict(netscaler.__salt__, {"config.option": mock}):
            self.assertTrue(
                netscaler.vserver_servicegroup_exists("vserverName", "serviceGroupName")
            )
    ...
    def test_vserver_servicegroup_add(self):
        ...
        with patch.dict(netscaler.__salt__, {"config.option": mock}):
            MockNSNitro.flag = None
            self.assertTrue(
                netscaler.vserver_servicegroup_add("vserverName", "serGroupName")
            )
            ...
            with patch.object(netscaler, "vserver_servicegroup_exists", mock):
                self.assertFalse(
                    netscaler.vserver_servicegroup_add("vserName", "serGroupName")
                )

                with patch.object(netscaler, "_connect", MagicMock(return_value=None)):
                    self.assertFalse(
                        netscaler.vserver_servicegroup_add("vName", "serGroupName")
                    )
    ...
    def test_vser_sergrp_delete(self):
        ...
        with patch.dict(netscaler.__salt__, {"config.option": mock}):
            self.assertFalse(
                netscaler.vserver_servicegroup_delete("vservName", "serGroupName")
            )
            ...
            with patch.object(netscaler, "vserver_servicegroup_exists", mock):
                MockNSNitro.flag = None
                self.assertTrue(
                    netscaler.vserver_servicegroup_delete("vName", "serGroupName")
                )

                with patch.object(netscaler, "_connect", MagicMock(return_value=None)):
                    self.assertFalse(
                        netscaler.vserver_servicegroup_delete(
    ...
    def test_vserver_sslcert_exists(self):
        ...
        with patch.dict(netscaler.__salt__, {"config.option": mock}):
            self.assertTrue(
                netscaler.vserver_sslcert_exists("vserverName", "serviceGroupName")
            )
    ...
    def test_vserver_sslcert_add(self):
        ...
        with patch.object(netscaler, "vserver_sslcert_exists", mock):
            ...
            with patch.dict(netscaler.__salt__, {"config.option": mock}):
                MockNSNitro.flag = None
                self.assertTrue(
                    netscaler.vserver_sslcert_add("vserverName", "serGroupName")
                )
    ...
    def test_vserver_sslcert_delete(self):
        ...
        with patch.object(netscaler, "vserver_sslcert_exists", mock):
            ...
            with patch.dict(netscaler.__salt__, {"config.option": mock}):
                MockNSNitro.flag = None
                self.assertTrue(
                    netscaler.vserver_sslcert_delete("vservName", "serGroupName")
                )

            with patch.object(netscaler, "_connect", MagicMock(return_value=None)):
                self.assertFalse(
                    netscaler.vserver_sslcert_delete("vserverName", "serGroupName")
                )
