...
class GlusterfsTestCase(TestCase):
    ...
    def test_create(self):
        ...
        with patch.dict(glusterfs.__salt__, {'cmd.run': mock}):
            self.assertRaises(SaltInvocationError, glusterfs.create, 'newvolume', 'host1:brick')
    ...
    def test_status(self):
        ...
        mock = MagicMock(return_value=xml_command_fail)
        with patch.dict(glusterfs.__salt__, {'cmd.run': mock}):
            self.assertRaises(CommandExecutionError, glusterfs.status, 'myvol1')

        mock = MagicMock(return_value=xml_volume_status)
        with patch.dict(glusterfs.__salt__, {'cmd.run': mock}):
            self.assertDictEqual(glusterfs.status('myvol1'),{
                'bricks': {'node01:/tmp/foo': {'host': 'node01',
                                                'hostname': 'node01',
                                                'online': True,
                                                'path': '/tmp/foo',
    ...
    def test_volume_info(self):
        ...
        with patch.dict(glusterfs.__salt__, {'cmd.run': mock}):
            self.assertDictEqual(glusterfs.info('myvol1'), {'myvol1': {'brickCount': '1',
                    'bricks': {'brick1': {'hostUuid': '830700d7-0684-497c-a12c-c02e365fb90b',
                                          'path': 'node01:/tmp/foo',
    ...
    def test_start_volume(self):
        ...
        mock = MagicMock(return_value=xml_command_fail)
        with patch.dict(glusterfs.__salt__, {'cmd.run': mock}):
            self.assertRaises(CommandExecutionError, glusterfs.start_volume, 'Newvolume1')
    ...
    def test_stop_volume(self):
        ...
        with patch.object(glusterfs, 'status', mock):
            ...
            mock = MagicMock(return_value=xml_command_fail)
            with patch.dict(glusterfs.__salt__, {'cmd.run': mock}):
                self.assertRaises(CommandExecutionError, glusterfs.stop_volume, 'Newvolume1')
    ...
    def test_delete(self):
        ...
        with patch.object(glusterfs, 'list_volumes', mock):
            ...
            self.assertRaises(SaltInvocationError, glusterfs.delete, 'Newvolume3')
            ...
            with patch.object(glusterfs, 'info', mock):
                ...
                with patch.dict(glusterfs.__salt__, {'cmd.run': mock}):
                    self.assertRaises(SaltInvocationError, glusterfs.delete, 'Newvolume1', False)
    ...
    def test_add_volume_bricks(self):
        ...
        mock = MagicMock(return_value=xml_command_fail)
        with patch.dict(glusterfs.__salt__, {'cmd.run': mock}):
            self.assertRaises(CommandExecutionError, glusterfs.add_volume_bricks, 'Newvolume1', ['bricks'])
        ...
        mock = MagicMock(return_value={'bricks': {'brick1': {'path': 'bricks'}}})
        with patch.object(glusterfs, 'info', mock):
