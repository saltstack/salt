...
class GpgTestCase(TestCase, LoaderModuleMockMixin):
    ...
    def test_helper_salt_user(self):
        ...
        with patch.dict(
            gpg.__salt__,
            {"user.info": MagicMock(return_value={"home": self.gnupghome})},
        ), patch.dict(
            gpg.__salt__, {"config.get": MagicMock(return_value=self.gnupghome)}
        ), patch.object(
            gpg.gnupg, "GPG", return_value=None
        ):
    ...
    def test_helper_default_user(self):
        ...
        with patch.dict(
            ...
            gpg.__salt__, {"config.option": MagicMock(return_value="root")}
        ), patch.object(
            gpg.gnupg, "GPG", return_value=None
        ):
            res = gpg._create_gpg()
    ...
    def test_helper_invalid_user(self):
        ...
        """
        with patch.dict(gpg.__salt__, {"user.info": MagicMock(return_value=None)}):
    ...
    def test_handling_no_gnupg_object(self):
        ...
        with patch.dict(
            ...
            gpg, "get_key", return_value={"fingerprint": "bar"}
        ), patch.object(
            gpg, "_create_gpg", return_value=None
        ):
            for fun, kwargs in {
                ...
                self.assertEqual(
                    {"name": fun, "res": getattr(gpg, fun)(**kwargs)},
                    {
                        ...
                        "res": {
                            "result": False,
                            "message": "Unable to initialize GPG.",
                        },
                    },
                )
    ...
    def test_list_keys(self):
        ...
        with patch.dict(
            ...
            gpg, "_list_keys", return_value=self.list_result
        ):
    ...
    def test_delete_key_happy(self):
        ...
        with patch.dict(
            ...
            self.gpgobject, "delete_keys", MagicMock(return_value="ok")
        ):
    ...
    def test_delete_key_fail_custom(self):
        ...
        with patch.dict(
            ...
            gpg, "get_key", return_value=get_key_result
        ), patch.object(
            gpg, "get_secret_key", return_value=None
        ), patch.object(
        ...
        self.assertEqual(
            ...
            {
                "result": False,
                "message": "Failed to delete public key {}: not ok".format(
                    "key_fingerprint"
                ),
            },
        )
    ...
    def test_search_keys(self):
        ...
        with patch.dict(
            ...
            gpg, "_search_keys", return_value=_search_result
        ):
    ...
    def test_create_key_happy(self):
        ...
        with patch.dict(
            ...
            self.gpgobject, "gen_key", MagicMock(return_value=gen_key_result)
        ):
            res = gpg.create_key(
    ...
    def test_create_key_happy_pillar(self):
        ...
        with patch.dict(
            ...
            gpg.__salt__, {"pillar.get": MagicMock(return_value="secret")}
        ), patch.object(
            gpg, "_create_gpg", return_value=self.gpgobject
        ), patch.object(
            self.gpgobject, "gen_key", MagicMock(return_value=gen_key_result)
    ...
    def test_create_key_fail(self):
        ...
        with patch.dict(
            ...
            gpg.__salt__, {"pillar.get": MagicMock(return_value=None)}
        ), patch.object(
            gpg, "_create_gpg", return_value=self.gpgobject
        ), patch.object(
            self.gpgobject, "gen_key", MagicMock(return_value=False)
        ):
    ...
    def test_import_key_fail_manual(self):
        ...
        with patch.dict(
            ...
            self.gpgobject, "import_keys", MagicMock(return_value=None)
        ):
            res = gpg.import_key(text=self.secret_key, gnupghome=self.gnupghome)
        self.assertDictEqual(
            res,
    ...
    def test_export_keys(self):
        ...
        with patch.dict(
            ...
            self.gpgobject, "export_keys", MagicMock(return_value=export_key_result)
        ):
    ...
    def test_trust_key_happy(self):
        ...
        cmd_run_result = [
            {"retcode": 0, "stderr": "4"},
            {"retcode": 0, "stderr": "2:4"},
        ]
        with patch.object(gpg, "get_key", MagicMock(return_value=gpgkey)), patch.object(
            salt.utils.versions, "version_cmp", MagicMock(return_value=-1)
        ), patch.dict(
    ...
    def test_trust_key_fail_generic(self):
        ...
        with patch.object(
            gpg, "get_key", MagicMock(return_value={"keyid": "butnofingerprint"})
        ):
            self.assertEqual(
    ...
    def test_trust_key_fail_ge042(self):
        ...
        with patch.object(
            ...
            gpg, "get_key", return_value={"fingerprint": "foo"}
        ), patch.object(
    ...
    def test_sign_message_happy(self):
        ...
        m_open = mock_open()
        with patch.object(gpg, "_create_gpg", return_value=self.gpgobject), patch.dict(
            gpg.__salt__, {"pillar.get": MagicMock(return_value={"secret": "this is"})}
        ), patch.object(gpg, "_list_keys", return_value=self.list_result), patch.object(
            self.gpgobject, "sign", return_value=signed_data
        ), patch.object(
            salt.utils.files, "flopen", m_open
        ):
    ...
    def test_sign_file_happy(self):
        ...
        with patch.object(
            ...
            self.gpgobject, "sign_file", return_value=signed_data
        ), patch.object(
            salt.utils.files, "flopen", m_open
        ):
            # Sign with provided passphrase, keyid
    ...
    def test_sign_message_happy_gnupg035(self):
        ...
        with patch.object(
            gpg, "_create_gpg", return_value=self.gpgobject
        ), patch.object(self.gpgobject, "sign", return_value=signed_data), patch.object(
            salt.utils.versions, "version_cmp", MagicMock(return_value=-1)
        ), patch.object(
            salt.utils.files, "flopen", m_open
        ):
    ...
    def test_sign_message_fail(self):
        ...
        with patch.object(
            gpg, "_create_gpg", return_value=self.gpgobject
        ), patch.object(gpg, "_list_keys", return_value=[]), patch.dict(
            gpg.__salt__, {"pillar.get": MagicMock(return_value=None)}
        ), patch.object(
            self.gpgobject, "sign", return_value=signed_data
        ), patch.object(
    ...
    def test_sign_fail_manual(self):
        ...
        with patch.dict(
            ...
            self.gpgobject, "sign", return_value=None
        ):
    ...
    def test_receive_keys_happy(self):
        ...
        with patch.dict(
            ...
            self.gpgobject, "recv_keys", return_value=recv_result
        ):
        ...
        self.assertEqual(
            ...
            {
                "result": True,
                "message": "Key D5D468D13E79442E92BBB9E50B3B4060449522BF added to keychain",
            },
        )
        self.assertEqual(res_1, res_2)
    ...
    def test_receive_keys_fail_manual(self):
        ...
        with patch.dict(
            ...
            self.gpgobject, "recv_keys", return_value=None
        ):
    ...
    def test_verify_happy(self):
        ...
        with patch.dict(
            ...
            self.gpgobject, "verify", return_value=verify_result
        ), patch.object(
            self.gpgobject, "verify_file", return_value=verify_result
        ), patch.dict(
            gpg.__salt__, {"cp.cache_file": MagicMock(return_value="foobar")}
        ), patch.object(
            salt.utils.files, "safe_rm", return_value=True
        ), patch.object(
            salt.utils.files, "fopen", m_open
        ):
    ...
    def test_verify_cached_signature(self):
        ...
        with patch.dict(
            ...
            self.gpgobject, "verify_data", return_value=verify_result
        ), patch.object(
            salt.utils.files, "flopen", file_mock
        ):
            res = gpg.verify(
                text="Text to sign",
    ...
    def test_verify_fail(self):
        ...
        with patch.dict(
            ...
            gpg.__salt__, {"cp.cache_file": MagicMock(return_value=False)}
        ), patch.object(
            salt.utils.files, "mkstemp", return_value="/noooo"
        ), patch.object(
            salt.utils.files, "safe_rm", return_value=True
        ), patch.object(
            salt.utils.files, "fopen", m_open
        ...
        self.assertEqual(
            ...
            {
                "result": False,
                "message": "Failed to store signature in tempfile: [Errno {}] Permission denied.".format(
                    errno.EACCES
                ),
            },
        )
    ...
    def test_encrypt_happy(self):
        ...
        with patch.object(gpg, "_create_gpg", return_value=self.gpgobject), patch.dict(
            ...
            self.gpgobject, "encrypt", return_value=encrypted_data
        ), patch.object(
            self.gpgobject, "encrypt_file", return_value=encrypted_data
        ), patch.object(
            salt.utils.files, "flopen", m_open
        ):
            ...
            res_2 = gpg.encrypt(
                text="secret data", recipients=None, passphrase="secret"
            )
    ...
    def test_encrypt_fail(self):
        ...
        with patch.object(gpg, "_create_gpg", return_value=self.gpgobject), patch.dict(
            ...
            self.gpgobject, "encrypt", return_value=MagicMock()
        ), patch.object(
            salt.utils.files,
            "flopen",
            mock_open(read_data={"foobar": IOError(errno.EACCES, "Permission denied")}),
    ...
    def test_decrypt_happy(self):
        ...
        with patch.object(
            ...
            self.gpgobject, "decrypt", return_value=decrypted_data
        ), patch.object(
            self.gpgobject, "decrypt_file", return_value=decrypted_data
        ), patch.object(
            salt.utils.files, "flopen", m_open
        ):
    ...
    def test_decrypt_fail(self):
        ...
        with patch.object(gpg, "_create_gpg", return_value=self.gpgobject), patch.dict(
            ...
            self.gpgobject, "decrypt", return_value=decrypt_result
        ), patch.object(
            salt.utils.files,
            "flopen",
            mock_open(read_data={"foobar": IOError(errno.EACCES, "Permission denied")}),
    ...
    def test_get_fingerprint_from_data(self):
        ...
        with patch.dict(
            gpg.__salt__, {"cmd.run_stdout": MagicMock(return_value=cmd_result)}
        ):
