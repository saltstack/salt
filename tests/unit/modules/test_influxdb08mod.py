...
class InfluxTestCase(TestCase):
    ...
    def test_db_list(self):
        ...
        with patch.object(influx08, '_client', mock_inf_db_client):
            assert influx08.db_list(user='root',
                                              password='root',
                                              host='localhost',
                                              port=8086) == DB_LIST
    ...
    def test_db_exists(self):
        ...
        with patch.object(influx08, 'db_list', side_effect=[[{'name': 'A'}],
                                                          None]):
            assert influx08.db_exists(name='A',
                                               user='root',
                                               password='root',
                                               host='localhost',
                                               port=8000)

            assert not influx08.db_exists(name='A',
                                                user='root',
                                                password='root',
                                                host='localhost',
                                                port=8000)
    ...
    def test_db_create(self):
        ...
        with patch.object(influx08, 'db_exists', side_effect=[True, False]):
            assert not influx08.db_create(name='A',
                                                user='root',
                                                password='root',
                                                host='localhost',
                                                port=8000)
            ...
            mock_inf_db_client = MagicMock(return_value=MockInfluxDBClient())
            with patch.object(influx08, '_client', mock_inf_db_client):
                assert influx08.db_create(name='A',
                                                   user='root',
                                                   password='root',
                                                   host='localhost',
                                                   port=8000)
    ...
    def test_db_remove(self):
        ...
        with patch.object(influx08, 'db_exists', side_effect=[False, True]):
            assert not influx08.db_remove(name='A',
                                                user='root',
                                                password='root',
                                                host='localhost',
                                                port=8000)
            ...
            mock_inf_db_client = MagicMock(return_value=MockInfluxDBClient())
            with patch.object(influx08, '_client', mock_inf_db_client):
                assert influx08.db_remove(name='A',
                                                   user='root',
                                                   password='root',
                                                   host='localhost',
                                                   port=8000)
    ...
    def test_user_list(self):
        ...
        mock_inf_db_client = MagicMock(return_value=MockInfluxDBClient())
        with patch.object(influx08, '_client', mock_inf_db_client):
            assert influx08.user_list(database='A',
                                                    user='root',
                                                    password='root',
                                                    host='localhost',
                                                    port=8086) == USER_LIST

            assert influx08.user_list(user='root',
                                                    password='root',
                                                    host='localhost',
                                                    port=8086) == USER_LIST
    ...
    def test_user_exists(self):
        ...
        with patch.object(influx08, 'user_list', side_effect=[[{'name': 'A'}],
                                                              None]):
            assert influx08.user_exists(name='A',
                                                 user='root',
                                                 password='root',
                                                 host='localhost',
                                                 port=8000)

            assert not influx08.user_exists(name='A',
                                                  user='root',
                                                  password='root',
                                                  host='localhost',
                                                  port=8000)
    ...
    def test_user_chpass(self):
        ...
        with patch.object(influx08, 'user_exists', return_value=False):
            assert not influx08.user_chpass(name='A',
                                                  passwd='*',
                                                  user='root',
                                                  password='root',
                                                  host='localhost',
        ...
        with patch.object(influx08, '_client', mock_inf_db_client):
            with patch.object(influx08, 'user_exists', return_value=True):
                assert influx08.user_chpass(name='A',
                                                     passwd='*',
                                                     user='root',
                                                     password='root',
                                                     host='localhost',
    ...
    def test_user_remove(self):
        ...
        '''
        with patch.object(influx08, 'user_exists', return_value=False):
            assert not influx08.user_remove(name='A',
                                                  user='root',
                                                  password='root',
                                                  host='localhost',
                                                  port=8000)
        ...
        with patch.object(influx08, '_client', mock_inf_db_client):
            with patch.object(influx08, 'user_exists', return_value=True):
                assert influx08.user_remove(name='A',
                                                     user='root',
                                                     password='root',
                                                     host='localhost',
                                                     port=8000)
    ...
    def test_query(self):
        ...
        mock_inf_db_client = MagicMock(return_value=MockInfluxDBClient())
        with patch.object(influx08, '_client', mock_inf_db_client):
            assert influx08.query(database='db',
                                           query='q',
                                           user='root',
                                           password='root',
                                           host='localhost',
