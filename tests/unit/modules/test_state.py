...
class StateTestCase(TestCase, LoaderModuleMockMixin):
    ...
    def test_low(self):
        ...
        with patch.object(state, '_check_queue', mock):
            assert not state.low({"state": "pkg", "fun": "installed",
                                        "name": "vi"})

            MockState.State.flag = False
            assert state.low({"state": "pkg", "fun": "installed",
                                        "name": "vi"}) == list
    ...
    def test_single(self):
        ...
        with patch.object(state, '_check_queue', mock):
            assert state.single("pkg.installed",
                                          " name=vim") == "A"

            assert state.single("pk", "name=vim") == \
    ...
    def test_show_state_usage(self):
        ...
        with patch.object(state, '_check_queue', mock):
            ...
            with pytest.raises(SaltInvocationError):
                state.show_state_usage(pillar="A")
            ...
            with patch.dict(state.__opts__, {"test": "A"}):
                ...
                with patch.object(salt.utils.state, 'get_sls_opts', mock):
                    ...
                    with patch.object(salt.utils.args, 'test_mode', mock):
                        ...
                        MockState.State.flag = False
                        assert state.sls_id("ABC", "http") == \
                                             {'': 'ABC'}
                        with pytest.raises(SaltInvocationError):
                            state.sls_id("DEF", "http")
    ...
    def test_sls(self):
        ...
        with patch.object(state, '_wait', mock):
            ...
            with patch.object(state, '_disabled', mock):
                with patch.dict(state.__context__, {"retcode": 1}):
                    assert state.sls("core,edit.vim dev",
                                               None,
                                               None,
                                               True) == \
                                     ["A"]
    ...
    def test_get_test_value(self):
        ...
        with patch.dict(state.__opts__, {test_arg: True}):
            assert state._get_test_value(test=None), \
                            'Failure when {0} is True in __opts__'.format(test_arg)

        with patch.dict(config.__pillar__, {test_arg: 'blah'}):
            assert not state._get_test_value(test=None), \
                            'Failure when {0} is blah in __opts__'.format(test_arg)
        ...
        with patch.dict(config.__opts__, {test_arg: False}):
            assert not state._get_test_value(test=None), \
                            'Failure when {0} is False in __opts__'.format(test_arg)

        with patch.dict(config.__opts__, {}):
            assert not state._get_test_value(test=None), \
                            'Failure when {0} does not exist in __opts__'.format(test_arg)

        with patch.dict(config.__pillar__, {test_arg: None}):
            assert state._get_test_value(test=None) is None, \
                            'Failure when {0} is None in __opts__'.format(test_arg)

        with patch.dict(config.__pillar__, {test_arg: True}):
            assert state._get_test_value(test=None), \
                            'Failure when {0} is True in __pillar__'.format(test_arg)

        with patch.dict(config.__pillar__, {'master': {test_arg: True}}):
            assert state._get_test_value(test=None), \
                            'Failure when {0} is True in master __pillar__'.format(test_arg)

        with patch.dict(config.__pillar__, {'master': {test_arg: False}}):
            with patch.dict(config.__pillar__, {test_arg: True}):
                assert state._get_test_value(test=None), \
                                'Failure when {0} is False in master __pillar__ and True in pillar'.format(test_arg)

        with patch.dict(config.__pillar__, {'master': {test_arg: True}}):
            with patch.dict(config.__pillar__, {test_arg: False}):
                assert not state._get_test_value(test=None), \
                                 'Failure when {0} is True in master __pillar__ and False in pillar'.format(test_arg)
        ...
        with patch.dict(state.__opts__, {}):
            assert state._get_test_value(test=True), \
                            'Failure when test is True as arg'
