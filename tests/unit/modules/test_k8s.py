...
class TestK8SNamespace(TestCase):
    ...
    def test_get_namespaces(self):
        ...
        a = len(res.get("items"))
        proc = salt.utils.subprocess.FdPopen(["kubectl", "get", "namespaces", "-o", "json"], stdout=PIPE)
    ...
    def test_get_one_namespace(self):
        ...
        proc = salt.utils.subprocess.FdPopen(["kubectl", "get", "namespaces", "default", "-o", "json"], stdout=PIPE)
        kubectl_out = salt.utils.json.loads(proc.communicate()[0])
    ...
    def test_create_namespace(self):
        ...
        proc = salt.utils.subprocess.FdPopen(["kubectl", "get", "namespaces", nsname, "-o", "json"], stdout=PIPE)
        kubectl_out = salt.utils.json.loads(proc.communicate()[0])
...
class TestK8SSecrets(TestCase):
    ...
    def test_get_secrets(self):
        ...
        a = len(res.get("items", []))
        proc = salt.utils.subprocess.FdPopen(["kubectl", "--namespace=default", "get", "secrets", "-o", "json"], stdout=PIPE)
        kubectl_out = salt.utils.json.loads(proc.communicate()[0])
        b = len(kubectl_out.get("items", []))
        self.assertEqual(a, b)
    ...
    def test_get_one_secret(self):
        ...
        a = res.get("metadata", {}).get("name", "a")
        proc = salt.utils.subprocess.FdPopen(["kubectl", "--namespace=default", "get", "secrets", name, "-o", "json"], stdout=PIPE)
        kubectl_out = salt.utils.json.loads(proc.communicate()[0])
        b = kubectl_out.get("metadata", {}).get("name", "b")
        self.assertEqual(a, b)
    ...
    def test_create_secret(self):
        ...
        proc = salt.utils.subprocess.FdPopen(["kubectl", "--namespace=default", "get", "secrets", name, "-o", "json"],
                                             stdout=PIPE)
        kubectl_out = salt.utils.json.loads(proc.communicate()[0])
        # if creation is failed, kubernetes return non json error message
    ...
    def test_delete_secret(self):
        ...
        time.sleep(0.1)
        proc = salt.utils.subprocess.FdPopen(["kubectl", "--namespace=default", "get", "secrets", name, "-o", "json"], stdout=PIPE, stderr=PIPE)
...
class TestK8SResourceQuotas(TestCase):
    ...
    def test_get_resource_quotas(self):
        name = self.name
        namespace = self.name
        create_namespace = salt.utils.subprocess.FdPopen(["kubectl", "create", "namespace", namespace], stdout=PIPE)
        create_namespace = salt.utils.subprocess.FdPopen(["kubectl", "create", "namespace", namespace], stdout=PIPE)
        request = """
apiVersion: v1
kind: ResourceQuota
...
""".format(name)
        ...
        create = salt.utils.subprocess.FdPopen(["kubectl", "--namespace={0}".format(namespace),
                                                "create", "-f", filename], stdout=PIPE)
        # wee need to give kubernetes time save data in etcd
        time.sleep(0.2)
        res = k8s.get_resource_quotas(namespace, apiserver_url="http://127.0.0.1:8080")
        a = len(res.get("items", []))
        proc = salt.utils.subprocess.FdPopen(["kubectl", "--namespace={0}".format(namespace),
                                              "get", "quota", "-o", "json"], stdout=PIPE)
        kubectl_out = salt.utils.json.loads(proc.communicate()[0])
        b = len(kubectl_out.get("items", []))
        self.assertEqual(a, b)
    ...
    def test_get_one_resource_quota(self):
        name = self.name
        namespace = self.name
        create_namespace = salt.utils.subprocess.FdPopen(["kubectl", "create", "namespace",
                                                          namespace], stdout=PIPE)
        request = """
apiVersion: v1
kind: ResourceQuota
metadata:
...
""".format(name)
        ...
        a = res.get("metadata", {}).get("name", "a")
        proc = salt.utils.subprocess.FdPopen(["kubectl", "--namespace={0}".format(namespace),
                                              "get", "quota", name, "-o", "json"], stdout=PIPE)
        kubectl_out = salt.utils.json.loads(proc.communicate()[0])
        b = kubectl_out.get("metadata", {}).get("name", "b")
        self.assertEqual(a, b)
    ...
    def test_update_resource_quota(self):
        name = self.name
        namespace = self.name
        create_namespace = salt.utils.subprocess.FdPopen(["kubectl", "create", "namespace", namespace],
                                                         stdout=PIPE)
        request = """
apiVersion: v1
kind: ResourceQuota
...
class TestK8SLimitRange(TestCase):
    ...
    def test_create_limit_range(self):
        ...
        proc = salt.utils.subprocess.FdPopen(["kubectl", "--namespace=default", "get", "limits", name, "-o", "json"],
                                             stdout=PIPE)
        kubectl_out = salt.utils.json.loads(proc.communicate()[0])
        self.assertTrue(isinstance(kubectl_out, dict))
...
""".format(name)
        ...
        create = salt.utils.subprocess.FdPopen(["kubectl", "--namespace=default", "create", "-f", filename],
                                               stdout=PIPE)
        # wee need to give kubernetes time save data in etcd
        time.sleep(0.1)
    ...
    def test_get_limit_ranges(self):
        ...
        a = len(res.get("items", []))
        proc = salt.utils.subprocess.FdPopen(["kubectl", "--namespace=default", "get", "limits", "-o", "json"],
                                             stdout=PIPE)
        kubectl_out = salt.utils.json.loads(proc.communicate()[0])
        b = len(kubectl_out.get("items", []))
        self.assertEqual(a, b)
