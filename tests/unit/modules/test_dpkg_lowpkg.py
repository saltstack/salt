...
class DpkgTestCase(TestCase, LoaderModuleMockMixin):
    ...
    def test_list_pkgs(self):
        ...
        with patch.dict(dpkg.__salt__, {'cmd.run_all': mock}):
            assert dpkg.list_pkgs('httpd') == {}

        mock = MagicMock(return_value={'retcode': 1,
                                       'stderr': 'error',
                                       'stdout': 'Salt'})
        with patch.dict(dpkg.__salt__, {'cmd.run_all': mock}):
            assert dpkg.list_pkgs('httpd') == 'Error:  error'
    ...
    def test_file_list(self):
        ...
        mock = MagicMock(return_value={'retcode': 1,
                                       ...
                                       'stdout': 'Salt'})
        with patch.dict(dpkg.__salt__, {'cmd.run_all': mock}):
            assert dpkg.file_list('httpd') == 'Error:  error'
    ...
    def test_file_dict(self):
        ...
        mock = MagicMock(return_value={'retcode': 1,
                                       ...
                                       'stdout': 'Salt'})
        with patch.dict(dpkg.__salt__, {'cmd.run_all': mock}):
            assert dpkg.file_dict('httpd') == 'Error:  error'
