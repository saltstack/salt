...
class IpsetTestCase(TestCase, LoaderModuleMockMixin):
    ...
    def test_new_set(self):
        ...
        self.assertEqual(
            ipset.new_set("s", "bitmap:ip"), "Error: range is a required argument"
        )
    ...
    def test_rename_set(self):
        ...
        self.assertEqual(
            ipset.rename_set("s"), "Error: New name for set needs to be specified"
        )
        ...
        with patch.object(ipset, "_find_set_type", return_value=True):
            self.assertEqual(
                ipset.rename_set("s", "d"), "Error: New Set already exists"
            )
...
comment support",
    ...
    def test_delete(self):
        ...
        with patch.object(ipset, "_find_set_type", return_value=None):
            self.assertEqual(
                ipset.delete("set", "entry"), "Error: Set set does not exist"
            )
    ...
    def test_check(self):
        ...
        with patch.object(ipset, "_find_set_type", return_value=None):
            self.assertEqual(
                ipset.check("set", "entry"), "Error: Set set does not exist"
            )
    ...
    def test_test(self):
        ...
        with patch.object(ipset, "_find_set_type", return_value=None):
            self.assertEqual(
                ipset.test("set", "entry"), "Error: Set set does not exist"
            )
