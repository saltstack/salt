...
pytestmark = pytest.mark.skipif(salt.utils.platform.is_windows(), reason='Not supported on Windows')
...
description:
    ...
    with patch.object(ansible, '_resolver', resolver), \
        patch.object(ansible._resolver, 'load_module', MagicMock(return_value=Module())):
        ret = ansible.help('dummy')
        assert sorted(ret.get('Available sections on module "{0}"'.format(Module().__name__))) == ['one', 'two']
