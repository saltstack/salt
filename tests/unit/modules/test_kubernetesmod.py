    ...
    def test_deployments(self):
        ...
        with mock_kubernetes_library() as mock_kubernetes_lib:
            with patch.dict(kubernetes.__salt__, {'config.option': Mock(side_effect=self.settings)}):
                ...
                assert kubernetes.kubernetes.client.ExtensionsV1beta1Api().list_namespaced_deployment().to_dict.called
                # pylint: enable=E1120
    ...
    def test_pods(self):
        ...
        with mock_kubernetes_library() as mock_kubernetes_lib:
            with patch.dict(kubernetes.__salt__, {'config.option': Mock(side_effect=self.settings)}):
                ...
                # pylint: disable=E1120
                assert kubernetes.kubernetes.client.CoreV1Api(). \
        ...
        with mock_kubernetes_library() as mock_kubernetes_lib:
            with patch('salt.modules.kubernetesmod.show_deployment', Mock(return_value=None)):
                with patch.dict(kubernetes.__salt__, {'config.option': Mock(side_effect=self.settings)}):
                    ...
                    # pylint: disable=E1120
                    assert kubernetes.kubernetes.client.ExtensionsV1beta1Api(). \
    ...
    def test_create_deployments(self):
        ...
        with mock_kubernetes_library() as mock_kubernetes_lib:
            with patch.dict(kubernetes.__salt__, {'config.option': Mock(side_effect=self.settings)}):
                ...
                assert kubernetes.create_deployment("test", "default", {}, {},
                                                              None, None, None) == {}
                # pylint: disable=E1120
                assert kubernetes.kubernetes.client.ExtensionsV1beta1Api(). \
    ...
    def test_setup_kubeconfig_file(self):
        ...
        with mock_kubernetes_library() as mock_kubernetes_lib:
            with patch.dict(kubernetes.__salt__, {'config.option': Mock(side_effect=self.settings)}):
                ...
                assert self.settings('kubernetes.kubeconfig') == \
                    config['kubeconfig']
    ...
    def test_setup_kubeconfig_data_overwrite(self):
        ...
        with mock_kubernetes_library() as mock_kubernetes_lib:
            with patch.dict(kubernetes.__salt__, {'config.option': Mock(side_effect=self.settings)}):
                ...
                assert config['kubeconfig'].lower().startswith(check_path.lower())
                assert os.path.exists(config['kubeconfig'])
