...
# TODO: There are probably better ways to handle this under pytest -W. Werner, 2020-06-02
class LocalOverrideBatchTestCase(BatchTestCase):
    def setUp(self):
        opts = {
            "batch": "",
            "conf_file": {},
        ...
        mock_client = MagicMock()
        mock_client.cmd_iter.return_value = []
        self.batch = Batch(opts, quiet="quiet", local=mock_client)

    # TODO: This and the rest of the valid bnum tests can be pytest.mark.parametrized -W. Werner, 2020-06-02
    def test_get_bnum_str(self):
        """
        Tests passing batch value as a number(str)
        """
        orig_batch_num = "2"
        expected_batch_num = 2
        self.batch.opts = {"batch": orig_batch_num, "timeout": 5}
        self.batch.minions = ["foo", "bar"]
        self.assertEqual(Batch.get_bnum(self.batch), expected_batch_num)
