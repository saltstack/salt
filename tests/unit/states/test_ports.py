...
class PortsTestCase(TestCase, LoaderModuleMockMixin):
    ...
    def test_installed(self):
        ...
        with patch.dict(
            ...
            comt = (
                "security/nmap does not have any build options,"
        ...
        with patch.dict(
            ...
            with patch.dict(ports.__opts__, {"test": True}):
                comt = (
                    "The following options are not available" " for security/nmap: IPV6"
                )
                ret.update({"comment": comt, "result": False})
                self.assertDictEqual(ports.installed(name, options), ret)

                comt = (
                    "security/nmap will be installed with the " "default build options"
                )
                ret.update({"comment": comt, "result": None})
                self.assertDictEqual(ports.installed(name), ret)
