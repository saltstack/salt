...
class PostgresDefaultPrivilegesTestCase(TestCase, LoaderModuleMockMixin):
    ...
    def test_present_schema(self):
        ...
        """
        with patch.dict(
            postgres_default_privileges.__salt__,
            ...
            self.assertDictEqual(
                postgres_default_privileges.present(
                    self.name, self.schema_name, "table"
                ),
                self.ret,
            )
        ...
        with patch.dict(
            ...
            with patch.dict(postgres_default_privileges.__opts__, {"test": True}):
                ...
                self.assertDictEqual(
                    postgres_default_privileges.present(
                        self.name, self.schema_name, "schema", defprivileges=["ALL"]
                    ),
                    self.ret,
                )
    ...
    def test_present_group(self):
        ...
        with patch.dict(
            ...
            with patch.dict(postgres_default_privileges.__opts__, {"test": True}):
                ...
                self.assertDictEqual(
                    postgres_default_privileges.present(
                        self.name, self.group_name, "group"
                    ),
                    self.ret,
                )
    ...
    def test_absent_schema(self):
        ...
        with patch.dict(
            ...
            with patch.dict(postgres_default_privileges.__opts__, {"test": True}):
                comt = (
                    "The requested default privilege(s)"
