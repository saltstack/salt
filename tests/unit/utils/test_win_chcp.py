...
class CHCPTest(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls._chcp_code = win_chcp.get_page_id()
    ...
    def setUp(self):
        win_chcp.set_page_id(self._chcp_code)
    ...
    def test_get_and_set_code_page(self):
        for page in (20424, "20866", 437, 65001, "437"):
            self.assertEqual(win_chcp.set_page_id(page), int(page))
            self.assertEqual(win_chcp.get_page_id(), int(page))

    def test_bad_page_code(self):
        with win_chcp.chcp(437):
            self.assertEqual(win_chcp.get_page_id(), 437)

            bad_codes = ("0", "bad code", 1234, -34, "437 dogs", "(*&^(*^%&$%&")

            for page in bad_codes:
                self.assertEqual(win_chcp.set_page_id(page), -1)
                self.assertEqual(win_chcp.get_page_id(), 437)

            for page in bad_codes:
                self.assertRaises(CodePageError, win_chcp.set_page_id, page, True)
