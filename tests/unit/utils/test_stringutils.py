...
class StringutilsTestCase(TestCase):
    def test_contains_whitespace(self):
        ...
        assert salt.utils.stringutils.contains_whitespace(does_contain_whitespace)
        assert not salt.utils.stringutils.contains_whitespace(does_not_contain_whitespace)

    def test_to_num(self):
        assert 7 == salt.utils.stringutils.to_num('7')
        assert isinstance(salt.utils.stringutils.to_num('7'), int)
        assert 7 == salt.utils.stringutils.to_num('7.0')
        assert isinstance(salt.utils.stringutils.to_num('7.0'), float)
        assert salt.utils.stringutils.to_num('Seven') == 'Seven'
        assert isinstance(salt.utils.stringutils.to_num('Seven'), six.text_type)
    ...
    def test_is_binary(self):
        ...
        assert salt.utils.stringutils.is_binary(bin_str)
        # Also test bytestring
        assert salt.utils.stringutils.is_binary(
    ...
    def test_to_str(self):
        ...
        if six.PY3:
            ...
            with pytest.raises(UnicodeDecodeError):
                salt.utils.stringutils.to_str(ut2, 'utf-8')
            assert salt.utils.stringutils.to_str(ut2, 'utf-8', 'replace') == u'\ufffd'
            with pytest.raises(UnicodeDecodeError):
                salt.utils.stringutils.to_str(bytearray(ut2), 'utf-8')
            assert salt.utils.stringutils.to_str(bytearray(ut2), 'utf-8', 'replace') == u'\ufffd'
        else:
            assert salt.utils.stringutils.to_str('plugh') == str('plugh')  # future lint: disable=blacklisted-function
            assert salt.utils.stringutils.to_str('áéíóúý', 'utf-8') == 'áéíóúý'.encode('utf-8')
            assert salt.utils.stringutils.to_str(UNICODE, 'utf-8') == STR
            assert salt.utils.stringutils.to_str(bytearray(STR), 'utf-8') == STR

            # Test utf-8 fallback with Windows default codepage
            with patch.object(builtins, '__salt_system_encoding__', 'CP1252'):
                assert salt.utils.stringutils.to_str('Ψ') == 'Ψ'.encode('utf-8')

    def test_to_bytes(self):
        for x in (123, (1, 2, 3), [1, 2, 3], {1: 23}, None):
            with pytest.raises(TypeError):
                salt.utils.stringutils.to_bytes(x)
        if six.PY3:
            ...
            assert salt.utils.stringutils.to_bytes('xyzzy') == 'xyzzy'
            assert salt.utils.stringutils.to_bytes(BYTES) == BYTES
            assert salt.utils.stringutils.to_bytes(bytearray(BYTES)) == BYTES
            assert salt.utils.stringutils.to_bytes(UNICODE, 'utf-8') == BYTES

    def test_to_unicode(self):
        assert salt.utils.stringutils.to_unicode(
                EGGS,
                normalize=True
            ) == \
            'яйца'
        ...
        if six.PY3:
            ...
            assert salt.utils.stringutils.to_unicode(str('xyzzy'), 'utf-8') == 'xyzzy'  # future lint: disable=blacklisted-function
            assert salt.utils.stringutils.to_unicode(BYTES, 'utf-8') == UNICODE

            # Test that unicode chars are decoded properly even when using
            # locales which are not UTF-8 compatible
            with patch.object(builtins, '__salt_system_encoding__', 'ascii'):
                assert salt.utils.stringutils.to_unicode('Ψ'.encode('utf-8')) == 'Ψ'
            with patch.object(builtins, '__salt_system_encoding__', 'CP1252'):
                assert salt.utils.stringutils.to_unicode('Ψ'.encode('utf-8')) == 'Ψ'
    ...
    def test_expr_match(self):
        val = 'foo/bar/baz'
        # Exact match
        assert salt.utils.stringutils.expr_match(val, val)
        # Glob match
        assert salt.utils.stringutils.expr_match(val, 'foo/*/baz')
        # Glob non-match
        assert not salt.utils.stringutils.expr_match(val, 'foo/*/bar')
        # Regex match
        assert salt.utils.stringutils.expr_match(val, r'foo/\w+/baz')
        # Regex non-match
        assert not salt.utils.stringutils.expr_match(val, r'foo/\w/baz')
    ...
    def test_check_whitelist_blacklist(self):
        ...
        assert not salt.utils.stringutils.check_whitelist_blacklist(
                'web_one',
        ...
        with pytest.raises(TypeError):
            salt.utils.stringutils.check_whitelist_blacklist('foo', whitelist=123)
        with pytest.raises(TypeError):
            salt.utils.stringutils.check_whitelist_blacklist('foo', blacklist=123)
