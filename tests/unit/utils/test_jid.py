...
class JidTestCase(TestCase):
    def test_jid_to_time(self):
        ...
        assert salt.utils.jid.jid_to_time(test_jid) == expected_jid

        # Test incorrect lengths
        incorrect_jid_length = 2012
        assert salt.utils.jid.jid_to_time(incorrect_jid_length) == ''

    def test_is_jid(self):
        assert salt.utils.jid.is_jid('20131219110700123489')  # Valid JID
        assert not salt.utils.jid.is_jid(20131219110700123489)  # int
        assert not salt.utils.jid.is_jid('2013121911070012348911111')  # Wrong length
    ...
    def test_gen_jid(self):
        ...
        with patch('salt.utils.jid._utc_now', return_value=now):
            ...
            assert ret == '20021225120000000000'
            salt.utils.jid.LAST_JID_DATETIME = None
            ret = salt.utils.jid.gen_jid({'unique_jid': True})
            assert ret == '20021225120000000000_{0}'.format(os.getpid())
            ret = salt.utils.jid.gen_jid({'unique_jid': True})
            assert ret == '20021225120000000001_{0}'.format(os.getpid())
    ...
    def test_gen_jid_utc(self):
        ...
        with patch('salt.utils.jid._utc_now', return_value=utcnow):
            ...
            assert ret == '20021225120700000000'
    ...
    def test_gen_jid_utc_unique(self):
        ...
        with patch('salt.utils.jid._utc_now', return_value=utcnow):
            ...
            assert ret == '20021225120700000000_{0}'.format(os.getpid())
            ret = salt.utils.jid.gen_jid({'utc_jid': True, 'unique_jid': True})
            assert ret == '20021225120700000001_{0}'.format(os.getpid())
