...
class FakeThreadingClass(object):
    name = 'thread-name'
    ...
    def test__read_proc_file_multiprocessing_false(self):
        ...
        with patch('os.getpid', MagicMock(return_value=12345)):
            with patch('salt.utils.files.fopen',
                       mock_open(read_data=b'msgpack')):
                with patch('salt.payload.Serial.loads',
                           MagicMock(return_value=proc_data)):
                    with patch('salt.utils.process.os_is_running',
                               MagicMock(return_value=True)):
                        with patch('threading.enumerate',
                                   MagicMock(return_value=[fake_thread])):
                            with patch('salt.utils.minion._check_cmdline',
                                       MagicMock(return_value=True)):
                                data = salt.utils.minion._read_proc_file('/var/cache/salt/minion/proc/20200310230030623022', opts)
