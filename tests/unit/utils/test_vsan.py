...
class GetVsanClusterConfigSystemTestCase(TestCase, LoaderModuleMockMixin):
    ...
    def setUp(self):
        ...
        patches = (
            (
                ...
                MagicMock(return_value={"vsan-cluster-config-system": self.mock_ret}),
            ),
        )
    ...
    def test_GetVsanVcMos_call(self):
        ...
        with patch("salt.utils.vsan.vsanapiutils.GetVsanVcMos", mock_get_vsan_vc_mos):
            vsan.get_vsan_cluster_config_system(self.mock_si)
        mock_get_vsan_vc_mos.assert_called_once_with(
            self.mock_si._stub, context=self.mock_context
        )
...
class GetVsanDiskManagementSystemTestCase(TestCase, LoaderModuleMockMixin):
    ...
    def setUp(self):
        ...
        patches = (
            (
                "salt.utils.vsan.vsanapiutils.GetVsanVcMos",
                MagicMock(return_value={"vsan-disk-management-system": self.mock_ret}),
            ),
        )
        for mod, mock in patches:
            patcher = patch(mod, mock)
...
class GetHostVsanSystemTestCase(TestCase):
    ...
    def test_hostname_argument(self):
        ...
        with patch(
            ...
            vsan.get_host_vsan_system(
                self.mock_si, self.mock_host_ref, hostname="passedin_hostname"
            )
...
class CreateDiskgroupTestCase(TestCase):
    ...
    def test_get_hostname(self):
        ...
        with patch(
            ...
            vsan.create_diskgroup(
                self.mock_si,
                self.mock_vsan_disk_mgmt_system,
                self.mock_host_ref,
                self.mock_cache_disk,
    ...
    def test_vsan_spec_all_flash(self):
        ...
        self.assertEqual(
            self.mock_spec.capacityDisks, [self.mock_cap_disk1, self.mock_cap_disk2]
        )
...
class GetClusterVsanInfoTestCase(TestCase, LoaderModuleMockMixin):
    ...
    def test_get_vsan_cluster_config_system_call(self):
        ...
        with patch(
            "salt.utils.vsan.get_vsan_cluster_config_system", mock_get_vsan_cl_syst
        ):
            vsan.get_cluster_vsan_info(self.mock_cl_ref)
    ...
    def test_VsanClusterGetConfig_raises_no_permission(self):
        ...
        with patch(
            "salt.utils.vsan.get_vsan_cluster_config_system",
            MagicMock(
                return_value=MagicMock(VsanClusterGetConfig=MagicMock(side_effect=exc))
            ),
        ):
            with self.assertRaises(VMwareApiError) as excinfo:
                vsan.get_cluster_vsan_info(self.mock_cl_ref)
...
class ReconfigureClusterVsanTestCase(TestCase):
    ...
    def test_get_cluster_name_call(self):
        ...
        with patch(
            "salt.utils.vmware.get_managed_object_name", get_managed_object_name_mock
        ):
    ...
    def test_cluster_reconfig_raises_no_permission(self):
        ...
        with patch(
            "salt.utils.vsan.get_vsan_cluster_config_system",
            MagicMock(
                return_value=MagicMock(VsanClusterReconfig=MagicMock(side_effect=exc))
            ),
        ):
            with self.assertRaises(VMwareApiError) as excinfo:
                vsan.reconfigure_cluster_vsan(self.mock_cl_ref, self.mock_cl_vsan_spec)
...
class _WaitForTasks(TestCase, LoaderModuleMockMixin):
    ...
    def test_wait_for_tasks_raises_no_permission(self):
        ...
        exc.privilegeId = "Fake privilege"
        with patch(
            "salt.utils.vsan.vsanapiutils.WaitForTasks", MagicMock(side_effect=exc)
        ):
            with self.assertRaises(VMwareApiError) as excinfo:
                vsan._wait_for_tasks(self.mock_tasks, self.mock_si)
        self.assertEqual(
