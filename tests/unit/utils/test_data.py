...
class DataTestCase(TestCase):
    ...
    def test_decode(self):
        ...
        with pytest.raises(UnicodeDecodeError):
            salt.utils.data.decode(self.test_data,
            keep=False,
            normalize=True,
            preserve_dict_class=True,
            preserve_tuples=True)
        ...
        assert salt.utils.data.decode(EGGS, normalize=True) == 'яйца'
        assert salt.utils.data.decode(EGGS, normalize=False) == EGGS

        # Test binary blob
        assert salt.utils.data.decode(BYTES, keep=True) == BYTES
        with pytest.raises(UnicodeDecodeError):
            salt.utils.data.decode(BYTES,
            keep=False)
    ...
    def test_decode_to_str(self):
        ...
        # Test binary blob
        assert salt.utils.data.decode(BYTES, keep=True, to_str=True) == \
            BYTES
        if six.PY3:
            with pytest.raises(UnicodeDecodeError):
                salt.utils.data.decode(BYTES,
    ...
    def test_encode(self):
        ...
        assert salt.utils.data.encode(BYTES, keep=True) == BYTES
        assert salt.utils.data.encode(BYTES, keep=False) == BYTES
    ...
    def test_encode_keep(self):
        ...
        # Test single string
        assert salt.utils.data.encode(unicode_str, encoding, keep=True) == \
            unicode_str
        with pytest.raises(UnicodeEncodeError):
            salt.utils.data.encode(unicode_str,
            encoding,
            keep=False)
        ...
        assert salt.utils.data.encode(data, encoding,
        ...
        for index, _ in enumerate(data):
            assert salt.utils.data.encode(data[index], encoding,
                                       keep=True, preserve_tuples=True) == \
                data[index]
            with pytest.raises(UnicodeEncodeError):
    ...
    def test_stringify(self):
        with pytest.raises(TypeError):
            salt.utils.data.stringify(9)
        assert salt.utils.data.stringify(['one', 'two', str('three'), 4, 5]) == ['one', 'two', 'three', '4', '5']
    ...
    def test_json_query(self):
        ...
        assert sorted(salt.utils.data.json_query(user_groups, expression)) == \
            primary_groups
...
class FilterFalseyTestCase(TestCase):
    ...
    def test_filter_exclude_types(self):
        ...
        assert ['foo', ['foo'], ['foo'], {'foo': 0}, {'foo': 'bar'}, [{'foo': ''}]] == new_list
        # Ignore list
        old_list = ['foo', ['foo'], ['foo', None], {'foo': 0}, {'foo': 'bar', 'baz': []}, [{'foo': ''}]]
