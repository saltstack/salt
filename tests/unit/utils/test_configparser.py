...
class TestGitConfigParser(TestCase):
    ...
    def _test_write(self, mode):
        ...
        with salt.utils.files.fopen(self.new_config, **kwargs) as fp_:
            self.conf.write(fp_)
        assert self.get_lines(self.new_config) == \
    ...
    def test_read_space_indent(self):
        ...
        '''
        assert self.conf.get('user', 'name') == u'Артём Анисимов'
    ...
    def test_set_new_option(self):
        ...
        assert self.conf.get('http', 'useragent') == 'myawesomeagent'
    ...
    def test_add_section(self):
        ...
        assert self.conf.get('foo', 'bar') == 'baz'
    ...
    def test_replace_option(self):
        ...
        assert self.conf.get('http', 'sslverify') == 'true'
    ...
    def test_set_multivar(self):
        ...
        assert self.conf.get(self.remote, 'fetch') == \
            orig_refspec
        # Add another refspec
        ...
        assert self.get_lines(self.new_config) == expected
    ...
    def test_remove_option(self):
        ...
        for item in ('fetch', 'pushurl'):
            ...
            with pytest.raises(salt.utils.configparser.NoOptionError):
                self.conf.get(self.remote,
                item)
    ...
    def test_remove_option_regexp(self):
        ...
        assert not self.conf.remove_option_regexp(
                self.remote,
                'fetch',
        ...
        assert self.conf.remove_option_regexp(self.remote, 'fetch', 'tags')
        # Confirm that the value is gone
        assert self.conf.get(self.remote, 'fetch') == \
            [orig_refspec, new_refspec_2]
        # Remove the other one we added earlier
        assert self.conf.remove_option_regexp(self.remote, 'fetch', 'foo')
        # Since the option now only has one value, it should be a string
        assert self.conf.get(self.remote, 'fetch') == orig_refspec
        # Remove the last remaining option
        assert self.conf.remove_option_regexp(self.remote, 'fetch', 'heads')
