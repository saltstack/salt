"""
Unit tests for salt.utils.crypt.py
"""


import salt.utils.crypt
from tests.support.mock import patch
from tests.support.unit import TestCase

try:
    from Cryptodome import Random as CryptodomeRandom

    HAS_CYPTODOME = True
except ImportError:
    HAS_CYPTODOME = False


try:
    from Crypto import Random as CryptoRandom  # nosec

    HAS_CRYPTO = True
except ImportError:
    HAS_CRYPTO = False


class CryptUtilsTestCase(TestCase):
    def test_random(self):
        # make sure the right liberty is used for random
        if HAS_CYPTODOME:
            self.assertIs(CryptodomeRandom, salt.utils.crypt.Random)
        elif HAS_CRYPTO:
            self.assertIs(CryptoRandom, salt.utils.crypt.Random)

    def test_reinit_crypto(self):
        # make sure reinit cryptot does not crash
        salt.utils.crypt.reinit_crypto()

        # make sure reinit does not crash when no crypt is found
        with patch("salt.utils.crypt.HAS_CRYPTO", None):
            with patch("salt.utils.crypt.Random", None):
                salt.utils.crypt.reinit_crypto()
