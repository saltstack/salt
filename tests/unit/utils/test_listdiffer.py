...
class ListDictDifferTestCase(TestCase):
    ...
    def test_added(self):
        assert len(self.list_diff.added) == 1
        assert self.list_diff.added[0] == \
                             {'key': 5, 'value': 'foo5', 'int_value': 105}

    def test_removed(self):
        assert len(self.list_diff.removed) == 1
        assert self.list_diff.removed[0] == \
                             {'key': 3, 'value': 'foo3', 'int_value': 103}

    def test_diffs(self):
        assert len(self.list_diff.diffs) == 3
        assert self.list_diff.diffs[0] == \
                             {2: {'int_value': {'new': 112, 'old': 102}}}
        # Added items
        assert self.list_diff.diffs[1] == {5: {'int_value': {'new': 105, 'old': NONE},
                                               'key': {'new': 5, 'old': NONE},
                                               'value': {'new': 'foo5', 'old': NONE}}}
        # Removed items
        assert self.list_diff.diffs[2] == {3: {'int_value': {'new': NONE, 'old': 103},
                                           'key': {'new': NONE, 'old': 3},
                                           'value': {'new': NONE, 'old': 'foo3'}}}

    def test_new_values(self):
        assert len(self.list_diff.new_values) == 2
        assert self.list_diff.new_values[0] == \
                             {'key': 2, 'int_value': 112}
        assert self.list_diff.new_values[1] == \
                             {'key': 5, 'value': 'foo5', 'int_value': 105}

    def test_old_values(self):
        assert len(self.list_diff.old_values) == 2
        assert self.list_diff.old_values[0] == \
                             {'key': 2, 'int_value': 102}
        assert self.list_diff.old_values[1] == \
                             {'key': 3, 'value': 'foo3', 'int_value': 103}

    def test_changed_all(self):
        assert self.list_diff.changed(selection='all') == \
                         ['key.2.int_value', 'key.5.int_value', 'key.5.value',
                          'key.3.int_value', 'key.3.value']

    def test_changed_intersect(self):
        assert self.list_diff.changed(selection='intersect') == \
                         ['key.2.int_value']

    def test_changes_str(self):
        assert self.list_diff.changes_str == \
                         '\tidentified by key 2:\n' \
