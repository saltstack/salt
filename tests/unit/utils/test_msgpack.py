...
class TestMsgpack(TestCase):
    ...
    def test_function_aliases(self):
        ...
        assert not msgpack_items - msgpack_util_items, 'msgpack functions with no alias in `salt.utils.msgpack`'
    ...
    def _test_buffered_base(self, pack_func, unpack_func):
        ...
        assert BytesIO.read != buffer.read
    ...
    def _test_pack_float(self, pack_func, **kwargs):
        assert b'\xca' + struct.pack(str('>f'), 1.0) == pack_func(1.0, use_single_float=True)
        assert b'\xcb' + struct.pack(str('>d'), 1.0) == pack_func(1.0, use_single_float=False)
