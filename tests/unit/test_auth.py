...
class MasterACLTestCase(ModuleCase):
    ...
    def test_master_publish_name(self):
        ...
        with patch('salt.utils.minions.CkMinions.check_minions', MagicMock(return_value=_check_minions_return)):
            ...
            assert self.fire_event_mock.call_args[0][0]['fun'] == 'test.ping'

            # Are we denied access to sys.doc?
            sys_doc_load = self.valid_clear_load
            sys_doc_load['fun'] = 'sys.doc'
            self.clear.publish(sys_doc_load)
            assert self.fire_event_mock.call_args[0][0]['fun'] != 'sys.doc'  # If sys.doc were to fire, this would match
    ...
    def test_master_publish_group(self):
        ...
        with patch('salt.utils.minions.CkMinions.check_minions', MagicMock(return_value=_check_minions_return)):
            ...
            assert self.fire_event_mock.call_args[0][0]['fun'] == 'test.echo'

            # Request sys.doc
            self.valid_clear_load['fun'] = 'sys.doc'
            # Did we fire it?
            assert self.fire_event_mock.call_args[0][0]['fun'] != 'sys.doc'
    ...
    def test_master_publish_some_minions(self):
        ...
        self.clear.publish(self.valid_clear_load)
        assert self.fire_event_mock.mock_calls == []
    ...
    def test_master_minion_glob(self):
        ...
        with patch('salt.utils.minions.CkMinions.check_minions', MagicMock(return_value=_check_minions_return)):  # Assume that there is a listening minion match
            self.clear.publish(self.valid_clear_load)
        assert self.fire_event_mock.called, 'Did not fire {0} for minion tgt {1}'.format(requested_function, requested_tgt)
        assert self.fire_event_mock.call_args[0][0]['fun'] == requested_function, 'Did not fire {0} for minion glob'.format(requested_function)
    ...
    def test_args_empty_spec(self):
        ...
        with patch('salt.utils.minions.CkMinions.check_minions', MagicMock(return_value=_check_minions_return)):
            ...
            self.valid_clear_load.update({'user': 'test_user_func',
                                          ...
                                          'arg': ['TEST']})
            self.clear.publish(self.valid_clear_load)
            assert self.fire_event_mock.call_args[0][0]['fun'] == 'test.empty'
            ...
            self.valid_clear_load.update({'user': 'test_user_func',
                                          ...
                                          'arg': ['alpha',
                                                  ...
                                                  {'kwa': 'kwarg #1',
                                                   ...
                                                   '__kwarg__': True}]})
            self.clear.publish(self.valid_clear_load)
            assert self.fire_event_mock.call_args[0][0]['fun'] == \
                             'my_mod.some_func'
...
class AuthACLTestCase(ModuleCase):
    ...
    def test_acl_simple_allow(self):
        self.clear.publish(self.valid_clear_load)
        assert self.auth_check_mock.call_args[0][0] == \
