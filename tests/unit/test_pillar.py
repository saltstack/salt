...
"""
    ...
    def test_templated_includes(self, tempdir):
        ...
        with fopen(
            os.path.join(d_repeated_inc, "template.sls"), "w"
        ) as f_repeated_inc_tpl:
            print("value: {{ tpl_val }}", file=f_repeated_inc_tpl)
        with fopen(
            os.path.join(d_repeated_inc, "init.sls"), "w"
        ) as f_repeated_inc_init:
            print(
                textwrap.dedent(
                    """
                    include:
        ...
        with fopen(
            os.path.join(d_recursive_inc, "init.sls"), "w"
        ) as f_recursive_inc_init:
            print(
                textwrap.dedent(
                    """
                    include:
        ...
        p_rep_inc = compiled_pillar["test_repeated_inc"]
        p_rec_inc = compiled_pillar["test_recursive_inc"]

        self.assertEqual(p_rep_inc["key1"]["value"], "value1")
        self.assertEqual(p_rep_inc["key2"]["value"], "value2")
        self.assertEqual(p_rec_inc["key1"]["value"], "default value")
        self.assertEqual(
            p_rec_inc["nested"]["test_recursive_inc"]["key1"]["value"], "new value",
        )
