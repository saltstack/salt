...
MISSING_MAN_MSG = """
Error: Missing man pages: {0}

These files can be generated by running 'tools doc man' prior to building.
"""
...
class SaltDistribution(distutils.dist.Distribution):
    ...
    def _property_data_files(self):
        ...
        missing = []
        for file in data_files[0]:
            if os.path.exists(file):
                continue
            missing.append(file)
        if missing:
            sys.stderr.write(
                MISSING_MAN_MSG.format(", ".join(missing))
            )
        return data_files
