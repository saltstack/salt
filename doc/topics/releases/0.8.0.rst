...
The :py:mod:`~saltapi.netapi.rest_cherrypy` netapi module is a regular WSGI
application written using the CherryPy framework. It was written with the
intent of also running from any WSGI-compliant server such as Apache and
mod_wsgi, Gunicorn, uWSGI, Nginx and FastCGI, etc.
