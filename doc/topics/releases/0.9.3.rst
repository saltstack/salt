...
file.recurse
The recurse state function will recursively download a directory on the master
file server and place it on the minion. Any change in the files on the master
will be pushed to the minion. The recuse function is very powerful and has been
tested by pushing out the full Linux kernel source.
...
file.absent
Make sure that the file is not on the system, recursively delets directories,
files and symlinks.
